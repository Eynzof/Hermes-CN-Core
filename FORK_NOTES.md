...
### P-048: Terminal output post-processing pipeline + rtk (reasoning toolkit) integration
...
### P-049: Re-enable bash as an optional explicit shell on Windows
...
**Tested.** `test_windows_bash_found_returns_bash` and `test_windows_bash_not_found_raises_helpful_error` in `tests/tools/test_shell_resolution.py`. All existing PowerShell/auto resolution tests pass unchanged.
