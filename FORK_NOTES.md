...
| **P-052** | `agent/groupchat_loop.py` (new), `agent/groupchat_orchestrator.py` (new), `tui_gateway/server.py`, `hermes_cli/config.py`, `hermes_cli/web_server.py`, `tests/agent/test_groupchat_loop.py` (new), `tests/agent/test_groupchat_orchestrator.py` (new), `tests/tui_gateway/test_groupchat.py` (new) | Adds an in-app multi-agent group chat with bounded Agent-to-Agent hand-offs. Pure helpers own standalone-token mention matching, context projection and a room-pinned relay policy; the independent wave orchestrator owns chain/root/parent metadata, same-wave snapshots, next-wave visibility, serial ordering and depth/turn/time guards. User messages retain the existing direct/`@all`/named routing. A completed Agent reply only relays when valid member mentions occur at the beginning; incidental prose mentions do not route and Agent-authored `@all` is disabled by default. `groupchat.submit` runs as a long handler so `groupchat.interrupt` can stop the active member and suppress remaining dispatches. Message events and REST transcript rows carry sender plus chain metadata. MVP remains serial and in-memory — no parallel fan-out, group-specific compression or cross-restart persistence. | The CN desktop wants an in-app group chat where several profiles (each = a model + SOUL.md persona) converse in one shared transcript, mirroring hermes-studio's feature while allowing a user-requested Agent to hand work to another Agent. Studio's Vue/Koa/Socket.IO + Python-bridge topology is incompatible with `tui_gateway`, so the fork keeps the decision semantics but implements an independent Core-owned orchestrator. | CN-specific feature; the pure mention/projection/orchestration helpers are stack-agnostic and could be upstreamed. Paired with Hermes-CN-Desktop `feat/agent-group-chat`. |
| **P-053** | `hermes_cli/main.py`, `tests/hermes_cli/test_dashboard_unified_launch.py` | Prevents a Desktop-managed Core child from re-claiming the dashboard/webhook/proxy port-lock set already held by its Desktop parent. Standalone CLI dashboards still claim and release the same Core-side locks. | The v0.19 Desktop shell and Core both coordinate ports. Without the managed-child exception, one healthy process tree rejects itself before binding 9120, even though no socket is occupied. | Should be upstreamed as a cross-repo managed-runtime compatibility fix. |
...
**Change.**
- `agent/groupchat_loop.py` (new): pure decision logic — `resolve_mention_targets` / `is_all_agents_mentioned` / `strip_mention_routing_tokens` (standalone-token `@mention` matching with CJK punctuation boundaries and `<quoted_message>` masking, sender excluded, `@all` reserved), `project_group_message` / `build_projected_history` (shared multi-party transcript → one member's first-person user/assistant view: self → `assistant`, others → `[name]:`-prefixed `user`; tool calls/results flattened), plus `GroupChatRelayConfig`, `resolve_agent_relay_targets` and stable hand-off instructions. Only consecutive valid mentions at the beginning of an Agent reply route by default; an incidental mention or Agent-authored `@all` does not.
- `agent/groupchat_orchestrator.py` (new): a deterministic serial wave engine independent of gateway I/O. Every target in one wave receives the same snapshot; the completed wave is visible to the next. It assigns message/chain/root/parent/depth/turn metadata, isolates a failed member, and enforces room-pinned `max_depth`, `max_turns` and `max_chain_seconds`. `GroupChatChainControl` owns cancellation plus the active member's interrupt callback.
- `tui_gateway/server.py`: `groupchat.create` validates member profiles and pins the relay policy. `groupchat.submit` is a long handler, leaving the WS reader free while it invokes profile-backed `AIAgent`s and emits sender/chain-tagged message plus chain lifecycle events. `groupchat.interrupt` signals the chain control, interrupts the active agent and prevents remaining dispatches. Per-room locks reject overlapping submissions without corrupting transcript order.
- `hermes_cli/config.py`: adds `group_chat.auto_relay` defaults (`enabled`, leading-mention requirement, Agent `@all` policy, depth/turn/time guards). The policy is read and pinned at room creation so a conversation's system prompt stays byte-stable.
- `hermes_cli/web_server.py`: `/api/sessions/{id}/messages` serves a `gc_`-prefixed room's **in-memory transcript** instead of the DB. Rows preserve sender and optional chain metadata; absent optional enum fields are omitted rather than serialized as invalid empty strings, so refresh cannot reject the entire transcript.
...
- MVP non-goals: parallel fan-out, declarative workflow graphs, group-specific context compression and cross-restart persistence (rooms are in-memory; a backend restart drops history).
...
**Tested.** `tests/agent/test_groupchat_loop.py`, `tests/agent/test_groupchat_orchestrator.py` and `tests/tui_gateway/test_groupchat.py` cover user routing, leading/incidental/`@all` relay boundaries, same-wave snapshots, next-wave history, depth/turn guards, failure isolation, actual active-agent interruption, sender/chain event attribution and refresh-safe REST rows. The paired Desktop deterministic browser suite covers a planner → critic → synthesizer chain, chain metadata, stop, reload/continue, long streams and failure isolation.
