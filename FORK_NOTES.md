...
|---|---|---|---|---|
| **P-058** | `agent/{auxiliary_client,image_routing,model_metadata,context_compressor,agent_init,agent_runtime_helpers}.py`, `model_tools.py`, `tools/vision_tools.py`, `.github/workflows/release-runtime.yml`, `scripts/stage_frozen_plugins.py`, focused tests | Makes agent construction and context-window initialization strictly offline for remote provider metadata: vision tool gating reads local configuration, explicit credential presence, and the bundled models.dev snapshot without constructing clients or refreshing OAuth; actual provider resolution stays lazy at tool invocation. `allow_network=False` now also skips remote provider/OAuth catalogs while preserving local endpoint probes. The frozen runtime additionally stages physical plugin Python sources and Dashboard APIs under `_internal/plugins` while excluding tests/caches. | On Windows, one cold `session.create` spent tens of seconds refreshing stale Anthropic OAuth and fetching models.dev before the first prompt, then logged false missing-`__init__.py`/`plugin_api.py` warnings because PyInstaller kept Python modules only inside PYZ while the plugin loader scans the filesystem. | Startup/network separation is generic and suitable for upstream; frozen plugin staging is CN Desktop release integration. |
| **P-057** | `hermes_cli/desktop_host.py`, `pyproject.toml`, `.github/workflows/release-runtime.yml`, `scripts/gen_win_version_info.py`, focused tests | Adds a Desktop-only Dashboard entry point and packages it on Windows as the GUI-subsystem `hermes-core-host.exe`; the existing console CLI stays in the same runtime payload for compatibility. The host accepts only Desktop launch parameters, reuses `cmd_dashboard()`, exposes an import-only smoke check, and carries its own Windows version metadata. | Antivirus products commonly terminate a generic console CLI that is spawned to open a local gateway. A narrow, windowless host gives Desktop a stable process identity and removes the console-launch pattern without changing the Dashboard/Gateway protocol or breaking older Desktop versions. | CN Desktop packaging integration; the narrow host pattern may be useful upstream. |
