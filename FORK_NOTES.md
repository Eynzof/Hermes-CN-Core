...
| **P-041** | `hermes_cli/web_server.py`, `tests/hermes_cli/test_web_server_static_mime.py` (new) | Module import pins the MIME types of every asset kind the dashboard bundle ships (`_STATIC_MIME_OVERRIDES`: .js/.mjs → text/javascript, .css, .html, .json, .svg, .wasm, .woff/.woff2) via `mimetypes.add_type`, overriding whatever the OS seeded. | On Windows, `mimetypes` seeds its global map from HKEY_CLASSES_ROOT, where third-party installers commonly rewrite `.js` to `text/plain`. StaticFiles/FileResponse resolve Content-Type through that map, and browsers refuse module scripts served as `text/plain` — affected users got a permanently blank dashboard (#81). | Should be upstreamed (upstream dashboard serves the same bundle through the same code path). |
