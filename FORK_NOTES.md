...
|---|---|---|---|---|
| **P-058** | `cron/scheduler.py`, `hermes_cli/main.py`, `hermes_cli/_cron_script_runner.py`, focused tests | Routes Python cron scripts through a private early entry point when running inside the PyInstaller runtime. The runner executes the script as `__main__` with ordinary Python `argv` and sibling-import behavior, while rechecking resolved-path containment under the active profile's `scripts/` directory before any profile dotenv is loaded. | In a frozen build, `sys.executable` is the Hermes runtime executable rather than a Python interpreter. Cron previously launched `[runtime.exe, script.py]`, so the script path reached the top-level CLI parser as an invalid command and exited with code 2, affecting both `no_agent` jobs and ordinary pre-run scripts (Desktop issue #520). | Generic frozen-runtime correctness and suitable for upstream. Native `.bat` support remains a separate follow-up. |
| **P-057** | `plugins/memory/__init__.py`, `agent/system_prompt.py`, `.github/workflows/release-runtime.yml`, focused tests | Makes bundled memory-provider discovery understand PyInstaller's split layout: manifests remain on disk while provider code is importable through the frozen module loader. The runtime now collects every `plugins.memory` submodule and smoke-tests OpenViking and Hindsight through the actual frozen binary. Managed-Desktop prompts pin the exact active `HERMES_HOME`, correct named-profile path guidance, and require memory verification through the managed CLI/API rather than a direct service probe alone. | `runtime-v0.19.0-cn.6` staged OpenViking/Hindsight manifests but omitted their physical `__init__.py`; directory-only discovery therefore advertised no providers in `/api/memory`. In the affected Desktop session the agent then inspected global `~/.hermes` and mistook a healthy OpenViking service for successful managed-profile configuration. | The importable-module discovery and active-profile path correction are generic; the frozen packaging gate and managed-Desktop guidance are CN-release-specific. |
