...
| **P-042** | `hermes_cli/web_server.py`, `tests/hermes_cli/test_web_server_config_deleted_paths.py` (new) | `PUT /api/config` accepts optional `deleted_paths` (dotted keys, e.g. `providers.custom:foo`), applied after `_deep_merge`; deleting a `providers.<id>` entry also purges root `custom_providers` list entries with the same `base_url`. | `_deep_merge` can only overwrite, never delete — a key the client removed from its PUT body silently survived on disk, so the desktop's "delete custom provider" was a no-op: the entry resurrected from `config.providers` on every reload, and `/api/model/set`-written `custom_providers` residue kept leaking stale metadata (Desktop #370/#188). | Could be upstreamed (upstream dashboard shares the merge-only PUT semantics). |
