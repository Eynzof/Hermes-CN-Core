...
| **P-042** | `hermes_cli/web_server.py`、`tests/hermes_cli/test_web_server_config_deleted_paths.py`（新增） | `PUT /api/config` 新增可选 `deleted_paths`（点分路径，如 `providers.custom:foo`），在 `_deep_merge` 之后应用；删除 `providers.<id>` 时按相同 `base_url` 同步清理根级 `custom_providers` 列表残留。 | `_deep_merge` 只会覆盖、永远不会删除——客户端从 PUT body 里移除的 key 在磁盘上原样存活，桌面端「删除自定义服务商」因此是 no-op：条目每次重载都从 `config.providers` 复活，`/api/model/set` 写入的 `custom_providers` 残留还持续泄漏过期元数据（Desktop #370/#188）。 | 可上游（官方 dashboard 同样是 merge-only PUT 语义）。 |
