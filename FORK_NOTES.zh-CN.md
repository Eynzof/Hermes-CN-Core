...
| **P-041** | `hermes_cli/web_server.py`、`tests/hermes_cli/test_web_server_static_mime.py`（新增） | 模块导入时用 `mimetypes.add_type` 钉死 dashboard 资源包各文件类型的 MIME（`_STATIC_MIME_OVERRIDES`：.js/.mjs → text/javascript、.css、.html、.json、.svg、.wasm、.woff/.woff2），覆盖操作系统种入的映射。 | Windows 上 `mimetypes` 从注册表 HKEY_CLASSES_ROOT 初始化全局映射，第三方安装器常把 `.js` 改写成 `text/plain`；StaticFiles/FileResponse 经这张全局表解析 Content-Type，浏览器拒绝执行 `text/plain` 的 module script——受影响用户的 dashboard 永远白屏（#81）。 | 建议上游（官方 dashboard 走同一条静态服务链路）。 |
