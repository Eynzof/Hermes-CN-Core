...
|---|---|---|---|---|
| **P-066** | `hermes_cli/dashboard_bridge/`（新增）、`hermes_cli/web_server.py`、`tui_gateway/server.py`、`tests/dashboard/test_dashboard_bridge_*.py`（新增） | 新增可选的类型化 Named Pipe 桥接，连接 DLR 数字生命客户端与 Dashboard 会话运行时，通过 `HERMES_DASHBOARD_BRIDGE=1` 开启并接入 `_lifespan`（`dashboard` 与 headless `serve` 都会托管）：有界 4 字节小端长度前缀帧 + 白名单协议（`bridge.hello`/`bridge.readiness`/`conversation.submit`/`conversation.events`/`bridge.close`）、fail-closed 受限 agent（空工具集、关闭记忆与后台审查、所有非文本回调抛错）、单轮不重放会话运行时、以及带严格 DACL 的用户级 Windows Named Pipe（非 Win32 为 no-op）。同时修复 `current_windows_sid()` 的 SID 解析：token 扫描把每个 SID 截断为空串，导致生产 pipe 名变成空串哈希、运行时监听器构造必然失败——而管道 E2E 测试用 UUID pipe 名，从未覆盖真实命名路径。 | DLR（Airi 数字生命）只能通过用户级 Windows Named Pipe 与 Hermes 对话——不接受 URL、headers、token、cookie 或任意 RPC，且无授权时 fail-closed；DLR 不存会话记忆与 Dashboard 凭据，只渲染 Hermes 来源事件。已安装的桌面运行时 `.exe` 不含此桥接，须重建 runtime。 | CN 专属桥接（服务于 DLR 客户端）；仅 Windows 的监听器在其他平台为 no-op，Linux CI 不受影响。 |
