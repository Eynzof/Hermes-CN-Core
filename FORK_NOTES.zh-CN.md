...
|---|---|---|---|---|
| **P-058** | `agent/{auxiliary_client,image_routing,model_metadata,context_compressor,agent_init,agent_runtime_helpers}.py`、`model_tools.py`、`tools/vision_tools.py`、`.github/workflows/release-runtime.yml`、`scripts/stage_frozen_plugins.py`、聚焦测试 | 将 agent 构建与上下文窗口初始化中的远端元数据解析改成严格离线：视觉工具门禁只读取本地配置、显式凭证存在性和内置 models.dev 快照，不再构造 client 或刷新 OAuth；真正调用工具时才懒解析 provider。`allow_network=False` 也会跳过远端 provider/OAuth catalog，同时保留本地端点探测。冻结运行时另外把插件 Python 源码与 Dashboard API 实体文件放入 `_internal/plugins`，并排除测试和缓存。 | Windows 冷启动的一次 `session.create` 曾在首条消息前花费数十秒刷新过期 Anthropic OAuth、访问 models.dev；PyInstaller 又只把 Python 模块放进 PYZ，而插件加载器扫描文件系统，因此产生大量缺少 `__init__.py`/`plugin_api.py` 的伪警告并丢失插件后端。 | 启动期网络隔离是通用修复，适合上游；冻结插件 staging 属于 CN Desktop 发版集成。 |
| **P-057** | `hermes_cli/desktop_host.py`、`pyproject.toml`、`.github/workflows/release-runtime.yml`、`scripts/gen_win_version_info.py`、聚焦测试 | 新增 Desktop 专用 Dashboard 入口，并在 Windows 运行时中以 GUI 子系统的 `hermes-core-host.exe` 打包；原控制台 CLI 仍保留在同一载荷内用于兼容。Host 只接收 Desktop 启动参数，复用 `cmd_dashboard()`，提供仅导入的冒烟检查，并使用独立 Windows 版本元数据。 | 杀毒软件容易终止由桌面外壳拉起、再启动本地网关的通用控制台 CLI。窄职责、无控制台的稳定 Host 身份可移除这种命令行启动特征，同时不改变 Dashboard/Gateway 协议，也不破坏旧版 Desktop。 | CN Desktop 打包集成；窄职责 Host 模式可考虑上游。 |
