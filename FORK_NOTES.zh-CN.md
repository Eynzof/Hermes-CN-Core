...
|---|---|---|---|---|
| P-063 | plugins/tts/moss/transcription.py（新增）、plugins/tts/moss/api.py（新增）、plugins/tts/moss/client.py、plugins/tts/moss/tools.py、plugins/tts/moss/__init__.py、plugins/tts/moss/plugin.yaml、hermes_cli/config_defaults.py、hermes_cli/tools_config.py、docs/moss-tts.md、website/docs（tts.md）、tests/plugins/test_moss_transcription.py（新增）+ test_moss_vision.py（新增）+ test_moss_provider.py + test_moss_tools.py + test_moss_provider_e2e.py | 在既有 Moss 插件（P-062）基础上补齐场景文档剩余两个缺口：(A) **语音转写** — 新增 `MossTranscriptionProvider`（实现 TranscriptionProvider ABC），以 `moss` 名字注册，使 `stt.provider: moss` 将网关语音消息路由到现有插件 STT 分发路径（不改 `BUILTIN_STT_PROVIDERS`）；同时新增受密钥门控的 `moss_transcribe` 工具（multipart 本地文件 / `file_id:` / 公网 URL；`diarize=true` 强制 `moss-transcribe-diarize-pro` 并返回归一化 `segments`；`keyterms` ≤20×≤30 字符；`async_mode` → `task_id` 并支持轮询；512 MB 上限）。(B) **MOSS-VL** — 新增受密钥门控的 `moss_vision` 工具，调用 `POST /v1/responses`（`moss-vl-1.0`）支持 1–5 张图片或 1 个视频（禁止混用；图片 30 MB / 视频 200 MB 上限；拒绝 localhost/内网 URL），本地媒体经 `POST /v1/files` 上传；对 `status=incomplete`/`max_output_tokens` 截断给出告警。两者都由自包含的 `requests` HTTP 层（`plugins/tts/moss/api.py`）承载，复用既有 `tts.moss` 的 key/base_url/timeout 解析，不依赖私有 `moss_tts` SDK 是否发布新客户端方法。新增 `stt.moss` 配置默认值，并在 `hermes tools` 的 Speech-to-Text 选择器注入插件 STT 提供方。已用真实 Moss key（D:/moss.txt）跑通实时 E2E：转写往返、双说话人 diarize、图片 OCR、file_id 视觉流程。 | 抓取的六份 Moss 场景文档覆盖转写/多说话人与图片视频理解，但 fork 之前只实现了 TTS（P-062）——网关语音消息与 Agent 图片/视频理解缺少 Moss 通道。STT 与视觉均停留在 footprint-ladder 第 3/4 级：插件提供方 + 门控工具，刻意不接入核心视觉路由（第三方 SaaS 后端应放在边缘）。 | 按 plan.md 属于树内第三方 SaaS 插件——如需上游化可整体搬迁到 `~/.hermes/plugins/tts/moss/`；自包含 HTTP 层保证其不随 `moss_tts` SDK 发版节奏而失效。 |
| **P-066** | `gateway/config.py`、`hermes_cli/web_server.py`、运行时发布/安装工作流与相关回归测试 | Dashboard 用请求内的凭据作用域读取刚保存的磁盘凭据，中文 IM 渠道尊重显式禁用；冻结包补齐认证/图像插件源码并在真实可执行文件内验证注册；安装测试从 CN 仓选择 Runtime 标签；手工候选构建可在签名前固定产物标签。 | 引导后状态滞后、凭据误启用已关闭渠道，以及仅检查包元数据遗漏按目录加载的插件代码。 | IM 修复可上游化；冻结打包和 CN 标签选择为 fork 专属。 |
...
| **P-061** | `tools/terminal_tool.py`、`tools/process_registry.py`、`tools/environments/base.py`、`model_tools.py`、`tests/tools/test_terminal_process_llm_ergonomics.py`（新增）、`tests/tools/test_vercel_sandbox_environment.py`、`FORK_NOTES.md`、`FORK_NOTES.zh-CN.md` | **terminal + process 工具的 LLM 易用性改造**：(1) `process(action='wait')` 新增 `pattern=`（阻塞式正则等待，`status='matched'`、`wait_matched`）、`inactivity_timeout=`（输出静默后提前返回，pattern 设置时默认 120s）、`since_chars` 游标（只返回新输出）；(2) `read_log`/`wait`/`poll` 新增 `output_path=` 全量日志导出与 `output_total_chars`/`output_truncated` 元数据（后台 spill，对齐前台 spill 文件）；(3) `poll(offset=N)` 变为按行游标读取新输出；(4) **前台超时不再杀进程**：`terminal(promote_on_timeout=true)` 把存活进程收养进后台注册表（registry 新增 `prepare_adopt_local`/`adopt_local`，`BaseEnvironment._wait_for_process` 新增 `promote_callback`），不再 exit 124；(5) `terminal(wait_for_pattern=...)` 命中即返回、进程转为后台会话继续跑；(6) **交互模式 + 会话续接**：`terminal(mode='interactive')`/`interactive=true` 启动持久 shell/REPL（本地、PTY），`terminal(command=..., process_id=<session_id>)` 向该进程 stdin 发送命令并等待其*新*输出（会话续接）；(7) `mode=` 归一化（run/execute/foreground/background/interactive）+ process 工具按工具覆盖别名 `wait`→`block`；(8) 统一结果元数据——`elapsed_seconds`、`exit_code_meaning`、`hint`、`wait_matched`、`output_truncated`、`original_path`。 | terminal + process 工具本身存在对 LLM 后端不利的差距：没有阻塞式 pattern 等待（只有限速的推送式 `watch_patterns`）、`process wait` 没有静默提前返回（活着但不出声的进程会把 agent 阻塞满整个窗口）、前台超时直接杀掉长任务（exit 124）而不是交回 session_id、没有持久交互 shell/会话续接、没有统一结果元数据。这些都会让模型多花轮次重跑命令、盲轮询或丢失工作。 | 全部通用、与后端无关（promotion 先做本地后端），可上游；命名（mode/process_id）只是既有参数之上的糖。关联：P-049（rtk/后处理管线）、P-013（参数键修复）。 |
| **P-062** | `plugins/tts/moss/`（新增）、`agent/tts_provider.py`、`hermes_cli/config_defaults.py`、`tools/tts_tool.py`、`cli-config.yaml.example`、`website/docs`（tts.md、tools-reference、toolsets-reference）、`tests/plugins/test_moss_provider.py` + `test_moss_tools.py`、`tests/tools/test_tts_moss_streaming.py` | **内置 Moss（mosi.cn）TTS 插件**：`MossProvider`（TTSProvider）以 `moss` 注册，`tts.provider: moss` 时 `text_to_speech` 走既有插件分发（不改 BUILTIN_TTS_PROVIDERS）；`MossStreamer`（48kHz PCM）注册到 `tools/tts_streaming`；四个 check_fn 门控的 `moss_*` 工具（对话、声音设计、声音克隆、声音列表）；TTSProvider ABC 新增方法（`synthesize_dialogue` / `design_voice` / `create_voice` / `async_synthesize` / `poll_task`）默认 NotImplementedError；`tts.moss` 配置默认值；密钥解析 config → env → 凭据池 → `MOSS_KEY_FILE` 密钥文件。已用真实 Moss 密钥完成 E2E（同步 mp3/wav、流式、声音列表、克隆+复用、对话、声音设计、异步）。 | Moss（mosi.cn）是中文市场 TTS 服务商，Hermes 原本没有后端；fork 希望以插件形式一等支持，同时保持核心工具 schema 精简（footprint ladder 第 3/4 级）。 | 计划要求的内置第三方 SaaS 插件 —— 如需上流可原样迁到 `~/.hermes/plugins/tts/moss/`。 |
...
**P-062 后续（2026-08-14）。** 继续清理：用 AST 分类器重扫整个 `tests/` 目录（2,956 个文件），由子代理按同一规则对每个命中候选做三分类。删除 **7 个整文件**（全部为源码形状钉死文件：`test_nous_oauth_401_guidance.py`、`test_48031_model_switch_after_auto_reset.py`、`test_adapter_connect_is_reconnect_contract.py`、`test_aiohttp_body_caps.py`、`test_discord_opus.py`、`test_no_shadowed_test_definitions.py`、`test_install_ps1_native_stderr_eap.py`）以及 **70 个文件中的 105 个测试**——`inspect.getsource`/AST/正则源码形状钉死、字面量模型/路由/常量目录钉死、`len()==N` 枚举计数钉死、签名存在性钉死、仅 `hasattr`/`callable` 存在性测试、裸 import 冒烟测试、写了一半的桩测试、以及只测内置函数/Mock 调用的空性能基准。约删除 3,088 行。保留了所有行为 / 回归 / 不变量 / E2E / 不抛异常契约测试（包括 AGENTS.md 认可的 `inspect.signature` 插件兼容契约）。验证：全套仍可收集（32,902 个测试、0 错误）、ruff 对所有改动文件干净、每个被编辑文件均重跑通过。审计清单：仓库根目录 `deletion_plan2.json`；扫描器/规则保留在 `.kimix_cache/test_hygiene/` 下。
...
### P-061：terminal + process 工具的 LLM 易用性——pattern 等待、超时保活、交互会话、统一元数据

**现象 / 需求。** `terminal`/`process` 工具存在对 LLM 后端不利的差距：

1. **没有阻塞式 pattern 等待**——只有限速的推送式 `watch_patterns`。启动服务器的模型只能盲轮询或重跑命令。
2. **`process(action='wait')` 没有静默提前返回**——活着但不出声的进程会把 agent 阻塞满整个窗口。
3. **前台超时直接杀掉长任务**——`_wait_for_process` 在 exit 124 处杀进程；模型只能从头重跑。
4. **没有持久交互 shell / 会话续接**——只有裸的 `process write/submit`；没有“向活 shell 发命令并读它的*新*输出”。
5. **没有统一结果元数据**——`elapsed_seconds`、`wait_matched`、`output_truncated`、`original_path` 缺失或零散；后台 200KB 滚动缓冲被丢弃的中间段不可恢复（没有后台 spill，前台有）。

**改动内容。**

1. **`tools/process_registry.py` —— `wait()` 新增 `pattern` / `inactivity_timeout` / `since_chars`。** `process(action='wait', pattern=r'...')` 一次性编译正则，一旦在*新*输出中出现即返回 `status='matched'` + `wait_matched=true` + `matched_pattern`（进程继续跑）——这是 `watch_patterns` 的阻塞版。`inactivity_timeout=N` 在输出静默 N 秒后提前返回部分输出（`status='timeout'`、`inactivity_timeout=true`、`process_running=true`）；设置 pattern 时默认 120s，保证无声进程永远不会把 agent 卡死。`since_chars=<游标>` 只报告游标之后产生的输出，且（无 pattern 时）一旦有新输出就返回（`status='output'`）——续接游标原语。所有结果现在带 `elapsed_seconds`；pattern 等待在 exit/timeout 时也带 `wait_matched`/`pattern`，模型能区分真匹配与幻觉匹配。
2. **`tools/process_registry.py` —— 后台 spill / 新输出读取。** `read_log(..., output_path=<路径|'auto'>)` 把完整累积缓冲导出到磁盘（`'auto'` 用会话临时文件），并始终报告 `output_total_chars` + `output_truncated`（新增 `_buffer_overflowed` 标志，200KB 窗口回绕即置位——后台版的前台 spill 文件，被丢的中间段可恢复）。`poll(offset=N)` 变成按行游标的新输出读取（`output` = N 之后的行 + `total_lines`）。`poll`/`kill_process` 在非零退出时还报告 `exit_code_meaning`（复用 `_interpret_exit_code`）与 `hint`（失败注解）。新增共享 `_buffer_append()` 统一 pipe/PTY/poller 读线程的滚动追加 + 溢出标志。
3. **前台 → 后台保活（本地后端）。** `BaseEnvironment._wait_for_process` 新增 `wait_for_pattern`（读线程逐 chunk 扫正则，保留 4096 字符滚动尾以覆盖跨 chunk 匹配）与 `promote_callback(proc) -> session_id | None`。pattern 命中**或**超时时，只要有 promote 回调就不再杀进程——`ProcessRegistry.adopt_local()`（配 `prepare_adopt_local()` 提前建会话、由调用方 `output_callback` 预热缓冲）把存活进程注册为受管后台会话，`_wait_for_process` 返回 `{promoted: true, session_id, timed_out|pattern_matched, matched_pattern, elapsed_seconds}`。完成态由既有 `_reconcile_local_exit()` 孤儿管道守卫惰性检测。`terminal_tool(promote_on_timeout=true, wait_for_pattern=...)` 接线：模型启动服务器 → 拿到早期 READY 命中（或超时）→ 用 `process(action='poll'/'wait'/'kill')` 继续。没有回调时历史 kill-124 行为逐字节不变（有测试兜底）。
4. **交互模式 + 会话续接。** `terminal(mode='interactive')` / `interactive=true` 启动持久 shell/REPL（本地后端、PTY；`command` 可省略——默认 `bash -i` / `pwsh -NoLogo -NoExit` / `powershell -NoLogo -NoExit`）。`terminal(command=..., process_id=<session_id>)` —— 新增 `process_id` schema 参数，与内部会话 `session_id` 区分——把命令发到该后台进程的 stdin 并等待其*新*输出（`_continue_background_session` → `submit_stdin` + `wait(since_chars=baseline, pattern=..., inactivity_timeout=...)`），即会话续接。P-013 参数修复表已有 `cmd`→`command`、`bg`→`background`、`patterns`→`watch_patterns` 等；本补丁再加 process 工具按工具覆盖别名 `wait`→`block`（poll 上的 `wait: true` 表示“阻塞”，而不是映射成整数 `timeout`）。
5. **Schema + 统一元数据。** `TERMINAL_SCHEMA` 新增 `mode`、`interactive`、`process_id`、`promote_on_timeout`、`wait_for_pattern`、`inactivity_timeout`；`PROCESS_SCHEMA` 新增 `pattern`、`inactivity_timeout`、`block`、`output_path`，`offset` 描述覆盖 poll。前台 `terminal` 结果新增 `elapsed_seconds`、`output_truncated`、`wait_matched`、`original_path`（token 过滤器保留的未过滤原文），与既有 `full_output_path`/`output_total_chars` spill 元数据并存。

**兼容性。** 纯增量：既有 `wait()`/`read_log()`/`poll()` 调用与 schema 保持原行为（新参数全部 opt-in；`elapsed_seconds`/`output_total_chars`/`output_truncated` 只增不减）。`close_spill()` 现在也封死后续追加（`_spill_capped=True`），避免保活运行残留的排空 chunk 重开并截断已导出的 spill 文件。`tests/tools/test_vercel_sandbox_environment.py` 中两处脆弱的精确 dict 断言按“行为契约优于快照”准则放宽为不变式检查（output + returncode）。
