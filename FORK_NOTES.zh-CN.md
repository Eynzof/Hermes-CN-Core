...
|---|---|---|---|---|
| **P-061** | `cron/scheduler.py`、`hermes_cli/main.py`、`hermes_cli/_cron_script_runner.py`、聚焦测试 | PyInstaller runtime 执行 Python cron 脚本时改走私有早期入口。runner 以 `__main__` 执行脚本，保持普通 Python 的 `argv` 与同目录导入行为，并在加载档案 dotenv 之前复验 resolved path 和 symlink 均位于当前档案的 `scripts/` 目录。 | 冻结构建中的 `sys.executable` 指向 Hermes runtime 可执行文件，并非 Python 解释器。原逻辑启动 `[runtime.exe, script.py]`，脚本路径会被顶层 CLI 解析器当作非法命令并以 code 2 退出，同时影响 `no_agent` 任务和普通 pre-run 脚本（Desktop issue #520）。 | 通用冻结 runtime 正确性修复，适合上游化；原生 `.bat` 支持单独跟踪。 |
