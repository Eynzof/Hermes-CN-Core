...
| **P-052** | `agent/groupchat_loop.py`（新增）、`agent/groupchat_orchestrator.py`（新增）、`tui_gateway/server.py`、`hermes_cli/config.py`、`hermes_cli/web_server.py`、对应测试 | 新增桌面多 Agent 群聊与有界 Agent 自动接力。用户消息继续支持普通全员、`@all`、单成员和多成员路由；Agent 已完成的回复只有在开头连续点名有效成员时才触发下一波，正文 mention 不触发，Agent `@all` 默认禁用。独立波次编排器保证同波快照一致、下一波可见上一波历史，并记录 chain/root/parent/depth/turn 元数据，执行深度、总回合和总时长保护。`groupchat.submit` 作为长 handler 运行，`groupchat.interrupt` 可中断当前成员并阻止剩余成员。房间策略在创建时固定，REST 回读保留 sender 与可选链路字段。 | CN 桌面端需要多个 Profile 在同一 transcript 中协作，并支持“一个 Agent 按用户要求点名另一个 Agent 继续处理”。Hermes Studio 的 Vue/Koa/Socket.IO + Python bridge 拓扑不能直接移植到 `tui_gateway`，因此只借鉴决策语义，独立实现 Core 权威编排器。 | CN 专属功能；纯 mention、上下文投影和有界编排逻辑具备上游复用价值。与 Hermes-CN-Desktop `feat/agent-group-chat` 配对。 |
| **P-053** | `hermes_cli/main.py`、`tests/hermes_cli/test_dashboard_unified_launch.py` | 避免 Desktop 受管 Core 子进程重复申请已由 Desktop 父进程持有的 Dashboard/Webhook/Proxy 端口锁；独立 CLI Dashboard 仍保留 Core 侧原有的申请与释放保护。 | v0.19 的 Desktop 壳和 Core 都新增了端口协调；缺少受管子进程例外时，同一棵健康进程树会在真正绑定 9120 前把自己判为冲突。 | 建议上游，属于双仓受管运行时兼容修复。 |
