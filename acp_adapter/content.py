...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    On native Windows the drive path is kept as-is; on WSL/Linux a Windows drive form
    (Zed via wsl.exe) becomes ``/mnt/<drive>/...``."""
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    # Native Windows must keep the drive path: ``/mnt/<drive>/...`` is a WSL (DrvFS) mount
    # point and is unreadable on the Windows host that received the URI.
    if sys.platform == "win32":
        ...
        return Path(path_text)
    # WSL/Linux: translate a Windows drive path to its DrvFS ``/mnt/<drive>/...`` form.
