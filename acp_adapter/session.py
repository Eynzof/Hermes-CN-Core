...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    # Resolve symlink aliases first so equivalent spellings of the same directory compare equal — macOS
    ...
    # sessions; it is lexical for missing paths. Ported from PrimeIntellect-ai/prime-agent#628.
    # 
    # Which module resolves depends on the path flavour: ``ntpath`` (os.path on Windows) prefixes the
    # current drive to every POSIX path, which would both mangle the WSL mount form below and turn
    # ``/nonexistent/...`` non-lexical, while a Windows drive path on a POSIX host is foreign and must be
    # left to ``windows_path_to_wsl``. ``posixpath`` is ``os.path`` on POSIX hosts, so alias resolution
    # there is unchanged.
    if os.name == "nt":
        path_module = posixpath if expanded.startswith("/") else os.path
    else:
        path_module = None if re.match(r"^[A-Za-z]:[\\/]", expanded) else os.path
    if path_module is not None:
        try:
            expanded = path_module.realpath(expanded)
        except OSError:
            expanded = path_module.normpath(expanded)
...
def _parse_model_config(mc: Any) -> dict:
    ...
    try:
        meta = orjson.loads(mc) if mc else None
    ...
    def _persist(self, state: SessionState) -> None:
        ...
        try:
            if db.get_session(state.session_id) is None:
                ...
                try:
                    db.update_session_meta(state.session_id, orjson.dumps(session_meta).decode('utf-8'), model_str)
                except Exception:
