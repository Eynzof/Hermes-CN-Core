...
def _args_json(arguments: Any) -> str:
    try:
        return orjson.dumps(arguments, default=str, option=orjson.OPT_INDENT_2).decode('utf-8')
...
def _format_structured_value(key: str, value: Any, *, indent: int = 0, max_depth: int = 3, max_items: int = 8) -> List[str]:
    ...
    if value in _EMPTYISH:
        return []
    if max_depth <= 0:
        preview = orjson.dumps(value, default=str).decode('utf-8') if isinstance(value, (dict, list)) else str(value)
