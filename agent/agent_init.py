...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if _api_key_required(agent.provider, effective_key, base_url):
        ...
        raise RuntimeError(f"no API key ({_param_status}, {_env_status})")
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    try:
        _cr_cfg = (
            _agent_cfg.get("compact_reminder", {})
    ...
    if not isinstance(_cr_cfg, dict):
        _cr_cfg = {}
    agent.compact_reminder_enabled = str(_cr_cfg.get("enabled", True)).lower() in {
        "true",
        "1",
        "yes",
    }
    ...
    # Unified reminder registry — context reminders share one injection path into
    # the current turn's user message copy (collection: agent/turn_context.py).
    #
    # [CN-fork] P-059: the fork's ``steer`` user provider was NOT re-registered —
    # upstream now owns /steer end-to-end (agent/interrupt_control.py ``steer()`` +
    # ``_drain_pending_steer()``, drained pre-API by turn_iteration_prep and as
    # ``result["pending_steer"]`` by turn_finalizer), so a registry-backed user
    # provider would be a second, unreachable queue. The fork's ``steer.enabled`` /
    # ``steer.max_length`` config keys only gated that provider (they were never
    # documented in cli-config.yaml.example) and are gone with it.
    from agent.reminder_registry import ReminderRegistry

    agent._reminder_registry = ReminderRegistry()
    if agent.compact_reminder_enabled:
        from agent.compact_reminder import CompactReminderProvider

        agent._reminder_registry.register_system_provider(
            CompactReminderProvider(
                threshold=agent.compact_reminder_threshold,
                cooldown_steps=agent.compact_reminder_cooldown_steps,
            )
        )
...
def _inject_context_engine_tools(agent):
    ...
    if (
        ...
        _existing_tool_names = {
            t.get("function", {}).get("name") for t in (agent.tools or []) if isinstance(t, dict)
        }
...
_CALLBACK_PARAMS = (
    ...
    "step_callback", "stream_delta_callback", "tool_calls_committed_callback",
...
def init_agent(
    ...
    stream_delta_callback: callable = None, tool_calls_committed_callback: callable = None,
