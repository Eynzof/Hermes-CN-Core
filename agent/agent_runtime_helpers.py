...
# Dependency-free leaf primitives. The pre-call sanitizer MUST NOT reach these through
# ``_ra()`` (that made the first sanitize call in a process pay the whole ``run_agent``
# import cascade — 44.28% of a profiled run, see agent/message_utils.py).
from agent.message_utils import (
    STUB_RESULT_CONTENT, VALID_API_ROLES, get_tool_call_id, get_tool_call_name,
...
def _trajectory_tool_call_turn(msg: Dict[str, Any]) -> str:
    ...
    for tool_call in msg["tool_calls"]:
        ...
        try:
            arguments = orjson.loads(raw_args) if isinstance(raw_args, str) else raw_args
...
def _prepend_corruption_marker(tool_msg: dict, marker: str) -> None:
    ...
    if not isinstance(existing, (str, type(None))):
        try:
            existing = orjson.dumps(existing).decode("utf-8")
        except TypeError:
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if inline_executor is None and function_name == "agent_swarm":
        # Fork tool: the registry handler needs the parent agent, so the concurrent
        # path dispatches it through the agent (mirrors tool_executor's branch).
        def _swarm_executor(_agent, args, _ctx):
            return agent._dispatch_agent_swarm(args)

        inline_executor = _swarm_executor
    if (
        inline_executor is None
        and getattr(agent, "_context_engine_tool_names", None)
        and function_name in agent._context_engine_tool_names
    ):
        # Context engine tools (context_usage, compact, and engine-specific tools
        # like lcm_grep) are dispatched through the per-agent context compressor
        # instance.  This mirrors the memory provider tool pattern and
        # tool_executor's sequential context-engine branch.
        def _context_engine_executor(_agent, args, _ctx):
            return agent.context_compressor.handle_tool_call(
                function_name, args, messages=messages or []
            )

        inline_executor = _context_engine_executor
...
def _drop_invalid_roles(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Drop messages whose role the API won't accept. Returns the input object unchanged when
    nothing is dropped so a clean transcript stays byte-stable (and identity-preserved)."""
    valid = VALID_API_ROLES
    dropped = False
    for msg in messages:
        if msg.get("role") not in valid:
            dropped = True
            logger.debug("Pre-call sanitizer: dropping message with invalid role %r", msg.get("role"))
    if not dropped:
        return messages
    return [m for m in messages if m.get("role") in valid]


def _drop_empty_content_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """[CN-fork] P-024: drop empty-content ``assistant``/``user``/``function`` turns that carry no
    payload at all.

    MiMo v2.5 and strict OpenAI-compatible gateways reject a request whose transcript holds a
    message with ``content == ""`` (HTTP 400 / "text is not set"); long sessions can leave such
    turns behind after context compression/truncation. Upstream later replaced this with a
    *substitution* heal (``repair_empty_non_final_messages`` re-writing ``[response interrupted]``);
    the fork keeps the DROP contract — it is exact (no invented turn is replayed to the provider)
    and ``tests/agent/test_sanitize_single_pass.py`` pins the fused pass to a reference multi-pass
    algorithm that has no heal step. Returns ``messages`` itself when nothing is droppable."""
    droppable = [m for m in messages if isinstance(m, dict) and is_empty_content_droppable(m)]
    if not droppable:
        return messages
    logger.debug(
        "Pre-call sanitizer: removed %d empty-content message(s)", len(droppable),
    )
    dropped_ids = {id(m) for m in droppable}
    return [m for m in messages if id(m) not in dropped_ids]
    ...
    for msg in messages:
        ...
        for idx, tc in enumerate(tcs):
            ...
            logger.warning(
                ...
                (name or "")[:80], coerced, get_tool_call_id(tc),
            )
...
def _pair_tool_calls_positionally(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _flush_unanswered_stubs() -> None:
        nonlocal stubs
        # Declaration order (not sorted by id) so the injected stubs read in the order the
        # model declared the calls — the pre-positional pass injected them in that order too.
        for key in declared_calls:
            tc, _variants = declared_calls[key]
            paired.append({
                "role": "tool", "name": get_tool_call_name(tc),
    ...
    for msg in messages:
        ...
        if role == "assistant":
            ...
            # ANY non-tool row closes the tool-result run, so the stub lands immediately
            # after the assistant message that declared the call (strict providers require
            # the results run to be contiguous with the call) — a user turn is only the
            # common case. Later tool messages are orphans.
            _flush_unanswered_stubs()
        paired.append(msg)
    # The transcript may end right after an unanswered assistant turn.
    _flush_unanswered_stubs()
    if dropped:
        logger.debug("Pre-call sanitizer: removed %d positionally orphaned tool result(s)", dropped)
    if stubs:
        logger.debug(
    ...
    for msg in messages:
        if msg.get("role") == "assistant":
            for tc in msg.get("tool_calls") or []:
              ...
              cid = (get_tool_call_id(tc) or "").strip()
              nm = get_tool_call_name(tc)
              if cid and nm:
                  call_names[cid] = nm
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    gated on the compressor). Order matters: empty-content turns are dropped first so a dropped turn
    cannot take part in the pairing and dedup passes.

    [CN-fork] P-024 keeps the fork's DROP contract for empty-content turns (see
    ``_drop_empty_content_messages``). Upstream's ``repair_empty_non_final_messages``
    substitution stays wired as the LAST step — it can no longer re-inflate a dropped
    turn (the empty-string turns are already gone), and it still nets the turns that are
    empty in a different way (``content`` missing/``None``) plus keeps the send-path
    heal bookkeeping/observability upstream tests rely on. Every step below is a no-op
    that returns the input list itself when there is nothing to repair, so a well-formed
    transcript is returned by identity, and no step reaches ``run_agent`` (``_ra()``)."""
    messages = _drop_invalid_roles(messages)
    messages = _drop_empty_content_messages(messages)
    ...
    return repair_empty_non_final_messages(messages)
