...
Text auto chain: main provider+model → custom endpoint → direct API-key
providers → None. Vision auto chain: main provider (if a supported vision backend) →
Anthropic → custom. OpenRouter and Nous Portal are never probed implicitly: they are
used only when they ARE the main provider or an explicit ``auxiliary.<task>.provider``.
...
class _OpenAIProxy:
    ...
    Forwards ``OpenAI(...)`` calls and ``isinstance(x, OpenAI)`` checks to the real SDK class,
    importing the SDK lazily on first use, and layers in the auxiliary-wide defaults:
    ``max_retries=0`` (Hermes owns the aux retry/fallback budget, #54465), a shared keepalive
    ``http_client`` when the caller passed none, and the neutral ``HermesAgent/<version>`` UA
    fallback for gateways whose WAF rejects the SDK UA (HTTP 403 / Cloudflare 1010).
    """
    __slots__ = ()

    def __call__(self, *args, **kwargs):
        # Copy once up front so we never mutate the caller's dict, then layer in the
        # auxiliary-wide defaults.
        kwargs = dict(kwargs)
        # Disable the OpenAI SDK's implicit retry/backoff (its default is max_retries=2). The
        # auxiliary layer already has its own bounded retry-once-on-transient (call_llm) +
        # multi-provider fallback + unhealthy-provider circuit breaker, so SDK-level retries are
        # redundant and, worse, can block the calling thread for up to ``2 x timeout`` on 429s /
        # connection stalls. setdefault preserves any explicit max_retries a caller passes.
        ...
        if "http_client" not in kwargs:
            try:
                ...
                pass
        # Replace the SDK's default "OpenAI/Python ..." UA with a neutral HermesAgent/<version>
        # UA when the caller didn't set one explicitly. Some OpenAI-compatible gateways/WAFs
        # reject the SDK UA with HTTP 403 (Cloudflare error 1010); provider/user-configured
        # default_headers run later at the resolver level and still win.
        try:
...
# Endpoints that reject image input: vision auto-detect skips these to the vision
# fallback chain instead of returning a client that 404s (Kimi Coding Plan Anthropic
# wire has no image_in).
#
# minimax / minimax-cn / minimax-oauth: MiniMax M2.x Anthropic-compatible endpoints
# expose text/reasoning/tool-use models.  The current models.dev entry for MiniMax-M2.7
# reports modalities.input=["text"], and sending image blocks to the endpoint produces a
# normal text refusal ("I don't see an image") rather than useful vision output.  In auto
# mode, skip the main MiniMax endpoint and use a dedicated vision backend instead.
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            exp = orjson.loads(base64.urlsafe_b64decode(payload)).get("exp", 0)
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nous:
        # A *missing* credential is a configuration state, not a payment/limit failure: the
        # unhealthy cache exists for "recently 402'd / rate-limited" providers, and marking nous
        # here would suppress an explicit nous route (or a later login) for the whole TTL.
...
def _get_provider_chain() -> List[tuple]:
    ...
    OpenRouter and Nous Portal are also excluded from this implicit chain (P-039): CN users
    typically cannot reach them, and probing them added a dead network wait to every
    compression / title-gen / vision call. They are still used when they ARE the main
    provider or when a caller explicitly requests them.
    """
    return [
...
def _try_payment_fallback(
    ...
    # The SESSION's runtime provider decides whether discovery is allowed: a live `/model xai-oauth`
    # session over a persisted ``provider: auto`` is a selection, so the disk value alone is not the
    # answer. Gating on the persisted value would also kill the reachable-rung fallback a payment
    # failure exists to serve (main provider 402s → walk local/custom → api-key), which is exactly
    # the recover-fast behaviour P-039 wants on CN installs that cannot reach OpenRouter/Nous.
    runtime_provider = (_normalize_main_runtime(main_runtime).get("provider") or "").strip()
    if runtime_provider and not _discovery_chain_allowed(runtime_provider, task):
        return None, None, ""
...
def _try_discovery_chain() -> Tuple[Optional[OpenAI], Optional[str], str]:
    ...
    logger.warning("Auxiliary auto-detect: no provider available (tried: %s). "
                   ...
                   "Configure a local model or an explicit auxiliary provider in config.yaml.", ", ".join(tried))
    return None, None, ""
...
def _resolve_auto_route(
    ...
    top-level chain; (3) custom → API-key providers, only with no policy and no working
    main client. OpenRouter and Nous Portal are not implicitly probed; explicit main or
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # Inherit the sync client's retry policy rather than hardcoding: clients built through
    # ``_OpenAIProxy`` default to ``max_retries=0`` (see #54465), and an explicit caller value
    # must survive the async hop.
    async_kwargs.setdefault("max_retries", getattr(sync_client, "max_retries", 0))
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    if custom_key == "no-key-required":
        # The entry's own chain (api_key / key_env / key_cmd / credential pool) resolved
        # nothing. A named entry pointing at the SAME host as the main model is that same
        # endpoint, so the main credential is the right one — without this, a user who
        # configured the key once on ``model`` and referenced the provider by name gets a
        # placeholder key and a 401 everywhere. A host mismatch never inherits (SECURITY).
        custom_key = _read_main_api_key_if_same_host(custom_base) or custom_key
    if custom_key == "no-key-required":
    ...
    if entry_api_mode == "anthropic_messages":
        if (
            ...
            # A remote anthropic_messages endpoint cannot be anonymous: shipping the placeholder
            # guarantees a 401 on every call, so report "no client" instead and let the caller
            # fall through its provider chain.
            logger.warning(
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → Anthropic).

    OpenRouter and Nous are included only when they are the active provider or an
    explicit vision override. Single source of truth for setup, tool gating, and
    ...
    if (
        ...
        vision_model = _resolve_provider_vision_default(main_provider) or _read_main_model()
        if not _main_model_supports_vision(main_provider, vision_model):
            main_provider = ""
        elif main_provider in _VISION_EXPLICIT_PROVIDER_ORDER:
            if _resolve_strict_vision_backend(main_provider)[0] is not None:
...
def _vision_main_provider_client(
    ...
    if not _main_model_supports_vision(main_provider, vision_model):
        ...
        # If the user explicitly selected a provider that has its own strict vision backend
        # (OpenRouter/Nous/Anthropic), use that provider's vision default rather than probing
        # unrelated remote aggregators (P-039).
        if main_provider in _VISION_EXPLICIT_PROVIDER_ORDER:
            ...
            if sync_client is not None:
                ...
                return sync_client, default_model
        logger.debug(
            "Vision auto-detect: skipping main provider %s (reports no vision capability) — falling through to vision backend chain",
            main_provider,
        )
        return None, None
