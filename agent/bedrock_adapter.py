...
def _parse_tool_args(args) -> Any:
    ...
    try:
        return orjson.loads(args) if isinstance(args, str) else args
...
def _tool_call_ns(tool_use_id: str, name: str, input_dict) -> SimpleNamespace:
    return SimpleNamespace(
        id=tool_use_id, type="function", function=SimpleNamespace(name=name, arguments=orjson.dumps(input_dict).decode('utf-8')),
    )
