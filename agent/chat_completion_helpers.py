...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _kill_stale_stream(self, elapsed: float, *, kill_count: int = 0, max_kills: int = 0) -> None:
        ...
        logger.warning(
            "Stream stale for %.0fs (threshold %.0fs, abort %d/%d) — no chunks received. "
            "model=%s context=~%s tokens. Killing connection.",
            elapsed, self._stream_stale_timeout, kill_count, max_kills,
        ...
        # [CN-fork] P-022: deliberately do NOT reset the timestamp here. The monitor's
        # grace gate (HERMES_STREAM_STALE_KILL_GRACE) spaces the aborts, and the true
        # last-chunk timestamp is what lets the bounded escalation detect a reader that
        # never unblocked; resetting it also made every abort look like a fresh attempt,
        # so the reconnect budget below never escalated.
