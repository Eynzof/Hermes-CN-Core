...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    while t.is_alive():
        ...
        if (
            ...
            # canonical comment block above ``_stale_streak()``.
            _bump_stale_streak(agent)
            # [CN-fork] P-022: kill the wedged stream FD-safely instead of a
            # plain client.close() (which can block or fail to unblock the
            # worker's recv() from another thread), and do NOT reset
            # last_chunk_time — the _last_stale_kill_at grace gate below
            # prevents repeated kills, and the true timestamp is what lets
            # the bounded escalation detect a worker that never unblocks.
            try:
                # Reject late chunks from this attempt before aborting its
                # request-local transport.  Anthropic and OpenAI both follow
                # the same FD-ownership contract: the poll thread only shuts
