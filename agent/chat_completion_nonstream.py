...
# [CN-fork] P-059 provider wait status: platforms with a persistent status rail. The
# Desktop/TUI stall watchdog is driven by gateway events, so they need a periodic
# model-wait heartbeat while the agent is blocked inside a provider HTTP request.
# Messaging gateways route status_callback to chat messages — suppress the
# high-frequency heartbeats there to avoid spamming users.
...
def _emit_provider_status(agent, kind: str, message: str) -> None:
    ...
    try:
        ...
        h.logger.debug("provider status callback failed", exc_info=True)
...
class _NonStreamRequest:
    ...
    def __init__(self, agent, api_kwargs: dict):
        ...
        # [CN-fork] P-059 provider wait status: cadence of the status-rail heartbeat that
        # tells the user what the agent is waiting on (0 disables it).
        self.wait_status_interval = h.env_float("HERMES_PROVIDER_WAIT_STATUS_INTERVAL_SECONDS", 30.0)
        self.next_wait_status_at = (
            self.wait_status_interval if self.wait_status_interval > 0 else float("inf")
        )
        self.thread = None
    ...
    def _ttfb_kill(self, elapsed: float) -> None:
        ...
        # [CN-fork] P-059: reconnect notice for the desktop/TUI status rail.
        _emit_provider_status(
            agent, "provider_retry",
            f"模型连接 {_format_elapsed_seconds(elapsed)} 未返回首字节，正在自动重连…",
        )
    ...
    def _idle_kill(self, event_stale_elapsed: float) -> None:
        ...
        _emit_provider_status(
            agent, "provider_retry",
            f"模型流 {_format_elapsed_seconds(event_stale_elapsed)} 没有新事件，正在自动重连…",
        )
    ...
    def run(self):
        ...
        while t.is_alive():
            ...
            # [CN-fork] P-059: periodic CN model-wait heartbeat for the desktop/TUI status
            # rail, so a slow provider reads as "still waiting on <model>" instead of an
            # unexplained spinner. Suppressed on messaging platforms (chat spam) and on a
            # late event-loop wake it advances by intervals instead of bursting.
            if elapsed >= self.next_wait_status_at:
                _emit_provider_status(
                    agent, "provider_wait",
                    _provider_wait_message(elapsed, self.api_kwargs.get("model")),
                )
                self.next_wait_status_at = elapsed + self.wait_status_interval
            last_event_ts, last_progress_ts, retry_started_ts = self._codex_watchdog_snapshot()
