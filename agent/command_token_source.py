...
def _mint(command: str, label: str) -> tuple[str, Optional[float]]:
    ...
    try:
        completed = subprocess.run(
            ...
            # errors="replace": a helper that prints a banner in the console code page (GBK on a
            # zh-CN Windows box) must not crash the capture thread with a UnicodeDecodeError
            # before we can report the exit status.
            errors="replace",
