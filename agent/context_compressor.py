...
def _summarize_tool_result(tool_name: str, tool_args: str, tool_content: str) -> str:
    ...
    except Exception as exc:  # noqa: BLE001 - summaries must never abort compression
        logger.debug("Tool-result summary failed for %s: %s", tool_name, exc)
        content_len = len(tool_content) if isinstance(tool_content, str) else 0
...
def _summarize_tool_result_unguarded(
    ...
    """Build a tool-result summary; guarded by ``_summarize_tool_result``."""
    try:
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(
        ...
        # Preserve compatibility with custom/legacy summary hooks that do not
        # yet accept the v0.19 memory-context keyword when there is no context
        # to pass through.
        if memory_context:
            summary_kwargs["memory_context"] = memory_context
