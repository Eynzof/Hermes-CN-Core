...
def _lazy_todo_header() -> str:
    """Lazy import to preserve the P-043 lazy-tool-import invariant."""
    from tools.todo_tool import TODO_INJECTION_HEADER
    return TODO_INJECTION_HEADER
...
def _classify_summary_failure(e: Exception) -> _SummaryFailureKind:
    ...
    return _SummaryFailureKind(
        ...
        json_decode=isinstance(e, orjson.JSONDecodeError) or "expecting value" in err,
...
def _truncate_tool_call_args_json(args: str, head_chars: int = 200) -> str:
    ...
    out = orjson.dumps(shrunken).decode('utf-8')
...
def _json_dict(text: Any) -> dict:
    ...
    try:
        parsed = orjson.loads(text) if text else {}
    # Just-loaded / actively-referenced skills survive verbatim (#32106). Pass-4 pressure demotion overrides
    # this.
    except (orjson.JSONDecodeError, TypeError):
        # [CN-fork] orjson REJECTS a JSON document holding lone UTF-16 surrogate escapes
        # ("invalid low surrogate in string"), which is exactly what json.dumps() emits for a
        # tool payload carrying unpaired surrogates (a clarify answer, terminal output decoded
        # with errors="surrogateescape"). stdlib json accepts them, so fall back rather than
        # losing the payload: without this the clarify summarizer dropped the whole answer and
        # degraded to "asked user a question" (tests/agent/test_context_compressor.py
        # ::TestSummarizeToolResultClarify::test_unpaired_surrogates_are_safe...).
        try:
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        mode: Optional[str] = None, memory_context: str = "", bypass_cooldown: bool = False,
    ) -> Optional[str]:
        ...
        try:
            ...
            return self._on_summary_failure(e, turns_to_summarize, focus_topic, memory_context, mode=mode)
...
Write only the summary body. Do not include any preamble or prefix."""
    ...
    def _on_summary_failure(
        ...
        if self.summary_model and self.summary_model != self.model and not getattr(self, "_summary_model_fallen_back", False):
            ...
            return self._generate_summary(turns_to_summarize, focus_topic=focus_topic, mode=mode, memory_context=memory_context)
    ...
    def _is_synthetic_compression_user_turn(cls, message: Any) -> bool:
        ...
        return text in {
            ...
            _BACKGROUND_PROCESS_NOTIFICATION_PREFIX, _lazy_todo_header() + "\n", _LENGTH_CONTINUATION_DROPPED_TOOLS_PREFIX,
        ))
    ...
    def compress(
        ...
        force: bool = False, memory_context: str = "", bypass_cooldown: bool = False, mode: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        ...
        the caller's attempt budget.
          ...
          "retentive", "technical") forwarded to the summariser as style guidance.
        """
