...
class SummaryDispatchMixin:
    def _summarize_window(
        ...
        # [CN-fork] P-059: the compact tool's mode ("balanced"/"aggressive"/...) reaches
        # the summarizer as style guidance. Only forward it when the caller set one AND
        # the hook accepts it, so engines released before ``mode`` keep working (same
        # contract as the ``bypass_cooldown`` guard above).
        if mode is not None and _accepts_keyword_argument(self._generate_summary, "mode"):
            summary_kwargs["mode"] = mode
        try:
