...
def _supported_compression_kwargs(
    ...
    memory_context: str, bypass_cooldown: bool = False, mode: Optional[str] = None,
) -> dict:
...
# [CN-fork] Transport label agent/prompt_builder.py::format_steer_marker() prepends to every
# steer ("Inject prompt to steers", 888a947748). It marks a TOOL-ROW delivery as genuine
# user text; the compaction anchor below is a standalone role="user" row, where the label
# is redundant. Keep the two in one place so the strip can never drift.
_STEER_LABEL_PREFIX = "User injection prompt:"
...
def _extract_steer_text_from_message(message: Any) -> Optional[str]:
    ...
    if inner.startswith(_STEER_LABEL_PREFIX):
        # [CN-fork] Strip the fork's transport label: this text is re-delivered as a
        # standalone role="user" anchor row after compaction, so the label is redundant there
        # (the row already IS a user message) and upstream pins the anchor to the steer text
        # itself (tests/agent/test_compression_busy_steer_anchor.py). Callers that still need
        # the label re-add it through format_steer_marker(), which is idempotent.
        inner = inner[len(_STEER_LABEL_PREFIX):].strip()
...
def _resolve_compress_call(
    ...
    bypass_cooldown: bool, mode: Optional[str] = None,
    ...
    compress_kwargs = _supported_compression_kwargs(
        ...
        bypass_cooldown=bypass_cooldown, mode=mode,
    )
...
def compress_context(
    ...
    bypass_cooldown: bool = False, mode: Optional[str] = None, defer_context_engine_notification: bool = False,
    commit_fence: Optional[CompressionCommitFence] = None,
) -> Tuple[list, str]:
    ...
    callers use the default ``False``. mode: Optional compaction mode string ("balanced", "aggressive",
      "retentive", "technical") forwarded to the compressor to control summarizer style guidance.
