...
def _resolve_args() -> list[str]:
    return shlex.split(os.getenv("HERMES_COPILOT_ACP_ARGS", "").strip(), posix=os.name == "posix") or ["--acp", "--stdio"]
...
def _render_message_content(content: Any) -> str:
    ...
    if isinstance(content, dict):
        ...
        return content["content"].strip() if isinstance(content.get("content"), str) else orjson.dumps(content).decode('utf-8')
...
class CopilotACPClient:
    ...
    def _session(
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None,
            ...
            proc.stdin.write(orjson.dumps({"jsonrpc": "2.0", "id": request_id, "method": method, "params": params}).decode('utf-8') + "\n")
            proc.stdin.flush()
