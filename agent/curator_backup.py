...
def _backup_cron_jobs_into(dest: Path) -> Dict[str, Any]:
    ...
    try:  # jobs_count is a diagnostic only — an unparseable file is still stored raw.
        info["jobs_count"] = len(_jobs_list(orjson.loads(raw)) or [])
...
def _write_manifest(dest: Path, reason: str, archive_path: Path, skills_counted: int, cron_info: Dict[str, Any]) -> None:
    ...
    (dest / "manifest.json").write_text(orjson.dumps(manifest, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode('utf-8'), encoding="utf-8")
...
def _read_manifest(snap_dir: Path) -> Dict[str, Any]:
    try:
        return orjson.loads((snap_dir / "manifest.json").read_text(encoding="utf-8", errors="replace"))
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    try:
        backup_jobs = _jobs_list(orjson.loads(backup_file.read_text(encoding="utf-8", errors="replace")))
