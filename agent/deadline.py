...
# Windows sizes the relative timeout of its locked waits in a 32-bit *millisecond* field, so
# ``Lock.acquire(timeout=...)`` / ``Condition.wait(timeout=...)`` raise ``OverflowError: timeout
# value is too large`` above CPython's PY_TIMEOUT_MAX (4_294_967 s); macOS/Linux are bounded by
# ``time_t`` instead and accept a full year. Stay ~16 min below the hard Windows cap: values are
# derived from this one by addition (``tools.approval_human_wait.human_wait_ceiling`` adds
# HUMAN_WAIT_MARGIN_S and feeds the result straight into ``agent.tool_executor``'s
# serialization-lock acquire), so the SUM has to stay lock-safe, not just this constant.
_WINDOWS_LOCK_TIMEOUT_MAX_S = 4_294_000.0

# One year — or the Windows locked-wait ceiling: semantically "unbounded" yet far below any
# platform limit (#83220).
MAX_SAFE_TIMEOUT_S = _WINDOWS_LOCK_TIMEOUT_MAX_S if sys.platform == "win32" else 31_536_000.0
