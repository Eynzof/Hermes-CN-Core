...
def _dump_call_args(fc: Dict[str, Any], **kwargs: Any) -> str:
    try:
        option = orjson.OPT_SORT_KEYS if kwargs.get("sort_keys") else 0
        return orjson.dumps(fc.get("args") or {}, option=option).decode('utf-8')
    except (TypeError, ValueError):
        return "{}"
...
def _tool_call_slot(fc: Dict[str, Any], part: Dict[str, Any], part_index: int, args_str: str,
    ...
    if fc_id := _provider_call_id(fc):
        key = orjson.dumps({"provider_call_id": fc_id}, option=orjson.OPT_SORT_KEYS).decode('utf-8')
        return key, tool_call_indices.get(key)
    thought_signature = part.get("thoughtSignature") if isinstance(part.get("thoughtSignature"), str) else ""
    key = orjson.dumps({"part_index": part_index, "name": fc["name"], "thought_signature": thought_signature}, option=orjson.OPT_SORT_KEYS).decode('utf-8')
...
def _error_object(body_text: str) -> Dict[str, Any]:
    ...
    try:
        parsed = orjson.loads(body_text) if body_text else None
