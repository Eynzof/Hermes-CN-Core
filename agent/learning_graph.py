...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return orjson.loads((get_hermes_home() / "skills" / ".usage.json").read_text(encoding="utf-8", errors="replace"))
        except Exception:
            return {}
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    for fname, source in (("MEMORY.md", "memory"), ("USER.md", "profile")):
        path = base / fname
        try:
            text, file_ts = path.read_text(encoding="utf-8", errors="replace").strip(), _to_int_ts(path.stat().st_mtime)
        except OSError:
            continue
