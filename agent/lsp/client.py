...
class LSPClient:
    ...
    async def _spawn(self) -> None:
        ...
        if sys.platform == "win32" and cmd[0].lower().endswith((".cmd", ".bat")):
            # Quote cmd[0]: cmd.exe re-tokenizes the command line, so an unquoted path
            # containing a space (e.g. ``C:/Program Files/``) is split into two args and
            # fails with "'C:/Program' is not recognized".
            cmd = ["cmd.exe", "/c", f'"{cmd[0]}"', *cmd[1:]]  # CreateProcess can't run .cmd/.bat shims directly
        try:
