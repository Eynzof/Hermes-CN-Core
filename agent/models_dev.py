...
def _serve_offline_fallback() -> Dict[str, Any]:
    ...
    package — this is what keeps model save/switch off the network stall.
    """
...
def fetch_models_dev(force_refresh: bool = False, *, allow_network: bool = True) -> Dict[str, Any]:
    ...
    global _models_dev_cache, _models_dev_cache_time, _models_dev_retry_after
    # Non-blocking mode: never touch the network. Serve the best offline data we
    # have (stale in-mem / disk / bundled snapshot) so the caller — a model
    # save/switch on the user's critical path — returns instantly even when
    # models.dev is slow or blocked. The background prewarm updates it. (P-028)
    ...
    with _models_dev_fetch_lock:
        ...
        if force_refresh:
            # A forced refresh (prewarm / `hermes config refresh`) must never leave the
            # registry empty: fall back to the bundled snapshot. (P-028)
            return _serve_offline_fallback()
        return _models_dev_cache
...
def prewarm_models_dev_async() -> Optional["threading.Thread"]:
    ...
    Hot paths read the registry non-blocking (``allow_network=False``), so
...
def get_model_capabilities(provider: str, model: str, *, allow_network: bool = False) -> Optional[ModelCapabilities]:
    ...
    if raw is None:
        return None
    input_mods = _dict_or_empty(raw.get("modalities", {})).get("input")
    ...
    return ModelCapabilities(
        ...
        supports_reasoning_control=(isinstance(reasoning_options, list) and len(reasoning_options) > 0),
