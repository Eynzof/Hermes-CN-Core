...
# [CN-fork] Hosts whose OpenAI-protocol route must keep the SDK's own UA and must never
# carry Hermes attribution: the official API and the ChatGPT/Codex backend (whose
# attribution headers are owned by agent/codex_headers.py via the chatgpt.com route entry
# in agent/client_lifecycle._ROUTE_DEFAULT_HEADERS). Everything else — the OpenAI-compatible
# gateways/proxies this fallback exists for — gets HermesAgent/<version>.
_FIRST_PARTY_UA_HOSTS = ("api.openai.com", "chatgpt.com")
...
def hermes_openai_default_headers(kwargs: dict) -> dict:
    ...
    if isinstance(base_url, str) and base_url.strip() and any(
        base_url_host_matches(base_url, host) for host in _FIRST_PARTY_UA_HOSTS
    ):
        # First-party route: upstream pins the SDK UA here and expects no originator /
        # chatgpt-account-id (tests/agent/test_codex_usage_attribution.py), and a
        # HermesAgent/... UA on api.openai.com is not something the WAF workaround needs.
        return {}
...
__all__ = [
    ...
    "enable_happy_eyeballs_on_client", "hermes_openai_default_headers",
]
