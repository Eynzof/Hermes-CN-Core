...
def _local_host_hints() -> list[str]:
    ...
    rel = windows_release() if sys.platform == "win32" else None
    host = (
        "WSL (Windows Subsystem for Linux)" if is_wsl()
        else (f"Windows ({rel})" if rel else "Windows") if sys.platform == "win32"
    ...
    # Windows-local terminal shell hint. With `terminal.shell` unset (auto), the actual shell is
    # git-bash first (when a working install exists), else PowerShell 7 / Windows PowerShell 5.1 —
    # this block tells the model which syntax to expect.
    shell = "auto"
    try:
    ...
    if shell == "bash":
        shell_hint = _WINDOWS_BASH_SHELL_HINT
    elif shell == "powershell":
        shell_hint = _WINDOWS_POWERSHELL_SHELL_HINT
    else:
        # auto (or any unknown value): prefer the hint matching the new default —
        # git-bash (if a working one exists), else pwsh, else Windows PowerShell 5.1.
        try:
            from tools.environments.local import _IS_WINDOWS, _find_bash

            # Gate on local._IS_WINDOWS (not just sys.platform) so the probe means
            # "Git Bash" specifically — on non-Windows the discovery returns any POSIX
            # bash, which is irrelevant here and would make CI results host-dependent.
            has_git_bash = bool(_IS_WINDOWS and _find_bash(raise_if_missing=False))
        except Exception:
            has_git_bash = False
        if has_git_bash:
            shell_hint = _WINDOWS_BASH_SHELL_HINT
        elif shutil.which("pwsh") or shutil.which("pwsh.exe"):
            shell_hint = _WINDOWS_PWSH_SHELL_HINT
        else:
            shell_hint = _WINDOWS_POWERSHELL_SHELL_HINT
...
def _load_skills_snapshot(skills_dir: Path) -> Optional[dict]:
    ...
    try:
        snapshot = orjson.loads(
            _skills_prompt_snapshot_path().read_text(encoding="utf-8", errors="replace")
        )
...
def _agents_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    ...
    for directory in _agents_md_directory_chain(cwd_resolved):
        for name in ("AGENTS.override.md", "AGENTS.md", "agents.md"):
            ...
            # Provenance label = section header in the system prompt; POSIX rendering keeps it
            # readable (and byte-stable across platforms) — ``os.path.relpath`` returns
            # ``..\\AGENTS.md`` on Windows.
            label = name if directory == cwd_resolved else PurePath(os.path.relpath(candidate, cwd_resolved)).as_posix()
