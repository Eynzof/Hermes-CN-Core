...
def _expand_skill_config_path(value: str) -> str:
    ...
    The separator that followed the placeholder is rebuilt with ``os.sep``: the resolved value is
    a path, so ``$HOME/wiki`` must come out as ``<home>\\wiki`` on Windows rather than as a
    mixed ``<home>/wiki`` (other ``$VAR`` expansions keep whatever the config wrote — they are
    not anchored at a home).
    """
    ...
    if subprocess_home:
        if value == "~":
            ...
            value = subprocess_home + os.sep + value[2:]
        # Callable replacement: a literal template would parse backslashes in the home path
        # as regex escapes.
        value = _HOME_VAR_RE.sub(
            lambda m: subprocess_home + (os.sep if m.group(1) else ""), value
        )
