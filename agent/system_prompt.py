...
def _post_workspace_parts(agent: Any) -> List[str]:
    ...
    parts += [_active_profile_line(agent)]
    if os.environ.get("HERMES_DESKTOP_MANAGED") == "1":
        parts.append(
            f"Managed Desktop scope: {get_hermes_home()} is the authoritative HERMES_HOME "
...
def format_tools_for_system_message(agent: Any) -> str:
    ...
    if not agent.tools:
        return "[]"
    return orjson.dumps([{"name": t["function"]["name"], "description": t["function"].get("description", ""),
                        "parameters": t["function"].get("parameters", {}),
                        "required": None}  # Match the format in the example
                       for t in agent.tools]).decode('utf-8')
