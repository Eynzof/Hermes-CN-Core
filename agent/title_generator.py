    ...
    try:
        ...
        try:
            response = call_llm(
                task="title_generation",
                messages=[{"role": "system", "content": prompt}, {"role": "user", "content": user_snippet}],
                # Same shape as the first attempt, minus the structured-output field the
                # provider just rejected (#72351/#83903 apply here too).
                max_tokens=TITLE_MAX_TOKENS, temperature=None, timeout=timeout, main_runtime=main_runtime,
                reasoning_config={"enabled": False},
            )
            message = response.choices[0].message
            ...
            logger.debug("Title generation traceback", exc_info=True)
            _report_failure(failure_callback, retry_err, "Title generation")
            return None
