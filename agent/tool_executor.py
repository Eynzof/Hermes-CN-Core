    ...
    def _run_tool(
        ...
        try:
            try:
                ...
                dedup = agent._tool_dedup_tracker.check_and_register(
    ...
    *,
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
            ...
            resume_ids = function_args.get("resume_agent_ids", {})
            if items:
                spinner_label = f"🐝 swarm: {len(items)} subagents · {description_preview}"
            elif resume_ids:
                spinner_label = f"🐝 swarm: resuming {len(resume_ids)} subagents"
            else:
                spinner_label = "🐝 agent swarm"
            spinner = None
            if agent._should_emit_quiet_tool_messages() and agent._should_start_quiet_spinner():
                face = random.choice(KawaiiSpinner.get_waiting_faces())
            ...
            try:
                ...
                function_result, function_args, middleware_trace, _execution_blocked, _execution_dispatched = _managed_values(
                ...
                tool_duration = time.time() - tool_start_time
                cute_msg = _get_cute_tool_message_impl('agent_swarm', function_args, tool_duration, result=_swarm_result)
                if spinner:
                    spinner.stop(cute_msg)
                elif agent._should_emit_quiet_tool_messages():
                    agent._vprint(f"  {cute_msg}")
                ...
                def _execute(next_args: dict) -> Any:
                    if function_name == "compact":
                        ...
                        if _ack_data.get("error"):
                            return _ack
                        instruction = next_args.get("instruction", "")
                        mode = next_args.get("mode", "balanced")
