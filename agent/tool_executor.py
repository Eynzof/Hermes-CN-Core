...
def _dedup_reminder(agent: Any, tool_name: str, args: Any) -> str:
    """Reminder text for a call that repeats an earlier step's call, else ``""``.

    [CN-fork] P-017 — the tracker is constructed in ``agent_init._init_turn_state``, so a
    fully-initialised ``AIAgent`` always has one, but the dispatch path is reachable from
    agents that never ran full init (upstream's ``test_start_order_gate._make_agent`` stub
    family builds ``AIAgent``-like objects by hand). Guarded with ``getattr``, exactly like
    the agent-level per-turn flags elsewhere (``run_agent`` reads ``_trim_after_tool_batch``
    the same way in ``_execute_tool_calls``): an uninitialised agent skips dedup instead of
    turning every tool call into ``Error executing tool '<name>': ...``.
    """
    tracker = getattr(agent, "_tool_dedup_tracker", None)
    if tracker is None:
        return ""
    dedup = tracker.check_and_register(tool_name, args)
    return dedup.reminder_text or ""
...
class _ConcurrentBatch:
    ...
    def _dispatch_worker(self, index: int, ref: _ToolCallRef, scope_block, start_gate: _WorkerStartOnce) -> Optional[_ToolOutcome]:
        ...
        try:
            ...
            dedup_reminder = _dedup_reminder(agent, ref.name, ref.args)
            if dedup_reminder and isinstance(result, str):
                result += dedup_reminder
...
def _execute_inline_compact(agent, messages: list, function_args: dict, tool_call_id: str) -> str:
    """Run the P-059 ``compact`` context-engine tool: validate, compress, report.

    ``handle_tool_call`` validates and acknowledges the request; the actual
    compression has to happen here in the executor, because the compaction must
    see — and rewrite — the live ``messages`` list of the current turn.
    """
    # Capture the assistant message that contains tool_call(compact) BEFORE
    # compression.  ``_sanitize_tool_pairs()`` inside the compressor removes
    # tool_calls whose results don't exist yet, which orphans the compact
    # tool_call.  We must restore it so the tool_result committed below has a
    # matching assistant tool_call, preserving strict user/assistant/tool
    # alternation.
    _compact_assistant_msg = None
    for _msg in reversed(messages):
        if _msg.get("role") == "assistant":
            for _tc in _msg.get("tool_calls") or []:
                if isinstance(_tc, dict) and (_tc.get("id") or "") == tool_call_id:
    ...
    _ack = agent.context_compressor.handle_tool_call("compact", function_args, messages=messages)
    orjson.loads(_ack)
    ...
    try:
        ...
        if len(compressed_messages) != len(messages):
            ...
            # Restore the assistant tool_call message if sanitization removed it
            # (no tool result exists yet for this tool_call).  The tool_result is
            # committed by the caller, so the matching assistant message has to
            # be in place first.
            if _compact_assistant_msg and not any(
                _msg.get("role") == "assistant"
                and any(
                    isinstance(_tc, dict) and (_tc.get("id") or "") == tool_call_id
                    for _tc in (_msg.get("tool_calls") or [])
                )
...
def _resolve_sequential_dispatch(agent, ref: _ToolCallRef, messages: list) -> _SequentialDispatch:
    ...
    if function_name == "compact":
        # The ``compact`` tool performs actual compression inline, not just
        # acknowledgment: handle_tool_call validates the request first, then
        # we run the full compression lifecycle against the live message list.
        return _SequentialDispatch(
            execute=lambda next_args: _execute_inline_compact(agent, messages, next_args, tool_call_id),
...
def _publish_sequential_result(agent, messages: list, ref: _ToolCallRef, managed: _ManagedToolResult, *, tool_duration: float, index: int, budget: BudgetConfig) -> bool:
    ...
    # Dedup check: detect consecutive identical tool calls across API
    # iterations.  Appends reminder text if this call repeats a previous
    # step's call.
    dedup_reminder = _dedup_reminder(agent, ref.name, ref.args)
    if dedup_reminder and isinstance(function_result, str):
        function_result += dedup_reminder
