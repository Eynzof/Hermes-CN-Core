...
class CodexEventProjector:
    ...
    def _command_spec(item: dict) -> tuple[str, str, dict, str]:
        ...
        if exit_code is not None:
            ...
            output = orjson.dumps({"exit_code": exit_code, "output": output}).decode('utf-8')
    ...
    def _mcp_tool_call_spec(item: dict) -> tuple[str, str, dict, str]:
        ...
        if error:
            ...
            content = orjson.dumps(result).decode('utf-8')[:4000] if result is not None else ""
    ...
    def _dynamic_tool_call_spec(item: dict) -> tuple[str, str, dict, str]:
        ...
        content = (
            orjson.dumps(content_items).decode('utf-8')[:4000] if isinstance(content_items, list) and content_items
