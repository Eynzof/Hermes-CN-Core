...
def build_api_messages(
    ...
    for idx, msg in enumerate(canonical_messages):
        ...
        if msg is current_turn_message and msg.get("role") == "user":
            ...
            # [CN-fork] P-059: collect the ephemeral system reminders (compact reminder)
            # for THIS API call and append them to the current-turn user row COPY only.
            # ``messages`` keeps its bytes and the cached prefix above this row is
            # untouched, so the prompt-cache invariant holds. Providers throttle
            # themselves; the registry isolates each provider's failures.
            _reminder_injections = []
            _reminder_registry = getattr(agent, "_reminder_registry", None)
            if _reminder_registry is not None:
                for _reminder in _reminder_registry.get_system_reminders(
                    agent, int(getattr(agent, "_api_call_count", 0) or 0)
                ):
                    _reminder_injections.append(f"[{_reminder.type}] {_reminder.content}")
            if _reminder_injections:
                _reminder_base = api_msg.get("content", "")
                if isinstance(_reminder_base, str):
                    api_msg["content"] = (
        ...
        # Fill a *display-hidden* placeholder's wire copy — the steer/compaction
        # placeholder is an intentionally-empty durable row whose only job is role
        # alternation, so the projection gives it the neutral "[response interrupted]"
        # payload (substituted from api_content) and the sanitizer leaves it alone
        # (#88955/#96870). A generic empty turn is NOT filled: the fork's P-024
        # sanitizer DROPS empty-content turns at the send boundary, so filling one
        # here would resurrect exactly the poisoned stub the drop exists to remove.
