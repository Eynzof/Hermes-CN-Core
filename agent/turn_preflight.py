...
def compress_after_tool_results(
    ...
    if (
        ...
        if messages is _post_tool_input and compression_skipped_due_to_lock(agent):
            ...
            # [CN-fork] P-059: the compaction rewrote the live context, so notify the
            # reminder providers — CompactReminderProvider resets its injection throttle
            # so the compact nudge can fire again on the compacted transcript.
            _reminder_registry = getattr(agent, "_reminder_registry", None)
            if _reminder_registry is not None:
                _reminder_registry.on_context_compacted(agent)
