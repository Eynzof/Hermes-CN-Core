...
def run_tool_round(
    ...
    # [CN-fork] P-041: tell consumers the assistant message carrying these tool calls is
    # committed. For a tool-call-only turn this is the boundary event the Desktop/gateway
    # stream state machine uses to leave ``awaitingResponse`` (the fork's emit site was
    # dropped when the loop was split into ``agent/turn_*.py``; the consumer,
    # ``tui_gateway/agent_callbacks.py``, and tests/agent/test_tool_call_streaming_convergence.py
    # both still require it). Same shape as before: guarded + non-fatal.
    _committed_cb = getattr(agent, "tool_calls_committed_callback", None)
    if _committed_cb is not None:
        with suppress(Exception):
            _committed_cb(assistant_msg)
    agent._execute_tool_calls(assistant_message, messages, effective_task_id, api_call_count)

    # [CN-fork] P-017: close this API call's dedup step. Captures the tool calls made and
    # advances the consecutive streak for the next ``begin_step()`` (turn_iteration_prep).
