...
/** ``provider`` plus optional overrides for the settings-UI connectivity button. ``api_mode`` ``"anthropic_messages"`` probes with the Anthropic protocol (x-api-key + ``{base}/v1/models``); anything else keeps the OpenAI-style bearer + ``/v1/models`` heuristics. */
...
/** ``agent_callbacks._agent_cbs`` tool_calls_committed_callback [CN-fork P-041]: the assistant message that requested the current batch of tool calls has been appended to the conversation and is about to be executed. Gives the UI a boundary for tool-call-only turns, which otherwise have no ``message.delta`` between ``message.start`` and ``tool.start``. */
export interface ToolCallsCommittedPayload {
  role?: string | null
...
export interface RpcMethods {
  ...
  /** Full model-id list for a provider (LAN/self-hosted picker refresh through the backend). */
  'provider.models': { params: ProviderModelsParams; result: ProviderModelsResult }
  /** Connectivity check against a provider's /models endpoint. */
  'provider.probe': { params: ProviderProbeParams; result: ProviderProbeResult }
...
export interface BackendGatewayEventMap {
  ...
  /** Tool-call-only assistant message committed; the tool batch is about to run. */
  'assistant.tool_calls_committed': ToolCallsCommittedPayload
