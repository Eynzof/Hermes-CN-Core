...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Hot-path frugal: this is the single chokepoint every cron mkdir routes through (the jobs/output
    dirs, the tick lock dir, the handoff dir), and on Windows/NTFS each mkdir syscall carries real
    overhead. An existing directory short-circuits on a single ``is_dir()`` stat."""
    if cron_dir.is_dir():
        return
...
def _epoch_file_age(name: str) -> Optional[float]:
    ...
    try:
        raw = (_current_cron_store().cron_dir / name).read_text(encoding="utf-8", errors="replace").strip()
...
def load_jobs() -> List[Dict[str, Any]]:
    ...
    if jobs and repair:
        ...
        # save_jobs() re-primes the cache with the repaired content, so the pre-read
        # signature is stale — do not store it.
