...
# Windows path literals: a drive path (optionally quoted, so an embedded space stays one token)
# and the UNC double-backslash form.
...
def _protect_windows_paths(line: str) -> tuple[str, dict[str, str]]:
    """Shield Windows path literals from :mod:`shlex`'s POSIX backslash escaping.

    On Windows a backslash separates path segments, so ``posix=True`` turns
    ``/bin/bash C:\\ops\\x.sh`` into ``/bin/bashC:opsx.sh``: no referenced-script path
    ever resolved and the guard silently failed OPEN (a script hiding ``hermes gateway restart``
    was never read). Each path literal is swapped for an inert placeholder before tokenizing and
    put back afterwards, so paths survive intact while the real splices (``launchctl
    kick\\start`` ≡ ``kickstart``, #80269) still tokenize the way the shell resolves them.
    No-op off Windows and for a line without a backslash.
    """
    if not _IS_WINDOWS or "\\" not in line:
        return line, {}
    mapping: dict[str, str] = {}
    prefix = "_HP"
    while prefix + "0_" in line:  # never collide with the command text itself
        prefix += "x"

    def _swap(match: "re.Match[str]") -> str:
        value = next((group for group in match.groups() if group is not None), "")
        placeholder = f"{prefix}{len(mapping)}_"
        mapping[placeholder] = value
        return placeholder

    return _WINDOWS_PATH_TOKEN_RE.sub(_swap, line), mapping


def _restore_paths(token: str, paths: dict[str, str]) -> str:
    """Put the protected Windows path literals of *token* back (``--opt=<path>`` included)."""
...
def _shlex_tokens(line: str) -> list[str]:
    ...
    Windows path literals are protected first (see :func:`_protect_windows_paths`): the CN fork
    runs PowerShell on Windows, where a backslash is a path separator, never an escape.
    """
    protected, paths = _protect_windows_paths(line)
    lexer = shlex.shlex(protected, posix=True, punctuation_chars=";&|()")
    lexer.whitespace_split = True
    lexer.commenters = "#"
    tokens = list(lexer)
    # A placeholder can sit INSIDE a token (``--opt=<path>``), so restore by substring.
    if not paths:
        return tokens
    return [
        _restore_paths(token, paths) for token in tokens
    ]
...
def _resolved_or_nothing(candidate: str, cwd: Optional[str]) -> Iterator[Path]:
    ...
    A rooted-but-driveless candidate (a POSIX ``/remote/x.sh``) is NOT anchored on Windows:
    ``Path()`` would prefix the current drive (``C:\\remote\\x.sh``) and a remote
    read would then ask a POSIX backend for a Windows-spelled path.
    """
...
def _references_at(segment: list[str], index: int, cwd: Optional[str]) -> Iterator[Path]:
    ...
    # Windows separates with \\ (and paths may carry a drive prefix): such an absolute
    # path has no "/", so a Windows script named without a known suffix was never read.
    if executable.strip("/\\") and ("/" in executable or "\\" in executable or executable.endswith((".sh", ".bash", ".zsh"))):
    ...
    for script_path, candidate_executed in candidates:
        ...
        if script_text is None and read_remote_script is not None:
            ...
            if not budget.charge_remote_read():
                ...
                break
            # ``as_posix()``: ``script_path`` went through ``Path()`` on THIS host, which re-spells a
            # POSIX remote path (``/remote/workspace/x.sh``) with backslashes on Windows — the remote
            # shell then receives an escaped path and the read finds nothing.
            script_text, unsafe = _sanitize_remote_script_text(
                read_remote_script(script_path.as_posix()), max_bytes=budget.bytes_remaining
            )
            if unsafe:
                if candidate_executed:
                    return _refuse_unreadable(
