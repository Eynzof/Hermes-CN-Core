...
def _get_hermes_home() -> Path:
    ...
    """
    return _hermes_home or hermes_constants.get_hermes_home()
        ...
        for job in load_jobs():
            ...
            try:
                ...
                if probe.returncode == 0 and probe.stdout.strip():
                    # ``git rev-parse --show-toplevel`` prints forward slashes on Windows;
                    # normalise so a job workdir inside the checkout cannot enter the set
                    # twice (the install-root entry above is already platform-native).
                    repos.add(os.path.normpath(probe.stdout.strip()))
            except Exception:
                continue
...
from cron.scheduler_script import (  # noqa: E402
    # [CN-fork] P-019 — ``_run_job_script`` keeps its pre-split name on ``cron.scheduler`` (the
    # frozen-runtime CN contract and the fork's cron tests import it from here).
