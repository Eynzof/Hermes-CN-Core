    ...
    * anything else — run with the current Python interpreter
    ...
    if suffix in {".sh", ".bash"}:
        ...
        if _is_frozen_runtime():
            # A separate CLI worker keeps script timeout/cancellation killable
            # and isolates sys.argv/stdout from concurrent Desktop sessions.
            from tools.runtime_compat import hermes_cli_argv
            argv = hermes_cli_argv("__run-script", str(path))
            env_overlay = {}
        else:
