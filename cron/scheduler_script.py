...
def _read_windows_pyvenv_cfg(venv_dir: Path) -> dict[str, str]:
    try:
        # [CN-fork] P-019 — lossy decode: a single non-UTF-8 byte in pyvenv.cfg raises
        # UnicodeDecodeError, which is NOT an OSError, so the guard below would not catch it.
        lines = (venv_dir / "pyvenv.cfg").read_text(
...
def _script_argv(path: Path) -> tuple[Optional[list[str]], dict[str, str], Optional[str]]:
    ...
    if path.suffix.lower() in {".sh", ".bash"}:
        ...
        if _bash is None:
            # [CN-fork] P-019 — Git Bash is no longer required on Windows.
            return None, {}, (
...
def _is_frozen_runtime() -> bool:
    ...
    [CN-fork] P-019 — the desktop runtime ships as a frozen ``hermes-agent-cn-runtime-*.exe``
    with no standalone ``python.exe``: inside it ``sys.executable`` IS the Hermes CLI binary
    itself. Spawning it with a script path would run ``hermes <script>.py`` and make argparse
    reject the path as an invalid subcommand ("invalid choice: 'whitecat_inspect.py'"). Cron
    scripts must therefore be executed in-process under this runtime.
    """
...
def _redact_script_output(stdout: str, stderr: str) -> tuple[str, str]:
    ...
    [CN-fork] P-019 — mirrors the subprocess-path redaction so the in-process frozen-runtime
...
def _run_python_script_in_process(script_path: Path, timeout: int) -> tuple[bool, str]:
    ...
    [CN-fork] P-019 — used ONLY under a PyInstaller-frozen runtime (see
    ``_is_frozen_runtime``), where ``sys.executable`` is the Hermes CLI binary and spawning it
    with a script path would run ``hermes <script>.py`` — argparse then rejects the path as an
    invalid subcommand and every ``no_agent`` / data-collection cron job fails at first tick.

    The trusted script (resolved inside HERMES_HOME/scripts/ by ``_resolve_script_path``) runs
    through the fork's shared in-process runner (``tools.runtime_compat``, P-063), which gives
    ``runpy.run_path`` semantics for ``__main__``/``sys.argv``, honours ``sys.exit`` codes and
    bounds execution by ``timeout`` on a best-effort basis (a hung script is abandoned, the
    daemon thread leaks, but the cron pool thread is returned — the same contract the
    subprocess timeout branch documents).

    Security note: in-process execution shares the gateway environment, so the subprocess env
    sanitisation (SECURITY.md §2.3) cannot apply here — scripts are still confined to the
    trusted HERMES_HOME/scripts/ directory.
    """
    # Function-local: cron must keep importing without the tools package in minimal runtimes.
    from tools.runtime_compat import run_python_script_in_process
    ...
    stdout, stderr = _redact_script_output(stdout.strip(), stderr.strip())

    if exit_code != 0:
        parts = [f"Script exited with code {exit_code}"]
        if stderr:
...
def _run_job_script(
    ...
    # [CN-fork] P-019 — PyInstaller-frozen runtime (CN portable desktop): sys.executable is the
    # Hermes CLI binary itself, so spawning it with the script path would run
    # `hermes <script>.py` and die with argparse's "invalid choice" error
    # (#whitecat_inspect). Execute the Python script in-process instead; .sh/.bash keep the
    # subprocess path (bash is resolved from PATH, never sys.executable).
    if _is_frozen_runtime() and path.suffix.lower() not in {".sh", ".bash"}:
