...
# ``SystemRoot`` is load-bearing on Windows: a fresh interpreter cannot initialise Winsock without it
# (``import _overlapped`` inside asyncio raises WinError 10106 "no service provider"), so the child
# processes this probe spawns (foreign_writer.py imports hermes_state -> asyncio) died before writing.
# Case-insensitive match: Windows env names are case-insensitive but iteration yields the stored
# spelling ('SYSTEMROOT' when a POSIX shell exported it), and a case-sensitive filter drops it.
_ESSENTIAL = {"PATH", "SYSTEMROOT", "WINDIR", "SYSTEMDRIVE", "LANG", "LC_ALL", "TZ"}
keep = {k: v for k, v in os.environ.items() if k.upper() in _ESSENTIAL}
os.environ.clear()
os.environ.update(keep)
os.environ.update(
    HOME=str(HOME),
    # ``Path.home()`` reads HOME on POSIX but USERPROFILE on Windows (HOMEDRIVE+HOMEPATH as the
    # fallback) and raises RuntimeError when neither is set — the import chain below does
    # ``Path.home()`` in a couple of module-level default expressions. Point the platform's own
    # variable at the same throwaway home instead of the discarded inherited one.
