...
from gateway.platforms.helpers import file_url_to_local_path
...
def _parse_docker_volume_mounts() -> List[Tuple[Path, Path]]:
    ...
    for entry in parsed if isinstance(parsed, list) else ():
        ...
        # Container paths are POSIX-absolute: on Windows ``Path("/output")`` is rooted but not
        # "absolute" (no drive), which silently dropped every configured mount and left MEDIA
        # tags from Docker agents unresolvable (#93950/#109024).
        if host_path is not None and (
            container_path.is_absolute() or container_path.as_posix().startswith("/")
        ):
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    # Container paths are POSIX-absolute. On a Windows host ``Path("/workspace/x")`` is only
    # *rooted* (no drive), so ``is_absolute()`` is False and every MEDIA tag from a Docker agent
    # was dropped before the mounts below could be consulted (#93950). Same POSIX-root test the
    # docker-volume parser uses for host paths.
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    try:
        ...
        return None
    if not expanded.is_absolute() and not expanded.as_posix().startswith("/"):
        return None
        ...
        for image_url, alt_text in images:
            ...
            try:
                ...
                if image_url.startswith("file://"):
                    # url[7:] left the leading slash of the Windows form ``file:///C:/dir/x.png``
                    # (``/C:/dir/x.png``) — no filesystem call can resolve that, so local images
                    # were skipped on Windows. The helper drops it and restores UNC netlocs.
                    sender, url_kw = self.send_image_file, {
                        "image_path": file_url_to_local_path(image_url)}
