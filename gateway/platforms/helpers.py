...
class ThreadParticipationTracker:
    ...
    def _load(self) -> list[str]:
        try:
            data = orjson.loads(self._state_path().read_text(encoding="utf-8", errors="replace"))
        except Exception:
            return []
...
# ─── file:// → local path. Every adapters' image/file delivery goes through this: ``url[7:]``
# silently dropped every attached image on Windows (see the function docstring).
_FILE_URI_HOST_RE = re.compile(r"^[A-Za-z0-9](?:[A-Za-z0-9._-]*[A-Za-z0-9])?(?::\d+)?$")
# ``/C:/dir`` is the canonical Windows form; ``/c%3A/`` is the same path with the drive colon
# percent-encoded (some producers, e.g. the desktop's URI builder, encode it).
_WINDOWS_ABS_PATH_RE = re.compile(r"^/[A-Za-z](?::|%3[Aa])([/\\]|$)")


def file_url_to_local_path(url: str) -> str:
    """Resolve a ``file://`` URL to a local filesystem path.

    ``url[7:]`` is wrong for the canonical Windows form ``file:///C:/dir/x.png``: the
    remainder keeps a leading slash (``/C:/dir/x.png``) that no filesystem call can
    resolve, so every attached image was silently skipped on Windows.  This drops that
    slash for a drive-letter path, unquotes percent escapes, and restores a real host as
    a ``//host`` UNC prefix.

    Anything that is not the canonical form keeps the old ``url[7:]`` meaning verbatim:
    adapters also build URIs from a raw local path (``f"file://{path}"`` →
    ``file://C:\\dir\\x.png``, ``file://C:/dir/x.png`` or ``file://./rel/x.png``), which
    ``urlparse`` reports with the whole payload in the netloc and an empty path, so
    re-splitting it would drop the file entirely.
    """
    ...
    if netloc and netloc != "localhost" and _FILE_URI_HOST_RE.match(netloc):
        # UNC share: file://host/share/x.png
        return f"//{netloc}{unquote(path)}"
    if _WINDOWS_ABS_PATH_RE.match(path):
        # Canonical Windows form: file:///C:/dir/x.png → C:\dir\x.png
        return os.path.normpath(unquote(path)[1:])
