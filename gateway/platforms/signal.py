...
class SignalAdapter(BasePlatformAdapter):
    ...
    async def _resolve_image_path(self, image_url: str) -> Tuple[Optional[str], Optional[str], Any]:
        ...
        if image_url.startswith("file://"):
            # url2pathname-aware (helpers.file_url_to_local_path): ``url[7:]`` left the leading
            # slash of ``file:///C:/dir/x.png``, so local images never resolved on Windows.
            file_path = file_url_to_local_path(image_url)
        else:
            try:
