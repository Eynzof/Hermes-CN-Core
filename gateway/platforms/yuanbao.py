    ...
    for elem in msg_body or []:
        ...
        if data_str:
            with contextlib.suppress(orjson.JSONDecodeError, TypeError):
                yield orjson.loads(data_str), content
    ...
    def _extract_text(cls, msg_body: list) -> str:
        ...
        for elem in msg_body:
            ...
            if elem_type == _TEXT_ELEM_TYPE:
                ...
                if content.get("data", ""):
                    try:
                        face_name = (orjson.loads(content["data"]).get("name") or "").strip()
    ...
    def _classify(text: str, msg_body: list):
        ...
        for elem in msg_body:
            ...
            if etype == "TIMCustomElem":
                try:
                    custom = orjson.loads((elem.get("msg_content") or {}).get("data", ""))
                except (orjson.JSONDecodeError, TypeError):
                    custom = None
...
class QuoteContextMiddleware(InboundMiddleware):
    ...
    def _extract_quote_context(self, cloud_custom_data: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            parsed = orjson.loads(cloud_custom_data) if cloud_custom_data else None
        except (orjson.JSONDecodeError, TypeError):
            parsed = None
        ...
        for match in self._AT_USER_RE.finditer(text):
            ...
            if entry:
                ...
                msg_body.append({"msg_type": "TIMCustomElem",
                                 "msg_content": {"data": orjson.dumps({"elem_type": 1002, "text": f"@{real_nick}", "user_id": uid}).decode('utf-8')}})
            else:
