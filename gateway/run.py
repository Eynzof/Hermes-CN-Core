...
def _validate_cron_startup() -> bool:
    """Validate cron subsystem readiness before starting the scheduler thread.

    Checks that jobs.json is parseable and warns if optional cron-expression
    support is unavailable. A validation failure disables automatic cron firing
    for this gateway process, but it does not block the gateway itself.
    """
    try:
        ...
        if JOBS_FILE.exists():
            try:
                ...
                logger.error(
                    ...
                    "Cron scheduler disabled until the file is repaired.",
                    exc,
                )
                return False
        try:
...
def _start_gateway_start_cron_and_housekeeping(runner):
    ...
    # [CN-fork] P-021: validate the local cron store before arming the ticker; a
    # corrupt jobs.json used to kill the ticker thread silently. Housekeeping is
    # independent of cron, so it still starts.
    if not _validate_cron_startup():
        logger.warning(
        ...
        housekeeping_thread = threading.Thread(
            ...
            kwargs={"adapters": runner.adapters, "loop": asyncio.get_running_loop(),
                    "cron_provider": None, "runner": runner, "cron_thread": None},
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duck-typed runners (tests, embedding callers) may not implement the optional watchdog hook, so
    # probe for it instead of assuming the mixin — start_gateway is called with stub runner classes.
