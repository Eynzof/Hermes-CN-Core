...
def _read_proc_field(pid: int, key: str) -> Optional[str]:
    ...
    with contextlib.suppress(OSError), open(f"/proc/{pid}/status", encoding="utf-8", errors="replace") as fh:
        for line in fh:
...
def check_systemd_timing_alignment(
    ...
    with contextlib.suppress(OSError), open("/proc/self/cgroup", encoding="utf-8", errors="replace") as fh:
        for line in fh:
