...
def _get_process_start_time(pid: int) -> Optional[int]:
    ...
    with contextlib.suppress(IndexError, ValueError, OSError):
        return int(Path(f"/proc/{pid}/stat").read_text(encoding="utf-8", errors="replace").split()[21])
    try:
...
def _posix_is_zombie(pid: int) -> bool:
    ...
    try:
        stat_fields = Path(f"/proc/{pid}/stat").read_text(encoding="utf-8", errors="replace").split()
