...
def normalize_hermes_home_env() -> None:
    ...
    *relative*: the many raw ``os.environ["HERMES_HOME"]`` readers (this fast path, the
    ...
    # ``expanduser`` keeps whatever separators the value was written with, so on Windows
    # ``~/hermes`` expanded to ``C:\Users\me/hermes`` — a spelling every later
    # ``os.path.join`` turns into a mixed-separator string and no ``str(Path(...))``
    # comparison matches. Normalise once here so the raw readers and the Path readers
    # agree on one native spelling (a no-op where the value is already canonical).
    normalized = os.path.normpath(expanded)
    if normalized != raw:
        os.environ["HERMES_HOME"] = normalized
