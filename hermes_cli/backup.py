...
def _safe_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        # Release the destination handle BEFORE the fallback: on Windows an open handle makes the
        # fallback's unlink of *dst fail with WinError 32, so the restore silently reported failure
...
def list_quick_snapshots(limit: int = 20, hermes_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    for d in _snapshot_dirs(_quick_snapshot_root(hermes_home)):
        ...
        if manifest_path.exists():
            try:
                results.append(orjson.loads(manifest_path.read_text(encoding="utf-8", errors="replace")))
