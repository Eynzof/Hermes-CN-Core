...
def _install_method_stamp(path: Path) -> Optional[str]:
    try:
        method = path.read_text(encoding="utf-8", errors="replace").strip().lower()
    except OSError:
        return None
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    git_path = root / ".git"
    try:
        if git_path.is_dir() or git_path.read_text(encoding="utf-8", errors="replace").strip().startswith("gitdir:"):
            return "git"
    except OSError:
        pass
    return "unknown"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    try:
        info = {}
        with open(get_hermes_home() / ".container-mode", "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                line = line.strip()
...
def _merge_partial_save(raw: dict, override: dict) -> dict:
    ...
    result = _fast_config_copy(override)
    for key, value in raw.items():
        if key not in result:
            result[key] = _fast_config_copy(value)
...
def _strip_default_values(
    ...
    def _strip(value: Any, default: Any, path: Tuple[str, ...]) -> Any:
        ...
        return None if value == default else _fast_config_copy(value)
...
def _terminal_env_value(value: Any) -> str:
    return orjson.dumps(value).decode("utf-8") if isinstance(value, (list, dict)) else str(value)
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cache_sig is None and not _default_has_env_templates():
            ...
            # freshness.
            cache_sig = (*_NO_FILE_CACHE_SIG, 0, 0, 0, 0)
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            if all(_env_ref_lookup(k) == v for k, v in env_snapshot.items()):
                return _fast_config_copy(cached[8]) if want_deepcopy else cached[8]
        ...
        if user_sig is not None:
            try:
                ...
                if lkg_copy is not None:
                    return _fast_config_copy(lkg_copy) if want_deepcopy else lkg_copy
        ...
        if user_has_env_templates or _default_has_env_templates():
            ...
            expanded, managed_config = _merge_managed_overlay(normalized)
        _LAST_EXPANDED_CONFIG_BY_PATH[path_key] = _fast_config_copy(expanded)
        if cache_sig is not None:
            # The cache stores its own copy so load_config() callers can mutate freely while
