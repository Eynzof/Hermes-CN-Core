...
def _windows_process_listing() -> str | None:
    ...
    if result is None or result.returncode != 0 or not (result.stdout or ""):
        ...
        # PowerShell writes in the console code page; wrap the command so the
        # LIST-style output decodes as UTF-8 regardless of the code page.
        from tools.environments.windows_env import ps_with_utf8

        result = bounded_probe_run(
            [powershell, "-NoProfile", "-Command", ps_with_utf8(ps_cmd)], timeout=15, errors="ignore"
        )
        if result is None:
            return None
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # PyInstaller-frozen CN portable runtime: sys.executable IS the CLI binary, so
    # spawning ``[sys.executable, "-c", watcher, ...]`` would run `hermes -c ...`
    # and die with argparse's "invalid choice".  The hidden
    ...
    if is_frozen_runtime():
        ...
        if respawn_env_overlay:
            watcher_env["HERMES_RESTART_WATCH_ENV"] = json.dumps(respawn_env_overlay)
    ...
    try:
        subprocess.Popen(watcher_argv, **devnull, **_watcher_popen_kwargs)
    except OSError:
...
def _ensure_user_systemd_env() -> None:
    ...
    """
    getuid = getattr(os, "getuid", None)  # windows-footgun: ok — POSIX systemd helper
    if getuid is None:  # Windows: no per-user systemd instance / /run/user to point at
        return
    uid = getuid()
...
def legacy_launchd_labels_for_install(exclude=()) -> list[str]:
    ...
    from hermes_constants import get_default_hermes_root
    try:
        # ``pwd``/``os.getuid`` are POSIX-only: on Windows there is no LaunchAgents
        # directory, so this install has no legacy-labelled units — fail closed
        # instead of raising ModuleNotFoundError out of the update restart path.
        import pwd
        home = Path(pwd.getpwuid(os.getuid()).pw_dir)  # windows-footgun: ok — POSIX launchd (macOS) helper
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    installed = unit_path.read_text(encoding="utf-8", errors="replace")
...
def _windows_tcp_listener_present(port: int) -> bool | None:
    """``True``/``False`` when the socket table can answer for ``port``; ``None`` when not.

    Windows silently DROPS the SYN for a closed (or firewall-filtered) port, so a connect probe
    times out exactly like a live listener with a saturated accept queue — ``_wait_for_tcp_port_free``
    would then never report the port free. The listener table answers directly.
    """
    try:
        import psutil
    ...
    try:
        for conn in psutil.net_connections(kind="tcp"):
            if conn.status == psutil.CONN_LISTEN and conn.laddr and conn.laddr.port == port:
                return True
        return False
    except Exception:  # pragma: no cover - platform/psutil refusal means 'undeterminable'
        return None
...
def _wait_for_tcp_port_free(host: str, port: int, *, timeout: float = 10.0) -> bool:
    ...
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if sys.platform == "win32" and _windows_tcp_listener_present(port) is False:
            return True  # nothing is LISTENing: the port is free (a connect probe would time out)
        try:
...
def _cmd_restart(args):
    ...
    # [CN-fork] P-034 (issue #168): a desktop-managed CLI must never silently restart
    # a *foreign* Windows gateway service -- it runs under its own HERMES_HOME and
    # would never pick up the channel params the desktop just saved.
    desktop_managed = _is_desktop_managed()
    if kind == "windows" and desktop_managed:
        force_takeover = os.getenv("HERMES_GATEWAY_FORCE_TAKEOVER") == "1"
        if service_configured and not force_takeover:
            # The desktop runs the gateway as a child of its dashboard and never
            # installs a Windows service. An installed service is therefore a
            # *foreign* gateway: surface a conflict the desktop can resolve with a
            # one-click takeover instead of silently restarting the wrong gateway.
            ...
            sys.exit(2)
        # Explicit takeover: stop the foreign service + any other local gateway under
        # a different HERMES_HOME, then run a desktop-managed gateway via the manual
        # --replace path below.
        if force_takeover:
    ...
    _wait_for_api_server_port_free()
    print("Starting gateway...")
    # A desktop-managed start takes over its own stale gateway (and any we just
    # freed during a force-takeover) via --replace, and clears any lingering
    # conflict marker now that we're starting our own.
    if desktop_managed:
        try:
            from gateway.status import set_gateway_conflict
    ...
    print("Starting gateway...")
    run_gateway(verbose=0, replace=desktop_managed, force=force)
