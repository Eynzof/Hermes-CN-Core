...
def build_models_payload(
    ...
    *,
    ...
    - ``slug_filter`` ([CN-fork]): when set, drop canonical rows whose
      slug isn't in the set. User-defined / custom rows always pass
      through — they're explicit user adds, not part of the canonical
      universe being filtered. Lets a region-specific client (e.g. the
      CN desktop) avoid surfacing 20+ canonical providers its users
    ...
    if slug_filter is not None:
        rows = [r for r in rows if r.get("is_user_defined") or r["slug"] in slug_filter]
    if picker_hints:
