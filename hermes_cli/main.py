...
def _has_any_provider_configured() -> bool:
    ...
    if auth_file.exists():
        try:
            ...
            auth = orjson.loads(auth_file.read_text(encoding="utf-8", errors="replace"))
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    #
    # (Upstream context: #56665 — prebuilt Docker/Nix installs ship
    # hermes_cli/tui_dist/entry.js but no ui-tui/ source at all.)
    if not tui_dev:
        if ext_dir:
            p = Path(ext_dir)
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "browser", "__slash-worker", "__compute-host", "__gateway-restart-watch",
        "__update-gateway-helper", "verify",
...
def _register_internal_worker_subcommands(subparsers) -> None:
    ...
    script_parser = subparsers.add_parser("__run-script", help=argparse.SUPPRESS)
    script_parser.add_argument("script")
    script_parser.set_defaults(func=_cmd_run_script)
