...
def _apply_profile_override() -> None:
    ...
    if profile_name is None and hermes_home_env:
        # ... unless the Desktop managed runtime owns profile routing: it stops and
        # respawns the dashboard with the target profile's exact HERMES_HOME, and on a
        # named -> default switch the stale active_profile is cleared only after the
        # replacement is ready (so a failed boot can recover). Letting this startup
        # path consult that file would redirect the new default process straight back
        # into the old named profile.
        if (
...
from hermes_cli.main_provider_setup import (
    ...
    # [CN-fork] the shared curses reasoning-effort prompt stays reachable as
    # ``hermes_cli.main._prompt_reasoning_effort_selection``: the fork's picker tests
    # resolve it here, and ``hermes_cli.setup``/``model_setup_flows`` call the sibling.
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "browser",
        "verify",
        # [CN-fork] P-063 internal worker subcommands (hidden from --help): the
        # frozen CN portable runtime has no standalone python, so tui_gateway
        # workers and the gateway restart-watcher are dispatched through the
        # CLI binary via these names.
...
# [CN-fork] P-063: route every "spawn python as interpreter" site through the
# frozen-runtime chokepoint. The CN portable desktop runtime is a PyInstaller
# frozen exe with no standalone ``python.exe`` — inside it ``sys.executable`` IS
# the Hermes CLI binary, so ``[sys.executable, "-m", mod]`` / ``-c`` spawns run
# ``hermes <arg>`` and die with argparse "invalid choice". These hidden
# subcommands let the frozen CLI play the worker/watcher role directly.
...
def _build_cli_parser():
    ...
    # [CN-fork] P-063: hidden worker subcommands (see the function's docstring).
