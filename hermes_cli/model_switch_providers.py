...
#: Row note when a provider's catalog came from the bundled/curated list instead of live discovery.
_FALLBACK_CATALOG_WARNING = "Live model discovery is unavailable; showing bundled fallback models."


def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True,
                         non_blocking: bool = False, fallback_flag: list | None = None) -> list:
    ...
    *fallback_flag*, when passed, is appended to if the bundled/curated list was used, so the row can
    tell the user its catalog is not live."""
    ...
    if not model_ids:
        ...
        if model_ids and fallback_flag is not None:
...
def _aws_live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str,
    ...
    try:
        return _live_or_curated_ids(slug, curated, *fallback_keys, merge_models_dev=False,
                                    non_blocking=non_blocking, fallback_flag=fallback_flag) or []
    except Exception:
        model_ids = _first_curated(curated, fallback_keys or (slug,)) or []
        if model_ids and fallback_flag is not None:
            fallback_flag.append(True)
        return model_ids
...
class _PickerBuild:
    ...
    def add_builtin_row(
        ...
        if warning:
            # A row built from the bundled/curated list instead of live discovery says so; pickers
            # render it so a stale catalog is never mistaken for the provider's real one.
            row["warning"] = warning
        if slug == "nous":
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    for hermes_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        ...
        b.add_builtin_row(
            ...
            warning=_FALLBACK_CATALOG_WARNING if fallback else "")
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if hermes_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = _live_or_curated_ids(hermes_slug, b.curated, merge_models_dev=False,
                                             non_blocking=b.non_blocking_catalogs, fallback_flag=fallback)
    ...
    for cp in CANONICAL_PROVIDERS:
        ...
        if cp_config and cp_config.api_key_env_vars:
            ...
            if lit and lit <= sib_vars < set(cp_config.api_key_env_vars) and cp.slug != b.current_provider:
                continue
        if not has_creds:
            # Providers that live outside PROVIDER_REGISTRY (openrouter, nous, qwen-oauth) declare
            # their key vars on the provider profile instead. Without this read their row only
            # surfaces when models.dev data happens to carry them or when the credential pool /
            # auth store is already populated, so an offline (or models.dev-less) picker lost the
            # row even though the key was right there in the environment. Restores the pre-merge
            # section-2b behaviour, reading through the picker's per-profile scoped env helper
            # (same contract as sections 1 and 2) rather than raw os.environ.
            try:
                from providers import get_provider_profile
                from hermes_cli.model_switch import _scoped_key_env
                _profile = get_provider_profile(cp.slug)
                if _profile is not None:
                    has_creds = _any_env(list(_profile.env_vars or ()), _scoped_key_env)
            except Exception as exc:
                logger.debug("Provider-profile credential check failed for %s: %s", cp.slug, exc)
