...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # The alias is <name> on POSIX and <name>.bat on Windows; a Hermes home moved between
    # the two can carry both, so the summary and the removal cover either candidate
    # (remove_wrapper_script clears every one it finds).
    platform_wrapper = _wrapper_path(canon)
    alias_path = platform_wrapper if platform_wrapper.exists() else wrapper_path
    has_wrapper = platform_wrapper.exists() or wrapper_path.exists()
    _print_delete_summary(canon, profile_dir, gw_running, alias_path if has_wrapper else None)
    if not yes:
        print()
        try:
