...
def _writer_is_spawned_process(record_pid: object, spawned_pid: int) -> bool:
    """True when *record_pid* is *spawned_pid* or a process it launched.

    Windows only: there ``sys.executable`` and ``hermes.exe`` are launcher *trampolines* that
    re-exec the real interpreter as a CHILD of the pid ``subprocess.Popen`` returns (uv venvs
    install one; see ``hermes_cli/update_cmd_windows.py``). The turn therefore reports the
    interpreter's pid, never the launcher's, so equality alone would reject every report and
    the spawner would kill a child whose turn had already ended (#113608). A nested one-shot
    can never qualify — it pops the report path and its parent is the turn's own interpreter,
    not the spawner. POSIX keeps the strict equality check: a venv ``python`` is the real
    interpreter there (copy/symlink), so the pids already match. Bounded walk, lazy psutil:
    the exact-match fast path never pays for this.
    """
    if os.name != "nt":
        return False
    try:
        ...
        return False
    try:
        proc = psutil.Process(int(record_pid))  # type: ignore[arg-type]
        for _ in range(4):
            parent = int(proc.ppid())
            if parent == spawned_pid:
                return True
            if parent in (0, 1) or parent == proc.pid:
                return False
...
def read_turn_report(path: str, pid: int) -> dict | None:
    ...
    *pid* is the process the spawner started; on Windows that may be a launcher trampoline, so a
    report from a process it launched counts as this child's (see
    :func:`_writer_is_spawned_process`).
    """
    try:
    ...
    if record_pid != pid and not (
        isinstance(record_pid, int) and _writer_is_spawned_process(record_pid, pid)
    ):
        return None
    return record
