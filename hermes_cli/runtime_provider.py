...
from hermes_cli.runtime_provider_custom import (  # noqa: E402,F401
    ...
    _try_resolve_from_custom_pool, _try_resolve_inline_model_custom_runtime, canonical_custom_identity, codex_model_provider_id, expand_direct_api_alias,
...
def _inline_model_custom_rung(requested_provider, explicit_api_key, explicit_base_url, target_model):
    """Rung 3b: a legacy Desktop config that stored a named custom endpoint INLINE under ``model``
    (``provider`` + ``base_url`` + ``api_key``) instead of under ``providers.<name>``. Runs before
    ``resolve_provider`` so the inline endpoint/key win and an unknown provider name is not rejected."""
    return _tag(_try_resolve_inline_model_custom_runtime(
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      3b. inline ``model`` provider: a legacy Desktop config that stored a named custom endpoint inline
...
def _ladder_rungs(requested_provider, explicit_api_key, explicit_base_url, target_model):
    ...
    yield _inline_model_custom_rung(requested_provider, explicit_api_key, explicit_base_url, target_model)
