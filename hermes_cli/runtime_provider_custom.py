...
# ------------------------------------------------------------------ inline model provider recovery
# Legacy Desktop installs stored a named custom endpoint INLINE under ``model`` (``provider`` +
# ``base_url`` + ``api_key``) instead of under ``providers.<name>``. ``_try_resolve_inline_model_custom_runtime``
# is the ladder rung that still serves those configs, and ``_inline_model_api_key_candidates`` lets a
# configured-but-secretless ``providers.<name>`` entry borrow the inline key. Both were part of the
# fork's ``runtime_provider`` and were dropped by the Sep 2026 merge; restored here beside the
# named-custom builder that consumes them.
...
def _inline_model_provider_matches(requested_provider: str, configured_provider: str) -> bool:
    ...
    return bool(requested_aliases and configured_aliases and requested_aliases & configured_aliases)
...
def _inline_provider_env_name(value: str) -> str:
    ...
    name = "".join(ch.upper() if ch.isalnum() else "_" for ch in raw).strip("_")
    while "__" in name:
...
def _inline_model_api_key_candidates(*, requested_provider: str, base_url: str) -> list[str]:
    """Credentials the inline ``model`` block can supply for ``requested_provider`` (``[]`` otherwise)."""
    rp = _rp()
    ...
    candidates = [
        ...
        _key_env_secret(model_cfg, f"model provider '{cfg_provider or requested_provider}'"),
    ]
    for env_name in _inline_provider_env_candidates(requested_provider, cfg_provider):
        candidates.append(get_secret_str(env_name, "").strip())
    return candidates


def _custom_endpoint_allows_no_key(base_url: str) -> bool:
    """A loopback endpoint (Ollama, LM Studio, vLLM) is allowed to run without a key."""
    host = base_url_hostname(base_url)
    return bool(host and _rp()._loopback_hostname(host))


def _raise_missing_named_custom_key(*, requested_provider: str, provider_name: str, base_url: str) -> None:
    raise _rp().AuthError(
...
def _try_resolve_inline_model_custom_runtime(
    *,
    ...
    if not requested_norm or requested_norm in {"auto", "custom", "moa"}:
        return None
    if not requested_norm.startswith("custom:") and rp.auth_mod.is_runtime_provider_routable(requested_norm):
        return None
    model_cfg = rp._get_model_config()
    cfg_provider = str(model_cfg.get("provider") or "").strip()
    if not _inline_model_provider_matches(requested_provider, cfg_provider):
    ...
    if not base_url:
        return None
    api_mode = (
        rp._parse_api_mode(model_cfg.get("api_mode"))
        or rp._detect_api_mode_for_url(base_url)
        or "chat_completions"
    )
    ...
    api_key_candidates = [
        ...
        *_inline_model_api_key_candidates(requested_provider=requested_provider, base_url=base_url),
        rp._host_derived_api_key(base_url),
    ]
    api_key = next((candidate for candidate in api_key_candidates if rp.has_usable_secret(candidate)), "")
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    candidates = [
        ...
        # Legacy Desktop configs kept the credential inline in ``model``; the configured entry has
        # no secret of its own, so the inline key is what it means (fork behaviour, restored).
        *_inline_model_api_key_candidates(requested_provider=requested_provider, base_url=base_url),
        *rp._host_gated_env_key_candidates(base_url, ollama=False),
    ]
    api_key: Any = next((c for c in candidates if rp.has_usable_secret(c)), "")
    api_mode = (custom_provider.get("api_mode") or rp._detect_api_mode_for_url(base_url) or "chat_completions")
    # An anthropic_messages endpoint with no key anywhere fails locally: a cloud 401 explains
    # nothing about ``providers.<name>.api_key`` (loopback endpoints stay keyless).
    if not rp.has_usable_secret(api_key) and api_mode == "anthropic_messages" and not _custom_endpoint_allows_no_key(base_url):
        _raise_missing_named_custom_key(
            requested_provider=requested_provider,
