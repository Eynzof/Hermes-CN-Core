...
def write_tty(seq: str) -> None:
    """Write raw escapes to the controlling terminal, falling back to sys.stdout. Never raises.

    The ``/dev/tty`` detour is POSIX-only on purpose: on Windows that path is drive-relative, so
    ``open("/dev/tty", "w")`` does not fail — it CREATES ``<current drive>:\\dev\\tty`` and every
    bell / OSC notification vanished into that junk file instead of reaching the terminal.
    """
    if os.name == "posix":
        try:
