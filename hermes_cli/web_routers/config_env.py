...
from hermes_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, get_compatible_custom_providers, redact_key, _deep_merge, ensure_starter_config_file
...
async def get_config(profile: Optional[str] = None, include_defaults: bool = True):
    ...
    def _seed_and_load_config(include_defaults: bool):
        # [CN-fork] First-run seed: a fresh install has no config.yaml and the dashboard's
        # Config page is the first surface that expects one (the CLI seeds it on its own
        # entry path). Restored with the fork's GET /api/config handler.
        ensure_starter_config_file()
        return _normalize_config_for_web(load_config() if include_defaults else read_raw_config())

    config = await scoped_to_thread(profile, lambda: _seed_and_load_config(include_defaults))
    ...
    def _run():
        ...
        with _profile_scope(body.profile or profile):
            ...
            with _CONFIG_MUTATION_LOCK:
                ...
                # [CN-fork] Provider management sends the complete ``providers`` mapping: when that key is
                # present, replace the mapping so omitted entries are deletions. Focused forms do not send
                # the key at all, so their saves must preserve it (deep-merge).
                merged = _deep_merge(existing, incoming, replace_keys={"providers"})
