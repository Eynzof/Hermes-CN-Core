...
# [CN-fork] P-002 — dashboard attachment upload helpers (max bytes / session-id
# allowlist / filename sanitiser live on ``web_server`` beside their helpers).
_parse_multipart_form = late("_parse_multipart_form")
_safe_upload_filename = late("_safe_upload_filename")
_unique_upload_path = late("_unique_upload_path")
# [CN-fork] P-059 — media helpers (media roots, path policy, range parser,
# streaming response) live on the ``web_server_files`` sibling and are
# re-imported by ``web_server``; late-bound here so a test's
# ``monkeypatch.setattr(web_server, ...)`` wins at call time.
_open_media_file = late("_open_media_file")
_parse_media_range = late("_parse_media_range")
_iter_media_file = late("_iter_media_file")
_ClosingStreamingResponse = late("_ClosingStreamingResponse")
...
async def get_media(request: Request, path: str):
    ...
    """
    from hermes_cli.web_server import _IMAGE_MEDIA_MAX_BYTES, _VIDEO_MEDIA_MAX_BYTES
...
async def stream_media_file(request: Request, path: str):
    ...
    from hermes_cli.web_server import _VIDEO_MEDIA_STREAM_MAX_BYTES
...
async def upload_attachment(request: Request):
    ...
    """
    from hermes_cli.web_server import _UPLOAD_MAX_BYTES, _UPLOAD_SESSION_RE
