...
from hermes_cli.web_server_memory import (
    _coerce_bool, _field_default, _field_is_set, _field_value, _field_visible, _memory_provider_manifest, _memory_provider_setup_info, _memory_provider_setup_manifest, _normalize_memory_provider_name, _normalize_memory_provider_schema, _read_memory_provider_existing_values, _require_memory_provider_ready, _run_setup_command,
)
...
@router.get("/api/memory/providers/{name}/status")
async def get_memory_provider_runtime_status(name: str, profile: Optional[str] = None):
    """Read-only provider runtime status (P-055): no secrets, ``details`` only."""
    _require_valid_memory_provider_name(name)
...
async def update_memory_provider_config(
    ...
    def _run():
        ...
        with _CONFIG_MUTATION_LOCK:  # RMW span vs. the dashboard's config autosave
            config = load_config()
            # ``activate: false`` (P-055) saves the connection fields WITHOUT switching the
            # active provider; omitted/true keeps the save-and-activate contract.
            if body.activate:
                _require_memory_provider_ready(name)
                _memory_section(config)["provider"] = name
                save_config(config)
        _invalidate_plugins_hub_cache()
        active = _normalize_memory_provider_name(_memory_section(config).get("provider"))
