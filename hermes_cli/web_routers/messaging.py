...
from hermes_cli.web_server_messaging import (
    _TelegramOnboardingPairing, _WhatsAppOnboardingSession, _telegram_onboarding_error_message, _telegram_onboarding_lock, _telegram_onboarding_pairings, _whatsapp_onboarding_payload, _whatsapp_onboarding_sessions,
)
...
# Both are defined on ``web_server_messaging`` but re-exported by ``web_server``, which is the
# seam the fork's Channels tests monkeypatch (``web_deps.late`` defaults to ``web_server``);
# binding them at import time would hide that patch from the handlers below.
_gateway_platform_config = late("_gateway_platform_config")
...
def _platform_enablement(
    ...
    try:
        # Resolved through the late seam (owned by ``web_server_messaging``, re-exported by
        # ``web_server``): the fork's Channels test for an empty ``required_env`` patches this
        # one helper to pin that the plugin connection check still runs.
