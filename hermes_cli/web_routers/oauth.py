...
# [CN-fork] P-025 — OAuth status TTL cache + its bust hook live on ``web_server``
# (late-bound so a test's monkeypatch there wins at call time).
_oauth_cache_key = late("_oauth_cache_key")
_oauth_status_cache_get = late("_oauth_status_cache_get")
_oauth_status_cache_put = late("_oauth_status_cache_put")
_invalidate_oauth_status_cache = late("_invalidate_oauth_status_cache")
...
async def list_oauth_providers(profile: Optional[str] = None, refresh: bool = False):
    ...
    [CN-fork] P-025 — served from a 20s per-profile TTL cache (``refresh=true``
    bypasses it) and assembled with every provider's status check run
