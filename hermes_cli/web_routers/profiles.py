...
async def get_active_profile_endpoint():
    ...
    def _run():
        ...
        active = _or_default(profiles_mod.get_active_profile)
        ...
        # existing desktop shells don't fail to load the profile list. See FORK_NOTES
        # P-008.
...
@router.put("/api/profiles/active")
...
async def open_profile_terminal_endpoint(name: str):
    with _profile_errors("POST /api/profiles/%s/open-terminal failed", name):
        ...
        if sys.platform.startswith("win"):
            # Use list argv to avoid cmd.exe re-tokenization of paths with
            # spaces. creationflags suppresses parent console flash ([CN-fork]
            # issue #90, #378; P-038).
