...
from typing import Any, BinaryIO, Dict, Iterator, List, Literal, Optional, Tuple
...
def _chat_image_extension(data: bytes) -> str | None:
    ...
    if head.startswith(b"RIFF") and head[8:12] == b"WEBP":
        return ".webp"
    for signature, extension in _CHAT_IMAGE_MAGIC:
...
def _decode_chat_image_upload(payload: ChatImageUpload) -> tuple[bytes, str, str]:
    ...
    extension = _chat_image_extension(data)
    if extension not in _CHAT_IMAGE_ALLOWED_EXTENSIONS:
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    """Persist a browser-provided image where the embedded TUI can read it."""
    data, mime_type, extension = _decode_chat_image_upload(payload)
    with _profile_scope(profile) as scoped_home:
        home = scoped_home or get_hermes_home()
        image_dir = Path(home) / "images"
        try:
        ...
        target = (
            image_dir
            / f"dashboard_{timestamp}_{secrets.token_hex(4)}_{stem}{extension}"
        )
        try:
...
def _spawn_gateway_restart(
    ...
    if existing is not None and existing.poll() is None:
        if extra_env:
            raise RuntimeError("gateway restart already in progress")
    ...
    return _spawn_hermes_action(
        subcommand, "gateway-restart", extra_env=extra_env
    ), False
...
async def restart_gateway(force: bool = False, profile: Optional[str] = None):
    """Kick off a ``hermes gateway restart`` in the background.

    ``force=1`` requests a desktop-managed takeover. ``profile`` pins the
    restart to a named Hermes profile when the desktop/dashboard is operating
    outside the default profile.
    """
    extra_env = {"HERMES_GATEWAY_FORCE_TAKEOVER": "1"} if force else None
    try:
        proc, _reused = _spawn_gateway_restart(profile, extra_env=extra_env)
    except HTTPException:
        raise
    except Exception as exc:
