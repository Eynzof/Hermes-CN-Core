...
def _gateway_platform_config(platform_id: str):
    """``(config, platform, platform_config)`` for ``platform_id``, resolved live from disk.

    Onboarding can save ``.env`` after the dashboard starts, so the Channels page resolves the
    gateway's own credential rules on every read rather than trusting a startup snapshot.
    Restored from the fork's ``web_server._gateway_platform_config`` (the dashboard decomposition
    moved the caller here); ``web_server`` re-exports it as the seam the Channels tests monkeypatch.
    """
