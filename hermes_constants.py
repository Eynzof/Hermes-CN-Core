...
def _home_or_temp() -> Path:
    """``Path.home()``, or the platform temp dir when the environment carries no home.

    Windows' ``Path.home()`` raises ``RuntimeError`` when ``USERPROFILE`` *and*
    ``HOMEDRIVE``/``HOMEPATH`` are absent (POSIX usually falls back to ``passwd``), and this
    function sits under every registry key / tool lookup — a stripped-environment process
    must still resolve *some* home instead of failing every tool call.
    """
    try:
        return Path.home()
    except RuntimeError:
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Hermes home path. Never raises."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else _home_or_temp() / "AppData" / "Local"
        return base / "hermes"
    return _home_or_temp() / ".hermes"
...
def _warn_profile_fallback_once() -> None:
    ...
    try:
        ...
        active = ""
    except Exception:  # noqa: BLE001 — pure diagnostics: must never break get_hermes_home()
        active = ""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    if " " in path and (
        path.split()[0].endswith("npx")
        or path.split()[0].endswith("npx.cmd")
        or path.split()[0].endswith("npx.exe")
    ):
        return True
...
def display_hermes_home(home: Path | None = None) -> str:
    ...
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\hermes/skills/
        # _home_or_temp(), not Path.home(): a stripped-environment child (bot-mode probe, cron worker)
        # has no USERPROFILE, and Path.home() raises RuntimeError there — every caller of this
        # display helper would die, including module-level import-time copies (turn_truncation).
        return "~/" + home.relative_to(_home_or_temp()).as_posix()
    except ValueError:
        # [CN-fork] Same chimera, other branch: a HERMES_HOME outside the user home (explicit
        # HERMES_HOME, profile home, service home) was rendered with ``str(home)``, so every
        # user-facing message that appends a "/..." suffix — e.g. the persistence-failure
        # copy's "(a copy is kept in {home}/sessions/)" — came out as
        # ``\srv\hermes-profile/sessions/`` on Windows: neither pastable nor greppable, and
        # inconsistent with the ``~/`` branch above (tests/agent/test_failed_turn_site_codes.py
        # pins the profile-aware copy).
...
def _read_proc(path: str) -> str:
    try:
        # [CN-fork] errors="replace": the kernel formats /proc entries in the C locale; a
        # non-UTF-8 byte must not turn container detection into an exception.
