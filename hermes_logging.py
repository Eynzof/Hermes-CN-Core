...
def _new_file_handler(
    ...
    # ``concurrent-log-handler`` (the Windows-only swap above) hardwires
    # ``delay=True`` — its ``_open()`` returns None by design and the file is
    # only touched by the first write — while stdlib's ``RotatingFileHandler``
    # creates it here. Everything downstream (``_record_stream_stat``, the
    # external-rotation check, ``hermes logs``, the profile-routing contract)
    # assumes ``logs/<file>`` exists as soon as setup_logging ran, so create it
    # explicitly and keep Windows at stdlib parity.
    try:
        path.touch(exist_ok=True)
    except OSError:
        pass  # an unwritable log dir is reported by the handler's own error path
    return handler
