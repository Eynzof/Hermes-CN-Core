...
def _conversation_messages(session_key: str):
    ...
    if not entry:
        return None, orjson.dumps({"error": f"Conversation not found: {session_key}"}).decode('utf-8')
    session_id = entry.get("session_id", "")
    if not session_id:
        return None, orjson.dumps({"error": "No session ID for this conversation"}).decode('utf-8')
    messages, error = _load_session_messages(session_id)
    if error:
        return None, orjson.dumps({"error": error}).decode('utf-8')
...
class _ToolHandlers:
    ...
    def conversations_list(self, platform: Optional[str] = None, limit: int = 50, search: Optional[str] = None) -> str:
        ...
        return orjson.dumps({"count": len(conversations), "conversations": conversations}, option=orjson.OPT_INDENT_2).decode('utf-8')
    ...
    def messages_read(self, session_key: str, limit: int = 50) -> str:
        ...
        return orjson.dumps({"session_key": session_key, "count": len(messages),
                           "total_in_session": len(filtered), "messages": messages}, option=orjson.OPT_INDENT_2).decode('utf-8')
    ...
    def attachments_fetch(self, session_key: str, message_id: str) -> str:
        ...
        return orjson.dumps({"message_id": message_id, "count": len(attachments), "attachments": attachments}, option=orjson.OPT_INDENT_2).decode('utf-8')
    ...
    def events_wait(self, after_cursor: int = 0, session_key: Optional[str] = None, timeout_ms: int = 30000) -> str:
        ...
        return orjson.dumps({"event": event} if event else {"event": None, "reason": "timeout"}, option=orjson.OPT_INDENT_2).decode('utf-8')
    ...
    def permissions_list_open(self) -> str:
        ...
        return orjson.dumps({"count": len(approvals), "approvals": approvals}, option=orjson.OPT_INDENT_2).decode('utf-8')
    ...
    def permissions_respond(self, id: str, decision: str) -> str:
        ...
        if decision not in {"allow-once", "allow-always", "deny"}:
            return orjson.dumps({"error": f"Invalid decision: {decision}. Must be allow-once, allow-always, or deny"}).decode('utf-8')
        return orjson.dumps(self.bridge.respond_to_approval(id, decision), option=orjson.OPT_INDENT_2).decode('utf-8')
