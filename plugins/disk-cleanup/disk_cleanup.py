...
def load_tracked() -> List[Dict[str, Any]]:
    ...
    try:
        return orjson.loads(tf.read_text(encoding="utf-8", errors="replace"))
    except (orjson.JSONDecodeError, ValueError):
        bak = tf.with_suffix(".json.bak")
        if bak.exists():
            try:
                data = orjson.loads(bak.read_text(encoding="utf-8", errors="replace"))
...
def save_tracked(tracked: List[Dict[str, Any]]) -> None:
    ...
    tmp.write_text(orjson.dumps(tracked, option=orjson.OPT_INDENT_2).decode('utf-8'), encoding="utf-8")
    if tf.exists():
