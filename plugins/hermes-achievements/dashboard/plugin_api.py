...
def save_state(state: Dict[str, Any]) -> None:
    ...
    path.write_text(orjson.dumps(state, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode('utf-8'), encoding="utf-8")
...
def save_snapshot(data: Dict[str, Any]) -> None:
    path = snapshot_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(orjson.dumps(_json_safe(data), option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode('utf-8'), encoding="utf-8")
