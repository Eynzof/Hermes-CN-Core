...
def _set_priority(conn, task_id: str, priority: int, board: Optional[str]) -> None:
    with kanban_db.write_txn(conn):
        ...
        conn.execute(
            ...
            (task_id, orjson.dumps({"priority": int(priority)}).decode('utf-8'), int(time.time())))
...
def _set_status_direct(conn: sqlite3.Connection, task_id: str, new_status: str) -> bool:
    ...
    with kanban_db.write_txn(conn):
        ...
        conn.execute(
            ...
            (task_id, run_id, orjson.dumps({"status": effective_status, "requested_status": new_status}).decode('utf-8'), int(time.time())))
...
class _EventTail:
    ...
    def _fetch(self, cursor: int) -> tuple[int, list[dict]]:
        if self._conn is None:
            ...
            self._conn = kanban_db.connect_existing(board=self._board)
