...
def _bundled_module_is_importable(module_name: str) -> bool:
    ...
    """
    # The split-layout test lives in the shared loader so every ``plugins/<kind>/`` family
    # resolves it the same way (P-057).
    return _loader.module_is_importable(module_name)


def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    """``(name, path)`` for bundled, then user-installed, then project-local; first-seen wins."""
    # PyInstaller split layout (P-057): ``--collect-data plugins`` stages each bundled
    # provider's manifest on disk while its code lives in the frozen module graph, so a
    # physical-``__init__.py``-only listing would skip it. The module prefix lets the shared
    # loader add the importable ones back — bundled providers keep precedence over
    # user/project copies.
    dirs = [(child.name, child) for child in _loader.iter_plugin_dirs(
        _MEMORY_PLUGINS_DIR, module_prefix="plugins.memory")]
...
def _load_provider_from_dir(provider_dir: Path, *, register_skills: bool = True) -> Optional["MemoryProvider"]:
    ...
    if mod is None:
        # PyInstaller split layout (P-057): ``load_plugin_module`` needs a physical
        # ``__init__.py`` beside the manifest, which the frozen bundle does not stage; the
        # provider code is importable through the frozen module loader anyway.
        if not _is_bundled(provider_dir) or not _bundled_module_is_importable(module_name):
            return None
        try:
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    # PyInstaller split layout (P-057): a bundled ``cli.py`` may exist only in the frozen
    # module graph while its manifest sits on disk — import it instead of reading a file.
    frozen_cli = (
        not (plugin_dir / "cli.py").exists()
        and _is_bundled(plugin_dir)
        and _bundled_module_is_importable(module_name)
    )
    if not (plugin_dir / "cli.py").exists() and not frozen_cli:
        return []
    try:
        cli_mod = sys.modules.get(module_name)
        if cli_mod is None and frozen_cli:
