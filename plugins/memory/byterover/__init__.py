...
def _brv_child_env(brv_path: str) -> Dict[str, str]:
    ...
    # dirname(), not str(Path(...).parent): re-rendering the resolved path through the host Path
    # flavour would rewrite a POSIX-style install dir (/opt/brv/bin) into a backslash form on Windows.
    brv_dir = os.path.dirname(brv_path)
    if brv_dir:
        env["PATH"] = brv_dir + os.pathsep + env.get("PATH", "")
    return env
...
class ByteRoverMemoryProvider(MemoryProvider):
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        return orjson.dumps(on_ok(result.get("output", ""))).decode('utf-8') if result["success"] else tool_error(result.get("error", fail_msg))
