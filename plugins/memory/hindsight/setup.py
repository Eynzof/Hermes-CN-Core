...
def run_setup(provider, hermes_home: str, config: dict) -> None:
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            # [CN-fork] P-051 — lossy orjson read: a non-UTF-8 byte in hindsight/config.json
            # must not invalidate the materialized config.
            materialized_config = orjson.loads(
                (Path(hermes_home) / "hindsight" / "config.json").read_text(
