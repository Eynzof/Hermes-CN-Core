...
class HolographicMemoryProvider(MemoryProvider):
    ...
    _TOOL_HANDLERS = {
        "fact_store": _tool_handler({
            "add": lambda self, a: orjson.dumps({"fact_id": self._store.add_fact(
                a["content"], category=a.get("category", "general"), tags=a.get("tags", "")), "status": "added"}).decode('utf-8'),
            ...
            "update": lambda self, a: orjson.dumps({"updated": self._store.update_fact(
                int(a["fact_id"]), content=a.get("content"), trust_delta=float(a["trust_delta"]) if "trust_delta" in a else None,
                tags=a.get("tags"), category=a.get("category"))}).decode('utf-8'),
            "remove": lambda self, a: orjson.dumps({"removed": self._store.remove_fact(int(a["fact_id"]))}).decode('utf-8'),
        ...
        "fact_feedback": lambda self, a: orjson.dumps(self._store.record_feedback(int(a["fact_id"]), helpful=a["action"] == "helpful")).decode('utf-8'),
    }
