...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def identity_signature(self) -> Dict[str, Any]:
        ...
        Memoized on the file CONTENT digest, not on stat(mtime,size): a stat-only key cannot detect
        an in-place, same-size rewrite when the filesystem timestamp is coarse (and the inode
        necessarily stays the same). The file is small, so hashing its bytes keeps the per-message
        call cheap while making identity changes reliably invalidate the cached agent on every
        supported platform. ``{}`` when the config cannot be read."""
        try:
    ...
    def _tool_profile(self, args: dict) -> str:
        ...
        return orjson.dumps({"result": card} if card else self._empty_profile_hint(peer)).decode('utf-8')
    ...
    def _tool_search(self, args: dict) -> str:
        ...
        return orjson.dumps({"result": result or "No relevant context found."}).decode('utf-8')
