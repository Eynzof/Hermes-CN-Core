...
def _read_config(path: Path) -> dict:
    ...
    return json.loads(path.read_text(encoding="utf-8-sig", errors="replace")) if path.exists() else {}


def _user_home_or_temp() -> Path:
    ...
    ``HOMEDRIVE``/``HOMEPATH`` are absent (POSIX falls back to passwd). This lookup sits on the
    first-time-setup path, so a stripped-environment process (gateway/desktop child) must still
    resolve *some* home instead of failing the whole config resolution.
    """
    try:
        return Path.home()
    except RuntimeError:
...
def resolve_global_config_path() -> Path:
    ...
    return _user_home_or_temp() / ".honcho" / "config.json"
