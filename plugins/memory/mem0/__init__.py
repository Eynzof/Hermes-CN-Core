...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def _tool_list(self, args: dict) -> str:
        """Paginated listing of the principal's stored memories (legacy v3 tool name; callable,
        deliberately not advertised in TOOL_SCHEMAS / the system prompt)."""
        page = max(1, int(args.get("page", 1)))
        page_size = min(max(1, int(args.get("page_size", 100))), 200)
        response = self._backend.get_all(
            filters={"user_id": self._user_id}, page=page, page_size=page_size,
        )
        results = response.get("results", []) if isinstance(response, dict) else []
        if not results:
            return orjson.dumps({"result": "No memories stored yet."}).decode('utf-8')
        items = [{"id": m.get("id"), "memory": m.get("memory", "")} for m in results]
        return orjson.dumps({
            "results": items, "count": response.get("count", len(items)), "page": page, "page_size": page_size,
        }).decode('utf-8')

    _TOOL_HANDLERS = {
        "mem0_search": (("query",), "Search failed", _tool_search, "skip"),
        "mem0_add": (("content",), "Failed to store", _tool_add, "count"),
        "mem0_update": (("memory_id", "text"), "Update failed", lambda self, a: orjson.dumps(self._backend.update(a["memory_id"], a["text"])).decode('utf-8'), "not_found"),
        "mem0_delete": (("memory_id",), "Delete failed", lambda self, a: orjson.dumps(self._backend.delete(a["memory_id"])).decode('utf-8'), "not_found"),
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        if self._backend is None:
            ...
            return orjson.dumps({"error": f"Mem0 backend not initialized: {err}.{self._oss_hint(' Check that {vs} is running and reachable.')}"}).decode('utf-8')
        if self._is_breaker_open():
            return orjson.dumps({"error": f"Mem0 temporarily unavailable (multiple consecutive failures). Will retry automatically.{self._oss_hint(' Check that your {vs} is running.')}"}).decode('utf-8')
