...
def _load_ovcli_config(path: Optional[Path] = None) -> dict:
    ...
    if not config_path.exists():
        return {}
    data = orjson.loads(config_path.read_text(encoding="utf-8-sig"))
...
def _write_env_vars(env_path: Path, env_writes: dict, remove_keys: tuple[str, ...] = ()) -> None:
    ...
    # newline="\n": a .env must stay LF on Windows too; the default text-mode write
    # rewrites every line as CRLF, losing the original separators on a round-trip.
    env_path.write_text("\n".join(new_lines) + ("\n" if new_lines else ""),
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def _tool_search(self, args: dict) -> str:
        ...
        return orjson.dumps({"results": formatted, "total": result.get("total", len(formatted))}).decode('utf-8')
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        return orjson.dumps({"level": level, "results": results, "requested": len(uris), "returned": len(results),
                           "truncated": len(uris) > len(selected)}).decode('utf-8')
