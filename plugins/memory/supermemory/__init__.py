...
def _read_json_dict(path: Path) -> dict:
    raw = _quietly(lambda: orjson.loads(path.read_text(encoding="utf-8-sig", errors="replace")), "Failed to parse %s", path) if path.exists() else None
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        def _init_client():
            try:
                return _build_client(self._api_key, config, self._container_tag)
            except Exception as exc:
                self._init_error = str(exc)
                raise
        self._client = _quietly(_init_client,
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        schemas = [orjson.loads(orjson.dumps(base).decode('utf-8')) for base in _BASE_SCHEMAS]  # deep copies
        ...
        return schemas + [{**orjson.loads(orjson.dumps(s).decode('utf-8')), "name": _KEBAB_ALIASES[s["name"]]} for s in schemas]
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        ...
        return resp if isinstance(resp, str) else orjson.dumps(resp).decode('utf-8')
