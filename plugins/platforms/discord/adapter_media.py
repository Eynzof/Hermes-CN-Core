            ...
            try:
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        # (``/C:/dir/x.png``) that os.path.exists can never resolve, so every attached
                        # image was silently skipped on Windows; the hand-built ``file://C:\...`` form
                        # (adapters pass ``f"file://{path}"``) must keep its old url[7:] meaning.
