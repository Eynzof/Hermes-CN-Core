...
# [CN-fork] Windows surfaces "address already in use" as WSAEADDRINUSE (10048) rather than
...
def _build_markdown_post_payload(content: str) -> str:
    ...
    return orjson.dumps({"zh_cn": {"content": rows}}).decode('utf-8')
...
def _load_lark_oapi() -> bool:
    ...
    if lark is not None:
        return True
    # Fork intent (kept from HEAD): only raise the SDK when the connection actually
    # requests it, via the shared lazy-deps installer/binder so the names land in this
    # module's globals. The merge had left the fork's guards around upstream's raw
    # importlib body, which orphaned _lark_bindings() and bound a second copy of the
    # SDK without the installer/plugin accounting (lazy_deps.ensure_and_bind).
...
class FeishuAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if not result.success and msg_type == "post" and _POST_CONTENT_INVALID_RE.search(result.error or ""):
                ...
                result = await _update(
                    "text", orjson.dumps({"text": _strip_markdown_to_plain_text(content)}).decode('utf-8'),
                )
            if result.success:
    ...
    async def _send_interactive_card(
        ...
        response = await self._feishu_send_with_retry(
            chat_id=chat_id, msg_type="interactive", payload=orjson.dumps(card).decode('utf-8'),
    ...
    async def _maybe_extract_text_document(self, cached_path: str, media_type: str) -> str:
        ...
        try:
            ...
            if ext not in {".txt", ".md"} and media_type not in {"text/plain", "text/markdown"}:
                return ""
            content = Path(cached_path).read_text(encoding="utf-8", errors="replace")
    ...
    def _load_seen_message_ids(self) -> None:
        try:
            payload = orjson.loads(self._dedup_state_path.read_text(encoding="utf-8", errors="replace"))
    ...
    def _build_outbound_payload(self, content: str, *, prefer_post: bool = False) -> tuple[str, str]:
        ...
        return "text", orjson.dumps({"text": content}).decode('utf-8')
    ...
    async def _send_uploaded_file_message(
        ...
        try:
            ...
            if (not caption
                ...
                payload = orjson.dumps(key_payload).decode('utf-8')
    ...
    def _build_media_post_payload(self, *, caption: str, media_tag: Dict[str, str]) -> str:
        payload = orjson.loads(_build_markdown_post_payload(caption))
...
def _probe_bot_sdk(app_id: str, app_secret: str, domain: str) -> Optional[dict]:
    ...
    try:
        ...
        return None if content is None else _parse_bot_response(orjson.loads(content))
    except Exception as exc:
...
def _probe_bot_http(app_id: str, app_secret: str, domain: str) -> Optional[dict]:
    ...
    try:
        token_res = _get_json(
            ...
            data=orjson.dumps({"app_id": app_id, "app_secret": app_secret}).decode('utf-8').encode("utf-8"),
        )
