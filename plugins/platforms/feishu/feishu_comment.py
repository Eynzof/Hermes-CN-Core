...
async def _exec_request(client, method, uri, paths=None, queries=None, body=None):
    ...
    logger.info("[Feishu-Comment] API >>> %s %s paths=%s queries=%s body=%s", method, uri, paths, queries, orjson.dumps(body).decode('utf-8')[:500] if body else None)
    ...
    try:
        data: dict = orjson.loads(raw.content).get("data", {}) if has_raw else {}
