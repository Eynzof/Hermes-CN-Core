...
def _persist_credentials(creds: Any, token_path: Path) -> None:
    ...
    try:
        _write_private_json(token_path, _normalize_authorized_user_payload(orjson.loads(creds.to_json())))
    except Exception:
...
def _write_private_json(path: Path, data: Any) -> None:
    ...
    atomic_write_text(path, orjson.dumps(data, option=orjson.OPT_INDENT_2).decode('utf-8'), create_mode=0o600)
...
def _load_pending_auth(email: Optional[str] = None) -> dict:
    ...
    try:
        data = orjson.loads(pending.read_text(encoding="utf-8"))
    except Exception as exc:
