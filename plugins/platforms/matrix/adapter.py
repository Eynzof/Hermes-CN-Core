...
def _run_media_tool(cmd: list, *, timeout: int, text: bool = False,
    ...
    # [CN-fork] P-051: text-mode pipes carry an explicit decoding — locale.getpreferredencoding()
    # is cp936/GBK on zh-CN Windows, and one undecodable byte kills CPython's reader thread.
    # Left None for the binary (ffmpeg PCM) call, where stdout must stay bytes.
    return subprocess.run(cmd, capture_output=True, text=text, timeout=timeout,
                          stdin=subprocess.DEVNULL, encoding=encoding, errors=errors)
        ...
        for idx, (image_url, alt_text) in enumerate(images, start=1):
            ...
            if image_url.startswith("file://"):
                result = await self.send_image_file(
                    chat_id=chat_id,                 image_path=file_url_to_local_path(image_url), caption=caption, metadata=metadata)
            else:
