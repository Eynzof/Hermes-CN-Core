...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_sticker(self, msg: Message, event: "MessageEvent") -> None:
        ...
        try:
            ...
            result = orjson.loads(await vision_analyze_tool(image_url=cached_path, user_prompt=STICKER_VISION_PROMPT))
...
def _apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
    ...
    if "mention_patterns" in telegram_cfg:
        _set_env("TELEGRAM_MENTION_PATTERNS", _json.dumps(telegram_cfg["mention_patterns"]).decode('utf-8'))
    for key, env in (
