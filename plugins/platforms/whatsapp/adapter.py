...
from hermes_constants import (find_node_executable, get_hermes_dir, get_hermes_home, get_subprocess_home, with_hermes_node_path)
    ...
    if _IS_WINDOWS:
        try:
            result = subprocess.run(
                ...
                capture_output=True, text=True, errors="replace", timeout=10,  # taskkill prints in the OEM/ANSI codepage, never UTF-8
            )
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _bridge_env(self) -> dict:
        ...
        profile_home = get_subprocess_home(bridge_env)
        if profile_home:
