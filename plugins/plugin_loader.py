...
def _has_init_file(child: Path) -> bool:
    """True when *child* carries an ``__init__.py``; an unreadable child raises ``OSError``.

    ``Path.exists()`` cannot answer this: since Python 3.14 it is ``os.path.exists`` and swallows
    every ``OSError``, so a mode-000 / ACL-denied child would be dropped silently instead of being
    skipped with the warning this listing promises (#111804). A direct ``stat()`` lets a
    ``PermissionError`` reach the caller, while a genuinely absent file is not an error.
    """
    try:
        (child / "__init__.py").stat()
        return True
    except FileNotFoundError:
        return False


def iter_plugin_dirs(root: Path, module_prefix: Optional[str] = None) -> List[Path]:
    """Sorted child dirs of *root* that have an ``__init__.py`` (skips ``_``/``.`` names).

    With *module_prefix*, a child whose ``<module_prefix>.<name>`` package is importable
    through the frozen module loader also counts (PyInstaller split layout: the manifest is
    staged on disk while the code lives in the frozen module graph — P-057).
    """
    if not root.is_dir():
        return []
    dirs: List[Path] = []
    for child in sorted(root.iterdir()):
        ...
        try:
            if child.is_dir() and (
                _has_init_file(child)
                or (module_prefix is not None
                    and module_is_importable(f"{module_prefix}.{child.name}"))
            ):
                dirs.append(child)
...
def module_is_importable(module_name: str) -> bool:
    """True when *module_name* has executable code available (P-057).

    PyInstaller stores Python modules in its frozen module graph while ``--collect-data``
    stages manifests and README files on disk. In that split layout
    ``plugins/<kind>/<name>/`` exists but its ``__init__.py`` does not, so a directory-only
    test would advertise no provider. ``find_spec`` bridges the two halves without importing
    the module during the cheap discovery pass.
    """
...
def load_plugin_module(module_name: str, plugin_dir: Path, *, parents: Tuple[str, ...],
    ...
    if not init_file.exists():
        # PyInstaller split layout (P-057): the manifest is staged on disk while the code lives
        # in the frozen module graph — import it through the normal module loader.
        if not module_is_importable(module_name):
            return None
        try:
