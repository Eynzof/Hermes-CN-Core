...
from agent.tool_result_store import (  # noqa: F401
    # Memory-bounded tool-result buffer with transparent disk spill, for large-result handling
    # in long-running sessions. Re-exported here so callers and tests can
    # `from run_agent import ToolResultStore` (plan surface).
    ToolCallResult,
    ToolResultStore,
)
...
class AIAgent(
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        try:
            ...
            loaded = ensure_lmstudio_model_loaded(
                self.model, self.base_url, getattr(self, "api_key", ""), target_ctx, return_load_result=True,
            )
            # [CN-fork] Push into the live compressor so the status bar reflects the real loaded ctx the
            # moment the load resolves, instead of holding the previous model's value (or "ctx --").
            ...
            if isinstance(ctx_len, int) and not isinstance(ctx_len, bool) and ctx_len > 0:
                ...
                if cc is not None:
                    cc.update_model(model=self.model, context_length=ctx_len, base_url=self.base_url,
    ...
    def _is_provider_stream_parse_error(self, error: BaseException) -> bool:
        ...
        return (getattr(self, "api_mode", None) == "anthropic_messages" and isinstance(error, ValueError)
                and not isinstance(error, (UnicodeEncodeError, orjson.JSONDecodeError))
    ...
    # Single source of truth lives in ``agent.message_utils`` (the sanitizer's leaf module);
    # aliased here for the many ``AIAgent._VALID_API_ROLES`` call sites and tests.
