...
def _stored_token_scopes() -> list[str]:
    try:
        data = orjson.loads(TOKEN_PATH.read_text(encoding="utf-8"))
    except Exception:
        return list(SCOPES)
...
def get_credentials():
    ...
    if creds.expired and creds.refresh_token:
        ...
        TOKEN_PATH.write_text(
            orjson.dumps(_normalize_authorized_user_payload(orjson.loads(creds.to_json())), option=orjson.OPT_INDENT_2).decode('utf-8'), encoding="utf-8"
        )
    if not creds.valid:
