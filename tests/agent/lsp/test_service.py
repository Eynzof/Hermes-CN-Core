    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                sys.platform == "win32",
                reason="POSIX-only scenario: a live child's half-closed stdout never EOFs the "
                       "parent on Windows (the pipe's write end stays reachable until the child "
                       "exits), so the reader-failure path cannot be driven this way. The "
                       "malformed_frame sibling covers the same replacement contract here.",
            ),
        ),
        "malformed_frame",
    ],
