...
def _git(*args: str, cwd: Path) -> str:
    # Same config isolation the production probes use
    # (``hermes_cli._subprocess_compat.noninteractive_git_env``): a host
    # ``core.autocrlf=true`` would otherwise store LF in the index while the worktree keeps
    # CRLF, and the isolated probe then reports the freshly created worktree as DIRTY, so
    # ``finalize_subagent_worktree`` legitimately refuses to prune it.
    env = {
        **os.environ,
...
@pytest.fixture
def mock_pyright(request, tmp_path):
    ...
    def _spawn(root: str, ctx: ServerContext) -> SpawnSpec:
        # cwd is the shared tmp_path, NOT ``root``: Windows refuses to delete a directory
        # that a live process has as its current directory (WinError 32), and this suite
        # removes workspace trees both while a multi-root server keeps serving its sibling
        # and (in the reaper test) while the client is still alive. The server's cwd is not
        # the subject of these tests — the workspace-folder bookkeeping is.
        return SpawnSpec(command=[sys.executable, MOCK_SERVER], workspace_root=root, cwd=str(tmp_path),
