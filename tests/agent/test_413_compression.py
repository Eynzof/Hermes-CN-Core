...
class TestPreflightCompression:
    ...
    def test_preflight_defers_rough_growth_to_the_provider_then_compresses(self, agent):
        """[F12 merge reconciliation of the fork's "rough growth after a fit" preflight.]

        The fork compressed on the rough estimate as soon as it grew past the threshold.
        The merged product keeps upstream's deferral gate
        (``ContextCompressor.should_defer_preflight_to_real_usage``, pinned by
        tests/agent/test_context_compressor.py::TestPreflightDeferral): a whole-context
        ROUGH estimate over threshold waits ONE request for the provider's own pricing,
        because the rough estimate deliberately overcounts schema-heavy requests. So this
        pins both halves of the merged contract in one turn-pair:

        1. a fitting real reading + a rough estimate over threshold -> DEFER (no
           summarisation call, the turn still completes, nothing is announced);
        2. once the provider prices the request over the threshold, the preflight fires
           on the very next turn (no further waiting) and announces itself.
        """
        agent.compression_enabled = True
        agent.context_compressor.context_length = 200_000
        agent.context_compressor.threshold_tokens = 100_000
        # The last request the provider priced FIT under the threshold; only the rough
        # estimate is over it, which is exactly the shape the deferral gate exists for.
        ...
        for i in range(20):
            ...
            big_history.append({"role": "assistant", "content": f"Response {i} padded"})
        ok_resp = _mock_response(content="After custom preflight", finish_reason="stop")
        agent.client.chat.completions.create.side_effect = [ok_resp, ok_resp]
        status_messages = []
        agent.status_callback = lambda ev, msg: status_messages.append((ev, msg))
        with (
            patch("agent.turn_context.estimate_request_tokens_rough", return_value=114_000),
            # [F12 merge reconciliation] the fork's preflight estimate used to be read from
            # agent.conversation_loop; the decomposition moved the call site to
            # agent.model_metadata (agent/turn_preflight.py imports it from there), which is
            # also what the sibling preflight tests in this class patch.
            patch("agent.model_metadata.estimate_request_tokens_rough", return_value=114_000),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            ...
            mock_compress.return_value = (
                [{"role": "user", "content": f"{SUMMARY_PREFIX}Previous conversation"}],
                "new system prompt",
            )
            result = agent.run_conversation("hello", conversation_history=big_history)
            # Phase 1: the estimate is over threshold but the provider has not proven the
            # request overflows, so the preflight waits for the next real reading.
            mock_compress.assert_not_called()
            assert result["completed"] is True
            assert not any("LCM context maintenance" in msg for _ev, msg in status_messages)
            
            # Phase 2: the provider priced the request over the threshold -> the deferral
            # gate opens and the preflight compresses on this turn.
            agent.context_compressor.last_prompt_tokens = 120_000
            agent.context_compressor.last_real_prompt_tokens = 120_000
            status_messages.clear()
            mock_compress.reset_mock()
            agent.run_conversation("hello again", conversation_history=big_history)
            mock_compress.assert_called_once()
            lifecycle_messages = [msg for ev, msg in status_messages if ev == "lifecycle"]
            assert "🔧 LCM context maintenance: preparing compacted context." in lifecycle_messages
            assert not any("Preflight compression" in msg for msg in lifecycle_messages)
