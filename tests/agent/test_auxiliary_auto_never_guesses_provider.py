...
def test_no_selected_main_provider_still_discovers(nous_is_the_only_working_provider):
    """No selected main provider ⇒ the built-in discovery chain still runs, but only over the
    REACHABLE rungs (local/custom → direct api-key providers): P-039 keeps OpenRouter and Nous
    Portal out of the implicit chain, so the nous rung is never consulted even though the fixture
    makes it answer. Without this restriction every compression / title-gen / vision call on a CN
    install paid a dead network wait probing portals it cannot reach."""
    api_client = MagicMock(name="api-key-client")
    with patch.object(aux, "_try_nous", return_value=(MagicMock(name="nous"), "nous-model")) as nous_try, \
         patch.object(aux, "_resolve_api_key_provider", return_value=(api_client, "api-model")), \
         patch.object(aux, "_read_main_provider", return_value="auto"):
        client, model, label = aux._resolve_auto_route(main_runtime={"provider": "auto"}, task="compression")
    assert client is api_client and (model, label) == ("api-model", "api-key")
