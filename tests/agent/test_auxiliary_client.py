...
class TestResolveProviderClientUniversalModelFallback:
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        ...
        with (
            ...
            patch(
                # ``resolve_anthropic_token`` lives in ``agent.anthropic_credentials`` since the
            # module decomposition; ``agent.anthropic_adapter`` only re-exports it for
            # external plugins, so patching the old path would not intercept the call.
...
class TestExpiredCodexFallback:
    ...
    def test_expired_codex_openrouter_key_is_not_implicit_fallback(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client.OpenAI") as mock_openai, \
            ...
            # CN fork P-039: the implicit auto chain is main provider → local/custom → direct
            # api-key providers → None. OpenRouter and Nous Portal are NEVER probed implicitly
            # (CN installs typically cannot reach them; probing them only added a dead network
            # wait to every compression / title-gen / vision call), so a bare OPENROUTER_API_KEY
            # must not make auto resolve an OpenRouter client here.
            from agent.auxiliary_client import _resolve_auto_route
            client, model, _provider = _resolve_auto_route()
            assert client is None
            assert model is None
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        """_resolve_auto_route Step-2 chain skips unhealthy providers."""
        from agent.auxiliary_client import (
            _resolve_auto_route,
            _mark_provider_unhealthy,
        )
