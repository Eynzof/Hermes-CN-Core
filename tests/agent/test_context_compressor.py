...
class TestTruncationMarkerNotImitable:
    ...
    def test_args_without_a_net_gain_leaf_are_left_byte_identical(self):
        ...
        # [F12 merge reconciliation] The case is the per-leaf break-even, not json.dumps'
        # separator whitespace: the merged helper re-serialises with the fork's compact orjson
        # pin (agent/context_compressor.py), so re-serialising costs ZERO bytes and a leaf that
        # saves even one byte is a genuine net reduction. 419 = head_chars (200) + len(marker)
        # for this shape, i.e. the exact boundary where `head_chars + len(marker) >= len(obj)`
        # still keeps the leaf byte-identical; 426 (upstream's number) sits 7 bytes above it and
        # is now legitimately shrunk.
        many_keys = json.dumps(
            {**{f"k{i}": i for i in range(300)}, "big": "y" * 419}, separators=(",", ":")
        )
