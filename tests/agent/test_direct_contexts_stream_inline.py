...
def test_inline_stream_stale_detector_still_fires_from_monitor_thread(
    ...
    # worker path, #112419).
    # [F12] The abort wakes the reader with the transport's own error, and which error
    # that is depends on the socket layer: on POSIX the FD-safe shutdown() makes the
    # parked recv() return EOF, so httpcore reports RemoteProtocolError; on Windows the
    # same shutdown() wakes recv() with WSAECONNABORTED (WinError 10053), which httpx
    # maps to ReadError. Both are the same undelivered-stream failure the loop
    # reclassifies together (agent/chat_completion_helpers.py, #112419), so pin the type
    # per platform instead of loosening the assertion.
    _undelivered_error = (
        httpx.RemoteProtocolError if sys.platform != "win32" else httpx.ReadError
    )
    with pytest.raises(_undelivered_error):
