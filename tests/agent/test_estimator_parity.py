...
class TestNoProgressDeadLoopBreaker:
    ...
    def test_commit_layer_no_progress_calls_recorder(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Close the handle INSIDE the temp-dir scope: Windows keeps an exclusive
            # lock on the open SQLite file, so leaving the ``with`` with a live
            # connection fails with WinError 32 before the assertions below ever run.
            # Every other SessionDB test in this suite closes its handle for the same
            # reason.
            db.close()
