...
class TestRecordFileMutationResult:
    ...
    def test_success_under_another_spelling_clears_failure(self, tmp_path, monkeypatch):
        ...
        agent._record_file_mutation_result(
            ...
            orjson.dumps({"error": "Could not find old_string"}).decode('utf-8'), is_error=True, task_id=task,
        )
        assert "notes.md" in agent._turn_failed_file_mutations
        landed = str(tmp_path / "notes.md")
        agent._record_file_mutation_result(
            "write_file", {"path": landed, "content": "y"},
            orjson.dumps({"bytes_written": 1, "files_modified": [landed]}).decode('utf-8'), is_error=False, task_id=task,
        )
        assert agent._turn_failed_file_mutations == {}
    ...
    def test_footer_omits_file_changed_after_failed_call(self, tmp_path):
        ...
        for target in (changed, untouched):
            agent._record_file_mutation_result(
                ...
                orjson.dumps({"error": "Could not find old_string"}).decode('utf-8'), is_error=True,
            )
        import os
        st = changed.stat()
