...
class TestProfileHomeProcessHome:
    ...
    def test_every_home_is_guarded(self, profile_home_env):
        # The OS user's real home via the product's own resolver (``pwd`` is POSIX-only;
        # ``get_real_home`` answers the same question on every platform, and it is the
        # helper ``file_safety._guard_homes`` itself adds to the guard set).
    ...
    def test_benign_paths_stay_writable(self, profile_home_env, tmp_path):
        ...
        if sys.platform == "win32":
            # ``ntpath.expanduser`` fabricates ``<parent-of-home>\\<name>`` for ANY ``~name``
            # (it cannot consult a passwd database), so the guard covers the invented home and
            # a mistyped account is denied — a Windows-only over-block, and harmless: it can
            # only ever deny paths under a home that does not exist. The POSIX branch below is
            # the original contract (``expanduser`` leaves an unknown account untouched).
            assert fs.is_write_denied("~nosuchuser-hopefully/.ssh/authorized_keys") is True
        else:
