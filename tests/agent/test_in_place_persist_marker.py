...
class TestInPlaceCommitPersistMarker:
    def test_post_commit_persist_does_not_reinsert_compacted_rows(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Close inside the temp-dir scope: Windows holds an exclusive lock on the open
            # SQLite file, so exiting the ``with`` with a live connection fails with
            # WinError 32 (the assertions above are the subject of this test).
            db.close()
    ...
    def test_stamp_helper_shared_by_micro_compact_sync(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for msg in compacted:
                assert msg.get(_DB_PERSISTED_MARKER) is True
            # Windows locks the open SQLite file: close before the temp-dir teardown.
            db.close()
