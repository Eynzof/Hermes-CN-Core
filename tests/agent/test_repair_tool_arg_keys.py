...
class TestRealTools:
    ...
    def test_clarify_real_schema(self):
        """clarify: the live schema advertises the batch ``questions`` array (the older/n        single-question shape stays ACCEPTED but unadvertised, see tools/clarify_tool.py), so the/n        pre-merge ``query``→``question`` alias has no target any more — a legacy single-question/n        key must pass through untouched for the handler to keep accepting it."""
        args = {"query": "What?"}
        result = repair_tool_arg_keys("clarify", args)
        assert result == {"query": "What?"}
        # ...and the key the handler does accept is never renamed onto the array-typed field.
        assert repair_tool_arg_keys("clarify", {"question": "What?"}) == {"questions": "What?"}
...
class TestArgRepairCallback:
    ...
    def test_callback_called_on_repair(self):
        ...
        try:
            ...
            assert len(calls) == 1
            # ``todo`` was renamed ``todo_list`` (legacy calls are normalized by the registry
            # alias map, so this dispatch still reaches the agent-loop guard).
            assert calls[0] == ("todo_list", ["file"], ["path"])
        finally:
            set_arg_repair_callback(None)
    ...
    def test_callback_exception_does_not_break_dispatch(self):
        ...
        try:
            ...
            assert "todo_list must be handled by the agent loop" in result
        finally:
            set_arg_repair_callback(None)
