...
class TestAgentRuntimePostHookOwnershipSync:
    ...
    _CASES = (
        ...
        # [F12 merge reconciliation] no ("read_preview", {}) case: read_preview is NOT an
    # agent-runtime (inline-dispatched) tool — agent/inline_tool_executors.INLINE_TOOL_EXECUTORS
    # has no "read_preview" key, and agent._invoke_tool falls through to
    # model_tools.handle_function_call for it (the parametrized case below fails on that
    # AssertionError, and desktop_preview(action="read") already covers the shared bridge). The
    # ownership set is the contract this class pins, so it must list only the inline tools.
