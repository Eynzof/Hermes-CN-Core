...
# [F12] linux_only: the recovery this pins is the monitor's FD-safe socket shutdown
# (agent/agent_runtime_helpers._shutdown_socket -> shutdown(SHUT_RDWR), never close(),
# #30858) waking a reader parked in recv(). POSIX honours that for a blocked recv();
# Windows does not — a local shutdown() leaves the pending recv() blocked (measured:
# shutdown() with and without a preceding settimeout(0) both keep recv() parked until
# the peer closes), so there the attempt can only be bounded by HERMES_STREAM_READ_TIMEOUT
# or by the monitor's bounded stale escalation (agent/chat_completion_stream_monitor.py,
# P-022 — exercised platform-independently by tests/agent/test_streaming_stale_timeout.py).
# The unit half of this file stays platform-neutral; only the end-to-end timing bound
# below needs the POSIX wakeup.
