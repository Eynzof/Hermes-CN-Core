...
def test_desktop_publishes_final_commands_before_wait_and_runs_in_order(tmp_path, monkeypatch, read_count, threaded_middleware):
    ...
    with ExitStack() as scope, patch(
        ...
        try:
            ...
            # The persistent interactive shell on Windows is Git-Bash (the fork's default
            # there), whose ``pwd`` prints the MSYS form ``/c/Users/...``; cmd/PowerShell print
            # ``C:\Users\...``. Assert the cwd reached the output in either rendering, with the
            # drive prefix/case normalised away.
            output = json.loads(messages[-1]["content"])["output"].replace("\\", "/")
            assert str(nested).replace("\\", "/").split(":/", 1)[-1] in output
