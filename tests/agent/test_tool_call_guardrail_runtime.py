...
def test_default_sequential_path_warns_repeated_exact_failure_without_blocking_execution():
    ...
    with patch("model_tools.handle_function_call", return_value=orjson.dumps({"error": "boom"}).decode('utf-8')) as mock_hfc:
...
def test_sequential_after_call_appends_guidance_to_tool_result_without_extra_messages():
    ...
    with patch("model_tools.handle_function_call", return_value=orjson.dumps({"error": "boom"}).decode('utf-8')):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_same_tool_failure_warning_tells_model_to_recover_with_tools():
    ...
    with patch("model_tools.handle_function_call", return_value=orjson.dumps({"exit_code": 1}).decode('utf-8')):
        agent._execute_tool_calls_sequential(msg, messages, "task-1")
...
def test_default_run_conversation_warns_without_guardrail_halt():
    ...
    with (
        patch("model_tools.handle_function_call", return_value=orjson.dumps({"error": "boom"}).decode('utf-8')) as mock_hfc,
...
def test_guardrail_halt_emits_final_response_through_stream_delta_callback():
    ...
    with (
        patch("model_tools.handle_function_call", return_value=orjson.dumps({"error": "boom"}).decode('utf-8')),
        patch.object(agent, "_persist_session"),
        patch.object(agent, "_save_trajectory"),
        patch.object(agent, "_cleanup_task_resources"),
    ):
