...
class TestCodeExecutionDelegation:
    ...
    def test_escalate_waits_then_sigkills_tree(self, monkeypatch):
        ...
        # ``SIGKILL`` does not exist on Windows; the escalation stage then passes ``None``,
        # which ``agent.deadline.kill_process_tree`` reads as "force-kill the tree"
        # (``taskkill /T /F``). The product resolves it with ``getattr(signal, "SIGKILL", None)``
        # so both stages still fire — assert the platform's force-kill signal.
        assert calls == [(6666, _signal.SIGTERM), (6666, getattr(_signal, "SIGKILL", None))]
