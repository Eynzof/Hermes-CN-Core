...
def _line(agent, caplog, resp):
    ...
    caplog.clear()
    # [F12 merge reconciliation] the record is emitted by the agent.conversation_loop
    # logger (agent/turn_usage.py keeps the origin module's logger name), and upstream
    # relied on hermes_logging.setup_logging() leaving the ROOT logger at INFO; the
    # fork's tests/agent/conftest.py stubs that entry point out, so enable the level
    # here. Assertions unchanged.
