...
def _live_system_guard(request, monkeypatch):
    ...
    def _first_token_basename(cmd) -> str:
        """Basename of argv[0], lowercased.

        A list/tuple argv is authoritative: on Windows the path arrives as
        ``C:\\...\\stub-bin\\docker`` and re-splitting the joined command string with
        shlex eats the backslashes (posix mode treats them as escapes), so
        ``docker exec ... hermes gateway start`` read as a host gateway spawn and got
        blocked even though tests/docker/ legitimately drives a container.
        """
        ...
        # Windows dispatches executables by extension, so a real invocation can arrive as
        # docker.exe (full path to Docker Desktop's binary) or docker.cmd (a wrapper); the
        # runtime comparison is by program name, not by file name.
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        in_container = _first_token_basename(cmd) in _CONTAINER_RUNTIMES
        if (
            not lookalike_ok
