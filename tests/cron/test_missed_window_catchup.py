...
def _shell_path(path: Path) -> str:
    """The POSIX spelling of ``path`` in the bash that runs job scripts.

    cron runs ``.sh`` jobs through the host's bash (cron/scheduler_script.py
    ``_script_argv``), so a native Windows path interpolated into a script body
    is eaten by backslash escaping and the write lands in a file literally named
    ``C:Users...``. Ask the shell to translate.
    """
    if os.name != "nt":
        return str(path)
...
def slot_env(tmp_path, monkeypatch):
    ...
    (home / "scripts" / "fire.sh").write_text(
        f"#!/bin/sh\necho fired >> {_shell_path(counter)}\necho fired\n", encoding="utf-8")
