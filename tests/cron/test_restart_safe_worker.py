...
def test_scoped_wrapper_exit_without_user_bus_names_the_cause_and_invalidates_probe(
    ...
    Linux-only: the ``scoped`` dispatch mode this drives can only be produced by the Linux user-bus
    probe (``_systemd_run_user_scope_available`` is gated on ``_IS_LINUX``), and the message it
    asserts on spells the bus path as ``/run/user/{os.getuid()}/bus`` — a POSIX-only API. The
    surrounding tests that do not need the scope mode stay unmarked.
    """
    import cron.scheduler as scheduler
