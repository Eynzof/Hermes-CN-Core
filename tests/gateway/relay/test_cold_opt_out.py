...
def test_cold_fronted_platforms_is_read_only(tmp_path, case):
    ...
    # ``PYTHONIOENCODING`` is set on purpose by this fork (hermes_cli/__init__.py setdefault) so that
    # every child process decodes stdio as UTF-8 on Windows; it is not relay config or a secret
    # leaking into the environment, which is what changed_env exists to catch.
    observed["changed_env"].pop("PYTHONIOENCODING", None)
