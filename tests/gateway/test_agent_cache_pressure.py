...
class TestPressureSignalScope:
    ...
    def _cgroup(self, monkeypatch, tmp_path, stat_text):
        ...
        def read_text(self, *args, **kwargs):
            # as_posix(): the faked cgroup paths are Linux spellings, and this
            # stub runs on Windows too where str(Path) uses backslashes.
            spelling = self.as_posix()
            if spelling == "/sys/fs/cgroup/hermes.service/memory.high":
                return "6979321856\n"
            if spelling == "/sys/fs/cgroup/hermes.service/memory.stat":
