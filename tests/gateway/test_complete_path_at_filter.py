...
def _fake_remote_backend(monkeypatch, remote_root: Path) -> list:
    ...
    # The recorded command is a POSIX shell line (`sh -c ...`); execute it with the local POSIX
    # shell instead of shell=True, which on Windows would hand the single-quoted script to cmd.exe.
    posix_sh = _shutil.which("bash") or _shutil.which("sh") or "/bin/sh"
    calls = []
    def fake_terminal_tool(command, **kwargs):
        calls.append((command, kwargs.get("task_id")))
        proc = subprocess.run(
            [posix_sh, "-c", command], cwd=remote_root, capture_output=True, text=True,
