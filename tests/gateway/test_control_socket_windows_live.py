...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    # The state file is only a fallback *claim*: collect_fleet_versions classifies its
    # live PID as current/stale only when the home's identity resolver verifies that
    # PID as the gateway — a live foreign writer stays an ``unknown`` row with no
    # self-reported sha (#110420). This process is not a gateway, so verify it
    # explicitly, exactly like the sibling tests/gateway/test_control_socket.py
    # state-file test (upstream 1348295036).
    monkeypatch.setattr(
        "gateway.status.live_gateway_pid_for_home", lambda _home: os.getpid()
    )
    fleet = ur.collect_fleet_versions()
    assert len(fleet) == 1, fleet
