...
def test_notify_sends_real_unix_datagram(tmp_path, monkeypatch):
    # AF_UNIX path limits are much shorter than a pytest temp path on macOS.
    address = str(Path("/tmp") / f"hermes-notify-{os.getpid()}-{uuid.uuid4().hex[:8]}.sock")
...
@pytest.mark.skipif(
    ...
    reason="systemd abstract sockets are only available on Linux",
)
