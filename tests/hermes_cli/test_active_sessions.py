...
def test_unknown_sibling_liveness_only_fences_its_own_session(tmp_path, monkeypatch):
    ...
    # pid 1 exists on POSIX but is no process on Windows (no init): pin the existence
    # probe for this pid so the scenario stays "owner pid exists, start time unreadable".
    from gateway.status import _pid_exists as _real_pid_exists
    monkeypatch.setattr(
        "gateway.status._pid_exists",
        lambda pid: True if int(pid) == 1 else _real_pid_exists(pid),
    )
