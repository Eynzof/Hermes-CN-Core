...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        hermes_home = tmp_path / ".hermes"
        if os.name == "nt":
            # Windows' default Hermes root is %LOCALAPPDATA%\hermes, not ~/.hermes: point
            # both at tmp_path so the restore really lands in the *default* home (and never
            # at the developer's real install, which the non-default-home guard protects).
...
class TestImportAtomicWrites:
    ...
    @pytest.mark.skipif(os.name != "posix", reason=(
        "POSIX permission bits: Windows st_mode is synthesized (0o666 for any writable file), "
        "so neither the target's 0o644 nor the staged file's mode is observable there"))
...
class TestSafeCopyDb:
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        # Unbounded, advancing clock: ``backup_mod.time`` IS the global ``time`` module, so
        # pytest's own timing consumes ticks too, and a fixed-length ``iter`` raised
        # StopIteration mid-teardown once the product had eaten all three. 0.6s steps still
        # cross the 1.0s deadline on the second SQLITE_BUSY progress.
        clock = itertools.count(100.0, 0.6)
