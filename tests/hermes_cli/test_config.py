...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unreadable_existing_config(self, tmp_path):
        ...
        with patch("builtins.open", side_effect=self._deny_config_reads(config_path)):
            # Upstream reworded this refusal to the plain-language "Your settings file
            # (...) cannot be read, so this change was not saved." (the same rework updated
            # test_save_config_refuses_to_overwrite_unreadable_existing_config); the
            # fail-closed contract itself — refuse and leave the file untouched — is unchanged.
            with pytest.raises(RuntimeError, match="this change was not saved"):
...
class TestEnvWriteDenylist:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # POSIX case: lowercase exec names are different, inert variables. On
            # Windows env names are case-insensitive, so the writer refuses the
            # lowercase spelling there (test_windows_policy_denies_mixed_case_exec_names).
            pytest.param(
                "git_config_parameters",
                marks=pytest.mark.skipif(
                    sys.platform == "win32",
                    reason="POSIX env names are case-sensitive; Windows denies this spelling",
                ),
            ),
            pytest.param(
                "ld_preload",
