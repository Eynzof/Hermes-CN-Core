...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason=("Windows: utils.file_signature degrades to mtime+size there (st_ino is 0 and st_ctime_ns is the creation time), so a cp -p / rsync -t style replacement is not detectable"),
)
