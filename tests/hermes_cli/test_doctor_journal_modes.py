...
class TestUnreadableReason:
    def test_missing_file_keeps_the_os_error_text(self, tmp_path):
        ...
        reason = doctor_platform._unreadable_reason(missing)
        # The raw OS error text is preserved; compare against what THIS platform raises for the
        # path instead of POSIX's strerror wording (Windows reports "WinError 2", localized).
        try:
            missing.stat()
        except OSError as exc:
            assert str(exc) in reason
        else:  # pragma: no cover - the path must not exist
            raise AssertionError("expected the missing file to raise")
...
class TestReportDatabaseJournalModes:
    ...
    def test_lists_every_managed_database(self, tmp_path, capsys):
        ...
        # Native separators: `relative_to` renders `\` on Windows, `/` on POSIX.
        board_db_rel = os.path.join("kanban", "boards", "myboard", "kanban.db")
        assert f"{board_db_rel} is in WAL mode" in out
...
class TestConfiguredDeleteNeverApplied:
    ...
    def test_partial_holder_scan_is_never_an_all_clear(self, tmp_path, capsys, monkeypatch):
        ...
        out = capsys.readouterr().out
        assert "cannot prove the database is quiet" in out
        # The reason text is platform-specific: POSIX surfaces the scan's own detail, Windows
        # reports the holder scan as unavailable outright (doctor_platform._report_database_holders).
        assert ("open-file scan unavailable" in out
                or "holder scan is unavailable on Windows" in out)
