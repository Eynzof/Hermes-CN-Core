...
def _point_home(monkeypatch, home):
    r"""Point ``os.path.expanduser("~")`` at *home* on every platform.

    ``ntpath.expanduser`` consults ``USERPROFILE`` before ``HOME``, so a test that
    only sets ``HOME`` still reads the developer's real profile on Windows and the
    ~/.copilot / ~/.config/github-copilot stores are never found.
    """
...
def test_auth_verified_false_without_evidence(tmp_path, monkeypatch, _clean_copilot_env):
    _point_home(monkeypatch, tmp_path)  # no ~/.config/github-copilot
...
def test_explicit_filter_drops_unverified_external_process_row(tmp_path, monkeypatch, _clean_copilot_env):
    ...
    _point_home(monkeypatch, tmp_path)  # no credential stores
...
def test_catalog_key_empty_when_cli_store_absent(tmp_path, monkeypatch, _clean_copilot_env):
    ...
    _point_home(monkeypatch, tmp_path)  # no ~/.copilot at all
