...
_STUBS = ("acme-cli", "copilot", "custom-acme")


def _stub(bindir, name):
    """The path ``shutil.which`` reports for stub *name* on this host.

    Windows resolves through ``PATHEXT``, so the same stub is exposed there as a
    ``.cmd`` twin and comes back with that suffix — the product resolves with
    ``shutil.which`` too, so the expectation has to follow the host.
    """
    import shutil

    return shutil.which(name, path=f"{bindir}{os.pathsep}{os.environ.get('PATH', '')}") or str(bindir / name)
...
def fake_cli(tmp_path, monkeypatch):
    ...
    for name in _STUBS:
        ...
        if os.name == "nt":
            # Windows resolves even an explicit path through PATHEXT, so every
            # stub needs the .cmd twin (see _stub).
            (bindir / f"{name}.cmd").write_text("@echo off\r\nexit /b 0\r\n", encoding="utf-8")
...
def test_an_out_of_tree_external_process_provider_resolves_end_to_end(fake_cli, monkeypatch):
    ...
    assert (creds["command"], creds["args"], creds["api_key"]) == (_stub(fake_cli, "acme-cli"), ["--acp"], "acme-acp")

    monkeypatch.setenv("ACME_CLI_PATH", str(fake_cli / "custom-acme"))
    monkeypatch.setenv("ACME_ACP_ARGS", "--acp=true --verbose")
    creds = resolve_external_process_provider_credentials("acme-acp")
    assert (creds["command"], creds["args"]) == (_stub(fake_cli, "custom-acme"), ["--acp=true", "--verbose"])
...
def test_copilot_acp_launch_details_are_unchanged(fake_cli, monkeypatch):
    ...
    assert creds["command"] == _stub(fake_cli, "copilot")
    ...
    assert resolve_external_process_provider_credentials("copilot-acp")["command"] == _stub(fake_cli, "custom-acme")
