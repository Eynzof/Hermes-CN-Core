...
class TestStopProfileGateway:
    ...
    def test_stop_profile_gateway_keeps_pid_file_when_process_still_running(self, monkeypatch):
        ...
        # Post-#21561: the stop loop sends one SIGTERM via ``os.kill`` then polls liveness via
        # ``gateway.status._pid_exists`` (safe on Windows — bpo-14484). On Windows the stop path
        # drains the marker watcher first and force-terminates a wedged PID instead (upstream's
        # split), so the stop ACTION is instrumented per host; every assertion below is about the
        ...
        if sys.platform == "win32":
            ...
            # Drain reports "did not exit gracefully" -> the force-terminate seatbelt runs with the
            # identity captured above (a real PID always has a start time; the fake one would
            # otherwise be refused by terminate_pid's recycle guard).
            monkeypatch.setattr("hermes_cli.gateway_windows._drain_gateway_pid", lambda pid, timeout: False)
            monkeypatch.setattr(
                "hermes_cli.gateway_windows._force_terminate_known_gateway_pids",
                lambda pids: calls.__setitem__("kill", calls["kill"] + 1),
            )
        monkeypatch.setattr("time.sleep", lambda _: None)
        monkeypatch.setattr(
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    def test_macos_excludes_launchd_pid_from_kill(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        # ``is_windows`` drives a host-dependent hard-stop short-circuit, not part of the
        # simulated platform: fake it too so the macOS path under test is the one that runs.
