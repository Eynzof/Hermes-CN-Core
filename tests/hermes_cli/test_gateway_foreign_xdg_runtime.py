...
# Windows has no os.getuid() and no /run/user/<uid> systemd sockets, so this whole
# module is POSIX-only (FORK_NOTES P-048).
pytestmark = pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX-only: os.getuid + user-systemd /run/user/<uid> socket probes",
)
