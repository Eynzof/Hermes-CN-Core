...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    def test_non_regular_referenced_script_fails_closed(self, monkeypatch, tmp_path):
        ...
        # A non-regular file the guard must refuse to scan: a FIFO on POSIX, the NUL device on
        # Windows (``os.mkfifo`` does not exist there; ``<dir>/NUL`` is still a character device).
...
class TestLifecycleGuardModule:
    ...
    # POSIX-shell referenced-script scanning: the guard tokenizes with shlex(posix=True), which
    # treats `\` as an escape, so an absolute Windows path never resolves to a real file.
    # Same Windows-only limitation the file already records for its other referenced-script tests.
    ...
    # macOS-only: the cloud-placeholder refusal keys on ~/Library/Mobile Documents and
    # ~/Library/CloudStorage FileProvider subtrees.
