...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    class FakeProc:
        ...
        def poll(self):
            # The dispatcher's Windows zombie reaper polls every proc parked in
            # ``_live_worker_procs``; a fake Popen must expose the same surface.
...
@pytest.mark.skipif(sys.platform == "win32", reason="Windows baseline: wait-status (WIFEXITED/WEXITSTATUS) classification is POSIX-only")
