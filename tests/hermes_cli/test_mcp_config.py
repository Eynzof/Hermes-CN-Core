...
def _set_interactive_stdin(monkeypatch, *, is_tty: bool = True) -> None:
    ...
    if os.name == "nt":
        # Windows additionally requires a real console handle (GetConsoleMode) because the CRT
        # reports isatty() for DEVNULL/detached stdin; a MagicMock cannot satisfy that, so stub the
        # console probe itself (same "stdin is interactive" contract this fixture asserts).
