...
def test_buffer_level_shift_space_no_raw_csi():
    ...
    async def _probe(payload: str) -> str:
        ...
        with create_pipe_input() as inp:
            app = Application(
                layout=Layout(HSplit([Window(BufferControl(buf))])),
                input=inp,
                # Force a headless output: the default create_output() needs a
                # real console (Win32Output on Windows) while pytest captures
                # stdout. The assertions below only read the buffer.
                output=DummyOutput(),
            )
