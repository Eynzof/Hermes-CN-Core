...
class TestNoninteractiveGitEnv:
    ...
    def test_safe_directory_reset_still_revokes_wildcard_for_real_git(self, tmp_path):
        ...
        system_config.write_text("[safe]\n\tdirectory = *\n", encoding="utf-8")
        global_config = tmp_path / "gitconfig"
        # Forward slashes: a raw Windows path in a git config file is parsed as escapes
        # ("fatal: bad config line 3"), which silently emptied the safe.directory sequence.
        trusted_config_path = str(trusted).replace("\\", "/")
        global_config.write_text(
            f"[safe]\n\tdirectory = \n\tdirectory = {trusted_config_path}\n", encoding="utf-8"
        )
