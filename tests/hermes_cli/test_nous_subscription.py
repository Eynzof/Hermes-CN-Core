...
def test_has_agent_browser_import_failure_falls_back_to_hermes_managed_node_path(
    ...
    managed_dir.mkdir()
    if os.name == "nt":
        # Windows resolves a bare command name through PATHEXT, so an extension-less
        # sh shim is invisible to shutil.which(); ship the .cmd twin here (the
        # production Windows install writes agent-browser.cmd).
        managed_bin = managed_dir / "agent-browser.cmd"
    ...
    monkeypatch.setattr(
        "hermes_constants.agent_browser_runnable",
        # normcase: shutil.which returns the PATHEXT-cased name (agent-browser.CMD) on Windows.
