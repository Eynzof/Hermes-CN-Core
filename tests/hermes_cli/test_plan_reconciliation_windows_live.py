...
def test_plan_reconciliation_live_windows(tmp_path, monkeypatch):
    ...
    # Real live process standing in for a manual gateway. The argv tail is inert data to the
    # child but is what the identity check (``_record_matches_live_gateway_pid`` ->
    # ``looks_like_gateway_runtime_command_line``) classifies on: a bare sleeper is no longer
    # accepted as a gateway, which is the point of the verification. The test reaps it below.
