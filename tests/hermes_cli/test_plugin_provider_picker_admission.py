...
def test_external_process_plugin_authenticated_flag_tracks_binary_and_catalog_uses_fallback(
    ...
    if os.name == "nt":
        # Windows resolves a bare command name through PATHEXT, so an
        # extension-less sh shim is invisible to shutil.which(); ship the .cmd
        # twin the production Windows binary would have.
        (tmp_path / "acme-acp.cmd").write_text("@echo off\r\nexit /b 0\r\n", encoding="utf-8")
