...
# ``--no-local`` rather than a ``file://`` URL: on Windows ``Path.as_uri()`` yields
    # ``file:///C:/...``, which this git reads as the path ``/C:/...`` (cli.py guards the same
    # quirk for attachments) — and a plain local path would ignore ``--depth``. ``--no-local``
    # forces the git transport on every platform, so the clone is genuinely shallow.
    subprocess.run(["git", "clone", "-q", "--depth", "1", "--no-local", str(origin), str(clone)], check=True)
...
def test_repair_ignores_parent_lines_inside_commit_messages(tmp_path):
    ...
    git(origin, "commit", "--allow-empty", "-qm", "c1")
    clone = tmp_path / "clone"
    subprocess.run(["git", "clone", "-q", "--depth", "2", "--no-local", str(origin), str(clone)], check=True)
...
def test_repair_does_not_mask_unrelated_object_loss(tmp_path):
    ...
    # Git writes loose objects read-only; Windows refuses to unlink those, so clear the bit first.
    loose.chmod(0o644)
    loose.unlink()
    assert gitlock.repair_broken_shallow_boundaries(clone) == 0
    fsck = git(clone, "fsck", "--connectivity-only", check=False)
