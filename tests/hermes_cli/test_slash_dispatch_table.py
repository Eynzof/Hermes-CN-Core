...
OLD_CHAIN_COMMANDS = [
    ...
    # [CN-fork] /swarm is a fork command and had its own branch in the pre-table chain
    # (``elif canonical == "swarm"`` → ``_handle_swarm_command``, now on the CLI commands
    # mixin); upstream's list predates it, so the dispatch table still handles it.
    "swarm",
]
