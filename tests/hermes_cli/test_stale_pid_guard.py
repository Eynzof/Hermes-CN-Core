...
class TestKillStaleDashboardProcesses:
    ...
    def test_stop_only_targets_the_invoking_hermes_home(self, monkeypatch):
        ...
        with mock.patch.object(
            ...
            # The dispatch at ``_kill_stale_dashboard_processes`` picks the platform's killer
            # (``_kill_pids_windows`` on win32, ``_kill_pids_posix`` elsewhere); patch whichever
            # one this host actually calls so the assertion stays about *selection*, not the host.
            dashboard_procs, "_kill_pids_windows" if sys.platform == "win32" else "_kill_pids_posix"
        ) as kill:
