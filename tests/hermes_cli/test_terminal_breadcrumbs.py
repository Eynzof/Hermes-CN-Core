...
def _fake_no_tty(monkeypatch):
    # ``os.ttyname`` is POSIX-only (absent on Windows), so the seam is installed
    # with ``raising=False``: the product reads ``os.ttyname`` at call time, so the
    # injected attribute drives the same branch on every platform and the
    # assertions below stay platform-independent.
    monkeypatch.setattr(
        tb.os, "ttyname", lambda fd: (_ for _ in ()).throw(OSError()), raising=False
    )
