...
def _patch_node_resolution(monkeypatch, main_mod, which=None) -> None:
    """Make node/npm resolution deterministic without touching the real host.

    Stubbing ``shutil.which`` alone is not enough: ``_tui_node_bin`` resolves through
    ``hermes_constants.find_node_executable``, which scans PATH itself on Windows (CreateProcess
    cannot run npm's bare ``.cmd`` shim). Both seams get the same resolver so the argv assertions
    stay platform-independent (and a caller-supplied ``which`` keeps its own guard).
    """
    import hermes_constants

    resolve = which or (lambda name: f"/bin/{name}")
    monkeypatch.setattr(main_mod.shutil, "which", resolve)
    monkeypatch.setattr(hermes_constants, "find_node_executable", resolve)
...
def test_make_tui_argv_skips_build_only_on_termux_when_fresh(
    ...
    _patch_node_resolution(monkeypatch, main_mod)
...
def test_make_tui_argv_exits_with_recovery_hint_when_workspace_unrecoverable(
    ...
    _patch_node_resolution(monkeypatch, main_mod, which=which)
