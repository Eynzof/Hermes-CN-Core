...
def _patch_gateway_discovery():
    ...
    The Windows scope is neutralised as well. ``_pause_windows_gateways_for_update``
    consults the machine-global ``schtasks`` entry (``gateway_windows.is_installed()``
    -> ``Hermes_Gateway``), which is NOT derived from the test's HERMES_HOME: on a
    Windows host that has the gateway autostart installed, an update run plans a
    cold start (``_windows_cold_start_plan`` -> ``cold_start_if_installed``), the
    conftest live-system guard refuses the detached ``gateway run`` spawn, and the
    verify step turns that into ``sys.exit(1)``. The pause/resume phase is a no-op
    here for the same reason it is in ``conftest.py::isolated_update_runtime`` and
    in the sibling update test files — no test in this file asserts on it.
    """
    with patch("hermes_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch.object(hermes_main, "_resume_windows_gateways_after_update", lambda *a, **k: None), \
