...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        # ``_locate_launchd_gateway_service`` probes ``gui/<uid>`` / ``user/<uid>``;
        # os.getuid() is POSIX-only, so the simulated macOS host pins it to the same
        # uid the fake domain above uses (raising=False: the attribute is absent on
        # Windows and the product reads it at call time).
