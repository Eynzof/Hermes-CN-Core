...
def test_configured_features_probe_reads_the_fresh_target_interpreter(tmp_path, monkeypatch):
    ...
    truth rather than the updater's own pre-install import caches. Positive: the child interpreter
    reports the feishu SDK missing → the configured platform is named. Negative: only the child is
    told the dependency is present (nothing is reported) — proving the verdict comes from the fresh
    process. Both preludes ride ``tools.lazy_deps.is_available``, the seam the registry
    ``check_fn`` (``feishu_deps_present``) reads, so the verdict never depends on what this dev
    venv happens to have installed (the CN desktop pre-bakes the feishu extra)."""
    home = tmp_path / "home"
    home.mkdir()
    ...
    child_deps_missing = "import tools.lazy_deps as ld; ld.is_available = lambda *_a, **_k: False\n"
    monkeypatch.setattr(main_install_repair, "_venv_probe",
                        lambda p, s, *a, env=None: probe(p, s, *a, env=env, prelude=child_deps_missing))
