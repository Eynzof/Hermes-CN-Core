...
try:
    # force_venv: the live-venv holder guard (other Hermes processes running from this
    # install's venv) refuses on a dev box whose own gateways are up; it is not what this
    # test exercises, and CI never relies on it — the shim-parent wait is.
    cli_main.cmd_update(SimpleNamespace(post_swap=None, yes=True, gateway=False, force_venv=True))
except Stop:
    pass
