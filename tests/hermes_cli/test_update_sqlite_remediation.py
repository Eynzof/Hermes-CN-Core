...
def test_summary_withholds_success_when_sqlite_remediation_failed(capsys, monkeypatch):
    ...
    # The fix command is the platform's installer one-liner (its sibling test pins the
    # Windows branch); pin this one to POSIX so `install.sh | bash` is asserted on any host.
