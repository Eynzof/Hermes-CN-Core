...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX-only: venv file ownership (``_venv_foreign_owned_paths`` returns [] when "
    "``os.geteuid`` is absent) and the ``venv/bin/hermes`` layout do not exist on Windows",
)
