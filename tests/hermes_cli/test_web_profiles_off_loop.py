...
def test_get_active_profile_runs_off_loop(client, monkeypatch, loop_probe):
    ...
    assert resp.status_code == 200, resp.text
    # [CN-fork] P-008: the fork's GET also carries ``name`` (mirroring the sticky ``active``)
    # because the hermes-agent-cn desktop's ActiveProfileResponse schema requires it;
    # upstream's response shape is ``{active, current}``. Pin the fork's full contract.
    assert resp.json() == {"name": "demo", "active": "demo", "current": "default"}
