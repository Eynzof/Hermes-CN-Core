...
@pytest.mark.asyncio
async def test_desktop_monitor_and_continuity_roundtrip(isolated_profiles):
    from hermes_cli import web_server

    job = await web_server.create_cron_job(web_server.CronJobCreate(
        name="持续跟进", prompt="报告更新", schedule="every 1h",
        context_from=["self"], monitor_url="https://example.com/updates",
    ), profile="worker_alpha")
    stored = web_server._call_cron_for_profile("worker_alpha", "get_job", job["id"])
    assert stored["context_from"] == ["self"]
    assert stored["monitor_url"] == "https://example.com/updates"
    updated = await web_server.update_cron_job(job["id"], web_server.CronJobUpdate(
        updates={"prompt": "新的任务要求", "context_from": [], "monitor_url": None},
    ), profile="worker_alpha")
    assert updated["prompt"] == "新的任务要求"
    assert not updated["context_from"]
    assert updated["monitor_url"] is None
    assert not web_server._call_cron_for_profile("default", "get_job", job["id"])
