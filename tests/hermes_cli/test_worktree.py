...
class TestPrMergedEscapeHatch:
    ...
    def _stub_gh(tmp_path, monkeypatch, stdout='[{"number": 1}]', exit_code=0):
        """Stub the ``gh`` CLI for ``_worktree_branch_pr_merged``.

        POSIX: a ``#!/bin/sh`` shim on PATH is enough. Windows cannot execute an
        extension-less (or ``.bat``) PATH shim through ``subprocess`` — the real ``gh``
        on the host answers instead (and is usually not authenticated) — so there the
        ``gh`` argv is answered directly and every other argv (the ``git`` probes) is
        delegated to the real runner.
        """
