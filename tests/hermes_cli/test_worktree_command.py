...
def test_list_shows_worktrees(repo):
    ...
    # ``git worktree list`` prints its paths with forward slashes even on Windows, so the
    # listing is compared in that spelling (the check is still "this repo is listed").
    assert repo.as_posix() in out.replace(chr(92), "/")
