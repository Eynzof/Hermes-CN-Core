...
class TestRuntimeFtsRebuild:
    ...
    def test_live_search_never_rebuilds_in_place_across_repeats(self, db, tmp_path, monkeypatch):
        """The fork's one-shot live in-place rebuild (and its
        ``_fts_runtime_rebuild_attempted`` flag) was replaced upstream: a corrupt index must never
        be rebuilt from a live path — ``append_message``/``search_messages`` fail open onto the
        canonical rows, mark the index stale, and the rebuild is deferred to the next ``SessionDB``
        open (see ``test_corruption_fails_open_and_rebuilds_on_reopen``). Every repeat keeps reading
        the canonical rows instead of starting an unbounded index scan."""
        if not db._fts_enabled:
            pytest.skip("FTS5 unavailable in this build")
        db_path = tmp_path / "state.db"
        db.create_session("s1", source="test")
        ...
        monkeypatch.setattr(
            db,
            "rebuild_fts",
            lambda: pytest.fail("a live search must not rebuild the full FTS index"),
        )
        assert not hasattr(db, "_fts_runtime_rebuild_attempted")

        for _ in range(3):
            results = db.search_messages("needle")
            assert results  # non-empty: served from the canonical message rows
            assert any("needle" in (r.get("snippet") or "") for r in results)
