...
class TestPinTransition:
    ...
    def test_cache_busting_detects_same_size_in_place_rewrite(self, tmp_path, monkeypatch):
        ...
        # The gateway now reads Honcho identity through the provider's ``identity_signature()`` and files
        # it under "memory.<signature key>" (peer_name -> user_identity); upstream moved these keys out of
        # the honcho.* namespace. What the test pins is unchanged: a same-size rewrite with a pinned mtime
        # must still be seen.
        assert first["memory.user_identity"] == "Igor"
        assert second["memory.user_identity"] == "Oleg"
