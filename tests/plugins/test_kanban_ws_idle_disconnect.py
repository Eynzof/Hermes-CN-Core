...
async def test_stream_events_reuses_connection_and_closes_after_disconnect(
    ...
    # The tailer opens read-side with the existing-only helper (fork 09f199097c): these tests
    # reach an existing board through _connect and must not stub kbc.connect().
    monkeypatch.setattr(mod.kanban_db, "connect_existing", _connect)
