...
def _run_install_node(tmp_path: Path, node_body: str) -> tuple[int, str, str, list[str]]:
    ...
    env.update(
        {
            "FIXTURE": str(fixture),
            "APT_LOG": str(apt_log),
            # os.pathsep, not ":": the driver runs under the host's bash, and on
            # Windows a hard-coded ":" joins the whole inherited PATH into one
            # bogus entry, which hides every host tool the installer probes
            # (e.g. `command -v xz` in install_node_line) and forces the
            # .tar.gz fallback the stubs here do not model.
    ...
    driver.write_text(
        ...
        f"HERMES_HOME={home}\n"
        "HAS_NODE=maybe\n"
        # install.sh's own default (line ~71); the fork's --isolated-dir layout
        # switch reads it under `set -u` when the adopted tree is linked.
