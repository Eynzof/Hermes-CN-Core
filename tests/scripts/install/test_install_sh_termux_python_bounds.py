...
def _write_executable(path: Path, content: str) -> Path:
    # LF endings: shebang scripts run under the host's bash, which treats a
    # trailing CR as part of the line ("bad interpreter").
...
def _write_fake_python(bin_dir: Path, name: str, version: str) -> Path:
    """A fake interpreter that reports `version`.

    A POSIX shell stub, not ``#!<sys.executable>``: install.sh and
    setup-hermes.sh run under the host's bash, and on Windows a native
    interpreter cannot open the script path bash hands it (MSYS passes
    ``/tmp/...`` on as ``C:\\tmp\\...``), which made every probe report an empty
    version. The ``-c`` payload is still executed by the real interpreter with
    sys.version / sys.version_info overridden, so the installers' range probe
    keeps its real semantics.
    """
    prologue = (
        "import sys\n"
        f"sys.version = {version!r} + ' (fake)'\n"
        "sys.version_info = "
        f"{tuple(int(part) for part in version.split('.')[:3]) + ('final', 0)!r}\n"
...
if [ "${{1:-}}" = "-c" ] && [ "$#" -ge 2 ]; then
    exec {shlex.quote(sys.executable)} -c {shlex.quote(prologue)} "$2"
fi
if [ "${{1:-}}" = "-m" ] && [ "${{2:-}}" = "venv" ]; then
    target="${{3:-venv}}"
    mkdir -p "$target/bin"
    cat > "$target/bin/python" <<'EOS'
#!/bin/sh
if [ "$1" = "-m" ] && [ "$2" = "pip" ]; then
    exit 0
fi
...
def test_install_stage_provisions_supported_python_from_tur(tmp_path: Path) -> None:
    ...
    # The staged dir is resolved relative to the stub's own (shell-spelled)
    # location: a native Windows path baked into the script is not usable from
    # MSYS bash.
    ...
    _write_executable(
        ...
        "#!/bin/sh\n"
        "for arg in \"$@\"; do\n"
        "    if [ \"$arg\" = 'python3.13' ]; then\n"
        "        cp \"$(dirname \"$0\")/../staged/python3.13\" "
        "\"$(dirname \"$0\")/python3.13\"\n"
        "    fi\n"
        "done\n"
        "exit 0\n",
    )
