...
def _bash_path(path: Path) -> str:
    """`path` spelled the way the local ``bash`` wants it.

    Git-for-Windows bash wants ``/c/...`` (and maps ``%TEMP%`` to ``/tmp``),
    the WSL launcher wants ``/mnt/c/...``. Ask the shell itself instead of
    hard-coding the WSL form, which made git-bash report "No such file or
