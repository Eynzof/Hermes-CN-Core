...
def test_add_contributor_refuses_a_case_collision(tmp_path, monkeypatch):
    ...
    # Assert on the directory listing, not on `(d / "agent@example-host.local").exists()`:
    # on the very case-insensitive filesystems this guard exists to protect (Windows,
    # default macOS) that path resolves to the pre-existing `agent@Example-Host.local`,
    # so an exists() probe reads True on a correctly-refused write. The listing (and the
    # untouched content) is the same contract on every platform: the refusal wrote
    # nothing new and did not clobber the existing mapping.
