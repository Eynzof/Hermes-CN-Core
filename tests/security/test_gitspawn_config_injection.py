...
def _make_malicious_repo(tmp: Path) -> tuple[Path, Path]:
    ...
    # Git parses double-quoted config values with C-style escapes: a Windows tmp_path (C:\Users\...)
    # would make .git/config unparseable ("bad config line N"), so no sink could ever fire and the
    # hardening tests below would pass trivially. Embed POSIX-style paths instead — sh/touch accept
    # them on Windows too.
    marker_cfg, hooks_cfg = marker.as_posix(), hooks.as_posix()
    hook.write_text(f"#!/bin/sh\ntouch {marker_cfg}.hook\n")
    hook.chmod(0o755)
    with (repo / ".git" / "config").open("a") as f:
        f.write(f'[core]\n\tfsmonitor = "touch {marker_cfg}.fsmonitor"\n\thooksPath = {hooks_cfg}\n')
        f.write(f'[diff "evil"]\n\tcommand = "touch {marker_cfg}.extdiff"\n')
        f.write(f'\ttextconv = "sh -c \'touch {marker_cfg}.textconv; cat\'"\n')
