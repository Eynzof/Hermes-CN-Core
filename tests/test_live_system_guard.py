...
def test_gateway_start_inside_a_container_exec_is_not_blocked():
    ...
    # The stub must be a runnable image on the host running this test. A shebang script
    # needs the POSIX exec bit; Windows has no exec bit and dispatches by extension, so
    # the stub is a .cmd there — which the guard normalizes back to the runtime name
    # (tests/conftest.py::_first_token_basename strips .cmd/.exe/.bat/.com). The asserted
    # contract is identical on both hosts: the container argv is exempt from the guard.
    if os.name == "nt":
        stub = os.path.join(stub_dir, "docker.cmd")
