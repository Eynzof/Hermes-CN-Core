...
class TestApprovalTimeoutOverflowClamp:
    ...
    def test_deadline_import_failure_fails_closed(self, monkeypatch):
        ...
        # One year on POSIX; the Windows locked-wait ceiling there, because
        # Lock.acquire rejects >4_294_967 s (CPython PY_TIMEOUT_MAX) — the
        # fallback mirrors agent.deadline.MAX_SAFE_TIMEOUT_S per platform.
        assert value == (4_294_000 if sys.platform == "win32" else 365 * 24 * 3600)
