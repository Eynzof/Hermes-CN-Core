...
def test_waiter_is_a_runner_entrypoint_the_approval_gate_lets_through(root):
    ...
    # waiter_command forward-slashes its argv on native Windows (Git Bash runs it);
    # as_posix() is the same path on POSIX and the rewritten form on Windows.
    assert parts[3] == (bot_relay.relay_root(root) / bot_relay.REPLIES_DIR
                        / f"{'b' * 32}.json").as_posix()
