...
def test_local_delivery_resolves_sibling_hermes(tmp_path, monkeypatch):
    # A real venv keeps the entrypoint in Scripts/hermes.exe on Windows and bin/hermes
    # on POSIX; _hermes_cli() looks for exactly that sibling name.
    ...
    sibling.touch()
    sibling.chmod(0o755)
    monkeypatch.setattr("sys.executable", str(bin_dir / ("python.exe" if sys.platform == "win32" else "python")))
