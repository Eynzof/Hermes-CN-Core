...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason=("simulates a POSIX host by patching os.name; on Windows that makes pathlib build "
            "PosixPath (UnsupportedOperation) for every first-time import and breaks pytest's "
            "own reporting"),
)
...
class TestLegacyKillProcessTree:
    ...
    @pytest.mark.skipif(
        ...
        reason="POSIX process-group API (os.getpgid/os.killpg) absent on Windows",
    )
