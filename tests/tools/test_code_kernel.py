...
def _live_kernel_processes() -> int:
    """Live ``hermes_kernel_runner.py`` children of this test process.

    Portable replacement for ``pgrep -fc -P <pid> hermes_kernel_runner`` (procps is
    POSIX-only). Direct children only, like ``pgrep -P``: on Windows the kernel
    process re-launches itself, so a recursive walk would count one kernel twice.
    The runner script name is unique to a session kernel (tools/code_kernel.py
    writes ``hermes_kernel_runner.py`` into the kernel tmpdir).
    """
    import psutil
    ...
    for child in psutil.Process(os.getpid()).children(recursive=False):
        try:
            ...
            continue
        if any("hermes_kernel_runner" in part for part in cmdline):
            alive += 1
    return alive
