    ...
    def test_child_dedicated_db_follows_parents_db_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIsInstance(child_db, SessionDB)
                self.assertIsNot(child_db, parent_db)
                # Same FILE, not the same string: where the host's TEMP is an 8.3 short
                # path (e.g. C:/Users/MAXWEL~1/...), the child's handle spells the temp
                # dir the long way while the parent keeps what tempfile handed out.
                self.assertEqual(
