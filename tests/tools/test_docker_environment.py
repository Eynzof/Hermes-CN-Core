...
def test_persistent_bind_mounts_survive_a_session_key_task_id(monkeypatch, tmp_path):
    ...
    for spec in mounts:
        ...
        # A Windows controller's sandbox dir always carries a drive prefix
        # ('C:\\...\\home:/root'); docker parses that one colon as part of the source.
        # ONLY the drive colon is legitimate — any other colon means the raw session
        # key leaked into the path, which is the 'too many colons' failure this guards.
        _drive, source_rest = os.path.splitdrive(source)
        assert ":" not in source_rest, (
            f"bind source still contains a colon, docker run would fail with "
            f"'too many colons': {spec}"
        )
        # Docker splits on ':' — a sane spec has exactly source:target, so the spec
        # carries one separator colon plus whatever the (drive-only) source has.
        assert spec.count(":") == 1 + source.count(":"), f"spec is not a two-field bind: {spec}"
