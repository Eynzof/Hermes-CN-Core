...
class TestStalenessCheck(unittest.TestCase):
    ...
    def test_write_file_refuses_before_mutation_when_modified_externally(self):
        ...
        self.assertNotIn("error", orjson.loads(read_file_tool(self._tmpfile, task_id="t1")))
        _modify_externally(self._tmpfile, "someone else changed this\n")

        refused = orjson.loads(write_file_tool(self._tmpfile, "new content\n", task_id="t1"))
        ...
        self.assertNotIn("error", orjson.loads(read_file_tool(self._tmpfile, task_id="t1")))
        written = orjson.loads(write_file_tool(self._tmpfile, "merged\n", task_id="t1"))
    ...
    def test_write_file_requires_full_unredacted_read_of_existing_file(self):
        ...
        refused = orjson.loads(write_file_tool(self._tmpfile, "x\n", task_id="t2"))
        self.assertTrue(refused.get("stale_write_blocked"), refused)
        self.assertIn("has not seen its full current content", refused["error"])

        patched = orjson.loads(patch_tool(mode="replace", path=self._tmpfile,
                                        old_string="original", new_string="patched", task_id="t2"))
        self.assertNotIn("error", patched)
        self.assertTrue(orjson.loads(write_file_tool(self._tmpfile, "x\n", task_id="t2")).get("stale_write_blocked"))

        with open(self._tmpfile, "w") as f:
            f.write("one\ntwo\nthree\n")
        self.assertNotIn("error", orjson.loads(read_file_tool(self._tmpfile, offset=1, limit=1, task_id="t2")))
        self.assertTrue(orjson.loads(write_file_tool(self._tmpfile, "x\n", task_id="t2")).get("stale_write_blocked"))
        ...
        with patch("agent.redact._REDACT_ENABLED", True):
            read = orjson.loads(read_file_tool(self._tmpfile, task_id="t2"))
            self.assertNotIn(secret, read["content"])
            refused = orjson.loads(write_file_tool(self._tmpfile, "token=«redacted:ghp_…»\n", task_id="t2"))
        self.assertTrue(refused.get("stale_write_blocked"), refused)
        with open(self._tmpfile) as f:
            self.assertEqual(f.read(), f"token={secret}\n")

        new_path = os.path.join(self._tmpdir, "brand_new.txt")
        self.assertNotIn("error", orjson.loads(write_file_tool(new_path, "one\n", task_id="t2")))
        self.assertNotIn("error", orjson.loads(write_file_tool(new_path, "two\n", task_id="t2")))
    ...
    def test_paged_read_of_large_file_is_a_full_baseline_that_survives_compaction(self):
        ...
        first = orjson.loads(read_file_tool(self._tmpfile, task_id="t3"))
        self.assertTrue(first.get("truncated"), first)
        self.assertTrue(orjson.loads(write_file_tool(self._tmpfile, "x\n", task_id="t3")).get("stale_write_blocked"))

        self.assertNotIn("error", orjson.loads(read_file_tool(self._tmpfile, offset=2001, task_id="t3")))
        reset_file_dedup("t3")
        written = orjson.loads(write_file_tool(self._tmpfile, "merged\n", task_id="t3"))
        ...
        with open(self._tmpfile, "w") as f:
            f.write("".join(f"line {i}\n" for i in range(1, 2501)))
        orjson.loads(read_file_tool(self._tmpfile, task_id="t3"))
        _modify_externally(self._tmpfile, "".join(f"other {i}\n" for i in range(1, 2501)))
        orjson.loads(read_file_tool(self._tmpfile, offset=2001, task_id="t3"))
        refused = orjson.loads(write_file_tool(self._tmpfile, "x\n", task_id="t3"))
        self.assertTrue(refused.get("stale_write_blocked"), refused)
    ...
    def test_warning_when_file_modified_externally(self, mock_ops):
        """Read, then external modify, then write — the stale write is refused before mutation.

        Upstream (tools/file_tools_write_guards.py) deliberately replaced the fork's
        warn-and-overwrite contract with a hard refusal; the assertion below pins that
        contract for the mocked-ops path as well as the real-ops path pinned by
        test_write_file_refuses_before_mutation_when_modified_externally
        (upstream/main renamed the old ``_warning`` test away).
        """
        mock_ops.return_value = _make_fake_ops("original content\n", 18)
        read_file_tool(self._tmpfile, task_id="t1")
