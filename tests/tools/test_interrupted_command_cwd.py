...
class TestInterruptDoesNotStealAnotherSessionsCwd:
    ...
    def test_next_command_still_runs_in_my_directory(self, env, tmp_path):
        ...
        # Bash reports the MSYS form (/c/Users/...) on Windows; normalize it the
        # way the local backend does before comparing paths. No-op on POSIX.
        observed = _msys_to_windows_path(result["output"].strip())
        assert os.path.realpath(observed) == os.path.realpath(str(mine))
