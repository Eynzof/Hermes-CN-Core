...
class TestWriteFileCRLFPreservation:
    def test_overwrite_crlf_file_with_lf_content_preserves_crlf(
        ...
        assert "error" not in orjson.loads(read_file_tool(str(target), task_id="crlf_write_1"))
    ...
    def test_overwrite_lf_file_stays_lf(self, hermes_home, tmp_path):
        ...
        assert "error" not in orjson.loads(read_file_tool(str(target), task_id="crlf_write_3"))
