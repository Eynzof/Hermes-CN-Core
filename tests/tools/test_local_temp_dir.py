...
# POSIX-only assertions: the env-var precedence chain matches paths with
# ``startswith("/")`` and compares against the *POSIX* form of ``tmp_path``,
# while the Windows branch of ``get_temp_dir`` deliberately returns the
# ``HERMES_HOME/cache/terminal`` dir with forward slashes (see
# tools/environments/local.py).  Same behaviour before and after the sync.
_WIN32_REASON = (
    "POSIX temp-dir precedence: Windows always uses HERMES_HOME/cache/terminal"
)
