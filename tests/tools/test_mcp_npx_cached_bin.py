...
def _cache(tmp_path, *, package, deps=None, bin_field, make_bin=True, entry="abc123"):
    ...
    # Windows picks the cache launcher by extension (the extensionless sh script cannot
    # be spawned there), so the fixture lays down the platform's real name.
    suffix = ".cmd" if sys.platform == "win32" else ""
    target = bindir / ((os.path.basename(package) if isinstance(bin_field, str) else name) + suffix)
    if make_bin:
...
def test_windows_selects_launchers_never_the_sh_script():
    ...
    # Only the extension selection/order is under test: os.path.join uses the HOST
    # separator, so normalise it away before comparing.
    assert [c.replace("\\", "/") for c in win] == ["/c/bin/mcp-linear.cmd", "/c/bin/mcp-linear.exe"]
    assert not any(c.endswith("mcp-linear") for c in win), "sh script must not be a candidate"
    posix = _npx_bin_candidates("/bin", "mcp-linear", windows=False)
    assert [c.replace("\\", "/") for c in posix] == ["/bin/mcp-linear"]
