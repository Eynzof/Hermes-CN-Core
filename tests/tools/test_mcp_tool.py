...
class TestMCPStatus:
    def test_status_distinguishes_configured_connecting_failed_and_disabled(
        ...
        import tools.mcp_tool as mcp_tool
        from tools import mcp_tool_config as _mcp_config, mcp_tool_discovery as _mcp_discovery
    ...
    def _run_with_future(mcp_mod, future):
        ...
        with patch.object(mcp_mod, "_mcp_loop", loop):
            with patch("agent.async_utils.safe_schedule_threadsafe", side_effect=_schedule):
                ...
                return _loop._run_on_mcp_loop(_unused_call(), timeout=1)
...
class TestHTTPConfig:
    ...
    def test_http_unavailable_raises(self):
        ...
        async def _test():
            # mcp >= 2.0 ships its own httpx (httpx2) and every object crossing the SDK
            # boundary must come from it, so the owned client is built from ``sdk_httpx()``:
            # patch that AsyncClient, not the global ``httpx`` module.
    ...
    def test_http_seeds_initial_protocol_header(self):
        ...
        async def _run(config, *, new_http):
            captured.clear()
        # mcp 2.0 dropped the legacy ``streamablehttp_client`` alias, so tools.mcp_tool has no
        # such attribute to patch on this SDK build; create=True keeps the legacy-path patch.
            with patch("tools.mcp_tool._MCP_HTTP_AVAILABLE", True), \
                 ...
                 patch("tools.mcp_tool.streamable_http_client", return_value=DummyTransportCtx()), \
                   patch("tools.mcp_tool.streamablehttp_client", create=True, side_effect=lambda url, **kwargs: DummyLegacyTransportCtx(**kwargs)), \
        ...
        # The seeded header carries the HANDSHAKE version (deliberately not the newest
        # protocol revision): a newest-revision header routes the handshake-era
        # ``initialize()`` onto the envelope ladder, which rejects it.
        assert captured["headers"]["mcp-protocol-version"] == LATEST_HANDSHAKE_VERSION
        ...
        # Same seeded handshake-version header on the legacy (mcp < 1.24) transport.
        assert captured["legacy_headers"]["mcp-protocol-version"] == LATEST_HANDSHAKE_VERSION
...
class TestUtilityHandlers:
    ...
    def test_list_prompts_success(self):
        from tools.mcp_tool_handlers import _make_list_prompts_handler
        from tools.mcp_tool import _servers
...
from tools import mcp_tool_config as _mcp_config, mcp_tool_discovery as _mcp_discovery, mcp_tool_loop as _mcp_loop
...
class TestSamplingCallbackText:
    ...
    def test_length_stop_reason(self):
        ...
        assert isinstance(result, CreateMessageResult)
        # mcp 2.0 renamed CreateMessageResult.stopReason -> .stop_reason (camelCase survives
        # only as the serialization alias); the file's _stop_reason helper reads both.
        assert _stop_reason(result) == "maxTokens"
