...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="needs st_ctime/inode to change on an in-place rewrite; on Windows "
    "st_ctime is the creation time and st_ino is stable, so file_signature "
    "degrades to mtime+size there (documented in utils.file_signature)",
)
