...
# P-049: prefer the managed ripgrep copy via dep_ensure; on Windows the grep
# shell backend is unavailable (PowerShell-only), so only rg is exercised there.
from hermes_cli.dep_ensure import _find_rg

_METHODS = ["_search_with_grep"] if sys.platform != "win32" else []
if _find_rg():
