...
class TestBlocksMutationsInSourceRepo:
    ...
    @pytest.mark.skipif(
        sys.platform == "win32",
        reason="Windows baseline: POSIX shell parsing mangles backslash paths",
    )
    ...
    @pytest.mark.skipif(
        sys.platform == "win32",
        reason="Windows baseline: os.path.expanduser reads USERPROFILE not HOME",
    )
...
class TestBlockMessageGuidance:
    ...
    def test_scratch_hint_honors_hermes_home(self, repo, monkeypatch):
        ...
        hit, msg = _detect("git rebase origin/main", repo, repo)
        assert hit is True
        # The hint must derive the clone dir from HERMES_HOME (not ~/.hermes);
        # built the same way the guard builds it so the check is OS-agnostic.
        assert str(Path("/custom/hermes-home").expanduser() / "scratch") in msg
