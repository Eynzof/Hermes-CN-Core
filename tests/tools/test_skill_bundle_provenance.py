...
def served_repo(tmp_path, monkeypatch):
    ...
    repo.mkdir()
    (repo / "SKILL.md").write_text(SKILL_MD, newline="\n")
    for rel, content in {
        ...
        if isinstance(content, bytes):
            ...
            # newline="\n": the byte-exact assertions below compare the FETCHED
            # bytes with the on-disk ones, so the fixture must not let text mode
            # translate \n to \r\n on Windows.
...
def test_same_dir_linked_siblings_are_fetched(served_repo, monkeypatch):
    ...
    (repo / "SKILL.md").write_text(
        SKILL_MD + "See [the format](./CONTEXT-FORMAT.md) and [deepening](DEEPENING.md).\n",
        newline="\n",
    )
    monkeypatch.setattr("tools.skills_hub.is_safe_url", lambda _url: True)
    monkeypatch.setattr("tools.skills_hub.check_website_access", lambda _url: None)
