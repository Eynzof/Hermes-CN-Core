...
def test_backup_fill_does_not_clobber_disk_hash(ledger_env):
    ...
    # newline="\n": text mode would write \r\n on Windows, and the assertion
    # below hashes the un-translated ``live`` string.
...
def test_backup_fill_ignores_tar_path_traversal(ledger_env):
    ...
    # The legitimate missing file WAS filled — proof the fill is live. as_posix():
    # the ledger records native paths, so a "/"-suffixed match needs normalizing
    # on Windows.
