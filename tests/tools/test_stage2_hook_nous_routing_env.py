...
# The subject is ``docker/stage2-hook.sh`` — the s6-overlay init hook of the
# Linux container image. Exercising its extracted sections needs a POSIX host:
# the assembled snippet is ~9 KB and this box's ``sh -c`` (Git Bash) truncates
# the argument at ~8.2 KB ("unexpected EOF while looking for matching"), and the
# tests guard on ``os.geteuid()``, which does not exist on Windows.
pytestmark = pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX-only: Linux container stage2 hook (sh -c, os.geteuid)",
)
