...
class TestForegroundTimeoutCap:
    ...
    def test_foreground_timeout_above_max_is_promoted_to_tracked_background(self, tmp_path, monkeypatch):
        ...
        marker = tmp_path / "ran"
        # Shell-safe marker path: a bare Windows path reaches bash with its
        # backslashes eaten (and the redirect lands elsewhere), so quote it and
        # use forward slashes on Windows — POSIX is unchanged.
        marker_arg = shlex.quote(str(marker).replace("\\", "/") if os.name == "nt" else str(marker))
        with patch("tools.terminal_tool._get_env_config", return_value=_make_env_config(cwd=str(tmp_path))), \
             patch("tools.terminal_tool._start_cleanup_thread"), \
             patch("tools.terminal_tool._check_all_guards", return_value={"approved": True}):
            result = json.loads(terminal_tool(command=f"echo x >> {marker_arg}", timeout=9999))
    ...
    def test_foreground_allows_help_variant_for_server_command(self):
        ...
        with patch("tools.terminal_tool._get_env_config", return_value=_make_env_config()), \
            ...
            # installed (P-049): disable rtk detection so the guard's raw-command
            # passthrough is hermetic on hosts that happen to have rtk.
