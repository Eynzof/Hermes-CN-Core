...
def test_foreground_tool_call_forwards_live_output_to_gateway_sink(monkeypatch):
    ...
    class FakeEnv:
        ...
        def execute(self, command, **kwargs):
            ...
            # A real backend reports the cwd it OBSERVED for this command
            # (base._extract_cwd_from_output sets cwd_observed/cwd from the
            # marker); the tool echoes it only on that signal.
            return {"output": "first chunk\n", "returncode": 0,
                    "cwd_observed": True, "cwd": self.cwd}
...
def test_foreground_command_prefers_recorded_session_cwd_over_init_time_cwd(monkeypatch):
    ...
    result = orjson.loads(terminal_tool.terminal_tool(command="pwd", task_id=task_id))
    assert result["exit_code"] == 0
    assert [c[0] for c in calls] == ["pwd"]
    command, executed_kwargs = calls[0]
    # Upstream's foreground→background handoff passes a yield_handler on the
    # local backend; every other kwarg is still exactly as before.
    assert callable(executed_kwargs.pop("yield_handler", None))
    assert executed_kwargs == {"timeout": 60, "cwd": "/workspace/live", "bounded_capture": True}
