...
class _RecordingTransport:
    def __init__(self):
        ...
        self.closed = False
        # upstream's _transport_is_dead() reads the private ``_closed``; without it this fake is
        # never recognised as a dead peer and _attach_session_transport fans out instead of rebinding.
        self._closed = False
...
def test_write_json_unserializable_payload_becomes_error_frame(monkeypatch, caplog):
    ...
    The reported payload was a ``datetime``, unserializable under ``json.dumps``; frames here go
    through orjson, which encodes ``datetime`` natively, so the guard needs a type orjson rejects.
    """
    import logging
...
def test_prompt_submit_rejects_non_string_text(text):
    """[CN-fork] P-056 — non-string ``text`` is an invalid-params error, not a crash.

    A JSON-RPC -32602 envelope keeps a malformed Desktop request from reaching
    turn handling with the wrong type (where it surfaced as an opaque -32000
    handler error).
    """
    resp = server.handle_request(
        {
...
def test_config_set_model_waits_for_lazy_agent_before_switch(monkeypatch):
    ...
    # ``_apply_model_switch`` also takes confirm_expensive_model / parsed_flags (upstream model
    # picker); the stub only cares about the first three positional values.
    def _fake_apply(sid, target_session, raw, **_kwargs):
...
def test_prompt_submit_rebinds_session_transport_for_reconnected_client(monkeypatch):
    ...
    _RealThread = threading.Thread  # captured before the module attribute is patched below

    class _ImmediateThread:
        # The turn thread must run inline so the events are emitted inside dispatch(); the
        # FanoutTransport drain (spawned with args=(peer,)) must NOT: _drain re-acquires the
        # transport lock the caller still holds, so running it inline deadlocks. Give it a real
        # thread — captured class, since threading.Thread itself is what we patched.
        def __init__(self, target=None, daemon=None, args=(), **_kwargs):
            self._target = target
            self._args = args
        def start(self):
            if self._args:
                _RealThread(target=self._target, args=self._args, daemon=True).start()
                return
            self._target()
    ...
    server._sessions["sid"] = session
    # A disconnected client's transport is CLOSED: upstream's _attach_session_transport keeps a
    # still-open peer and fans the turn out to both clients (multi-client sessions), so only a dead
    # previous transport triggers the exclusive rebinding this test pins.
    old_transport.close()
    monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
    monkeypatch.setattr(server, "make_stream_renderer", lambda cols: None)
    monkeypatch.setattr(server, "render_message", lambda raw, cols: None)
...
# utils.file_signature pins (mtime, size, ino, ctime). An in-place rewrite that restores mtime and
# size leaves only the inode (atomic replace) / ctime (never backdatable) to change — and on Windows
# st_ino may be 0 while st_ctime is the creation time, so the key deliberately degrades to mtime+size
# there (see utils.file_signature's own note). The #111105 contract holds on POSIX.
