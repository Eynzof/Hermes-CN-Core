...
def test_ws_transport_replies_with_error_for_unserializable_response(caplog):
    """#92506: an unserializable payload must surface as a
    ...
    The reported trigger was a ``datetime`` from profile.yaml ui_meta, which was unserializable
    under ``json.dumps``; this fork serializes frames with orjson, which encodes ``datetime``
    natively, so the payload that cannot be framed is a ``set`` (still TypeError under orjson).
    """
    sent = []
    ...
    bad = {"jsonrpc": "2.0", "id": "profiles", "result": {"created": {1, 2}}}
