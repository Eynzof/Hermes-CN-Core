...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    # macOS exposes /tmp as a stable system alias for /private/tmp. Keep that
    # exact spelling valid without accepting arbitrary symlinked temp dirs or
    # normalized path tricks such as /tmp/nested/../file.
    system_tmp_alias = (
        os.path.join(raw_temp_dir, basename)
        if raw_temp_dir == "/tmp" and temp_dir != raw_temp_dir
        else None
    )
    if operand != canonical_operand and operand != system_tmp_alias:
        return False
