...
# Cap used when ``agent.deadline`` cannot be imported (see ``_get_approval_timeout``): mirrors
# ``agent.deadline.MAX_SAFE_TIMEOUT_S`` — one year, or the Windows locked-wait ceiling, because
# ``Lock.acquire(timeout=...)`` raises OverflowError past CPython's PY_TIMEOUT_MAX (4_294_967 s)
# there. Kept in sync by tests/tools/test_approval_timeout_overflow.py.
_FAIL_CLOSED_TIMEOUT_S = 4_294_000 if sys.platform == "win32" else 365 * 24 * 3600
...
def _get_approval_timeout() -> int:
    ...
    Clamped to ``agent.deadline.MAX_SAFE_TIMEOUT_S``: a larger value overflows ``time_t`` inside
    ``Thread.join`` / ``Lock.acquire`` on macOS (one year) and the 32-bit millisecond field of
    Windows' locked waits (4_294_967 s), and crashed every parallel tool batch; clamping at the
    ...
    try:
        ...
        # fail CLOSED: the raw value would re-open the overflow. Local mirror of
        # agent.deadline's platform-aware cap (that import is what just failed).
        safe_cap = _FAIL_CLOSED_TIMEOUT_S
    if raw > safe_cap:
