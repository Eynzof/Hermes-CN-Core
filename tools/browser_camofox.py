...
def camofox_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        return orjson.dumps({"success": False, "error": (
            f"Cannot connect to Camofox at {get_camofox_url()}. "
            "Is the server running? Start with: npm start (in camofox-browser dir) "
            "or: docker run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser")}).decode('utf-8')
...
def _camofox_private_page_block(session: Dict[str, Any], task_id: Optional[str], action: str) -> Optional[str]:
    ...
    return orjson.dumps({"success": False, "error": (
        ...
        f"({blocked_url}). Refusing to {action} on this page in this browser mode.")}).decode('utf-8')
...
def _tab_action(task_id: Optional[str], guard_action: Optional[str], suffix: str,
    ...
    return _with_tab(task_id, guard_action, lambda session: orjson.dumps(
        result(_post(_tab_path(session, suffix), {"userId": session["user_id"], **body}))).decode('utf-8'))
...
def camofox_snapshot(full: bool = False, task_id: Optional[str] = None, user_task: Optional[str] = None) -> str:
    ...
    def body(session):
        ...
        return orjson.dumps({"success": True, "snapshot": snapshot, "element_count": refs_count}).decode('utf-8')
...
def camofox_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        return orjson.dumps(redact_browser_typed_text_for_display(response, text)).decode('utf-8')
    except Exception as e:
...
def camofox_get_images(task_id: Optional[str] = None) -> str:
    ...
    def body(session):
        ...
        return orjson.dumps({"success": True, "images": images, "count": len(images)}).decode('utf-8')
...
def camofox_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    def body(session):
        ...
        return orjson.dumps({"success": True, "analysis": redact_sensitive_text(analysis), "screenshot_path": screenshot_path}).decode('utf-8')
...
def camofox_console(clear: bool = False, task_id: Optional[str] = None) -> str:
    ...
    return orjson.dumps({
        ...
        "note": "Console log capture is not available with the Camofox backend. "
                "Use browser_snapshot or browser_vision to inspect page state."}).decode('utf-8')
