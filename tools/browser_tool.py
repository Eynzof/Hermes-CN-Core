...
def _build_browser_env() -> dict:
    ...
    # top of the credential-scrubbed env. Every browser subprocess spawn goes
    # through this builder, so the injection covers them all.
    env = _prepare_browser_subprocess_env(env)
    return env
...
def _dumps(payload: Dict[str, Any], **kw) -> str:
    return orjson.dumps(payload, **kw).decode('utf-8')
...
def _post_redirect_block(nav_session_key: str, url: str, final_url: str, auto_local_this_nav: bool) -> Optional[str]:
    ...
    return orjson.dumps(_err(f"Blocked: redirect landed on {what}")).decode('utf-8')
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if safety_error is not None:
        return orjson.dumps(safety_error).decode('utf-8')
...
def _parse_eval_value(raw_result: Any) -> Any:
    ...
    if isinstance(raw_result, str):
        try:
            return orjson.loads(raw_result)
...
def _eval_failure_response(result: Dict[str, Any]) -> str:
    ...
    return orjson.dumps(_lp._copy_fallback_warning(_err(err), result)).decode('utf-8')
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        if any(code in str(e) for code in ("404", "405", "501")):  # server without eval support
            return orjson.dumps(_err("JavaScript evaluation is not supported by this Camofox server. "
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    try:
        images = orjson.loads(raw_result) if isinstance(raw_result, str) else raw_result
