...
def _project_hash(working_dir: str) -> str:
    ...
    return xxhash.xxh64(str(_normalize_path(working_dir)).encode()).hexdigest()[:16]
...
def maybe_auto_prune_checkpoints(retention_days: int = 7, min_interval_hours: int = 24, delete_orphans: bool = True,
    ...
    try:
        ...
        now = time.time()
        try:
            if marker.exists() and now - float(marker.read_text(encoding="utf-8", errors="replace").strip()) < min_interval_hours * 3600:
                out["skipped"] = True
                return out
        except (OSError, ValueError):
