...
def _wsl_windows_path_to_posix(path: str) -> str:
    ...
    if not (wsl and drive):
        return path
    # DrvFS paths are always POSIX — build with "/" explicitly so the translation is identical on
    # Windows hosts (os.path.join would use os.sep and yield ``\mnt\c\...``).
