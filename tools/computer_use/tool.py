...
def _reject_unsafe(action: str, args: Dict[str, Any]) -> Optional[str]:
    ...
    if action == "type" and (pat := next((p.pattern for p in _BLOCKED_TYPE_PATTERNS if p.search(args.get("text", ""))), None)):
        return orjson.dumps({"error": f"blocked pattern in type text: {pat!r}",
                           "hint": "Dangerous shell patterns cannot be typed via computer_use."}).decode('utf-8')
    if action == "key" and (blocked := next((b for b in _BLOCKED_KEY_COMBOS
        ...
        return orjson.dumps({"error": f"blocked key combo: {sorted(blocked)}", "hint": "Destructive system shortcuts are hard-blocked."}).decode('utf-8')
    if args.get("bring_to_front") and args.get("delivery_mode") != "foreground":
        return orjson.dumps({"error": "bring_to_front requires delivery_mode='foreground'",
                           "code": "bring_to_front_requires_foreground"}).decode('utf-8')
    return None
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return orjson.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `hermes computer-use install`. "
                                   "If a Python dependency is missing, the error above shows the exact install command."}).decode('utf-8')
    try:
        with _backend_lock:
...
def _request_approval(action: str, args: Dict[str, Any]) -> Optional[str]:
    ...
    if result.get("approved"):
        return None
    return orjson.dumps({"error": result.get("message") or "denied by user", "action": action}).decode('utf-8')
...
def _do_drag(backend, action, args, **delivery):
    ...
    if (args.get("from_element") is None or args.get("to_element") is None) and not (src and dst):
        return orjson.dumps({"error": "drag requires from_coordinate/to_coordinate or from_element/to_element"}).decode('utf-8')
...
def _do_listing(backend, action, args, key, **_):
    return orjson.dumps({key: (items := getattr(backend, action)()), "count": len(items)}).decode('utf-8')
...
_ACTIONS: Dict[str, _ActionSpec] = {
    ...
    "set_value": _input(lambda backend, action, args, **_: (
        orjson.dumps({"error": "set_value requires `value`"}).decode('utf-8') if args.get("value") is None
        else backend.set_value(value=str(args["value"]), element=args.get("element")))),
    "focus_app": _ActionSpec(lambda backend, action, args, **_: (
        orjson.dumps({"error": "focus_app requires `app`"}).decode('utf-8') if not args.get("app")
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any], session_id: Optional[str] = None) -> Any:
    ...
    if spec is None:
        return orjson.dumps({"error": f"unknown action {action!r}" + (f" — did you mean {hint!r}? See the action enum in the tool schema."
                                                                 if (hint := _ACTION_SUGGESTIONS.get(str(action))) else "")}).decode('utf-8')
    ...
    if (spec.input and isinstance(requested_app := args.get("app"), str) and requested_app.strip()
        ...
        return orjson.dumps({"ok": False, "action": action, "code": "input_target_mismatch", "error": (
            ...
            f"Call capture(app={requested_app.strip()!r}) or focus_app first, then retry.")}).decode('utf-8')
...
def _text_response(res: ActionResult) -> str:
    return orjson.dumps(_action_payload(res)).decode('utf-8')
...
def _maybe_follow_capture(backend: ComputerUseBackend, res: ActionResult, do_capture: bool,
    ...
    if isinstance(resp, dict) and resp.get("_multimodal"):
        ...
        resp["content"][0]["text"] = resp["text_summary"] = orjson.dumps(payload).decode('utf-8') + "\n\n" + resp["text_summary"]
        resp["action_result"] = payload
        return resp
    return orjson.dumps({**orjson.loads(resp), **payload}).decode('utf-8')  # text capture: merge the action payload in
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    with contextlib.suppress(TypeError, json.JSONDecodeError):
        parsed = orjson.loads(analysis_text)
