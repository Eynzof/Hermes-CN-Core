...
def _inherit_parent_endpoint(parent_agent, surface_base_url: Optional[str], surface_api_key: Any) -> tuple:
    ...
    runtimes keep ``client=None``). Within the live source the mounted ``client`` outranks ``_client_kwargs``: a
    mid-session credential refresh/rotation rebuilds the client while the kwargs dict keeps the key it was built
    with, so kwargs-first would hand the child a key that is already expired (same precedence as
    :func:`tools.delegate_tool._inherit_parent_api_key`)."""
