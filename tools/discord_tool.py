...
def _discord_request(
    ...
    req = urllib.request.Request(
        url, data=None if body is None else orjson.dumps(body), method=method,
        headers={
...
def _listing(key: str, items: List[Dict[str, Any]]) -> str:
    return orjson.dumps({key: items, "count": len(items)}).decode('utf-8')
...
def _server_info(token: str, guild_id: str, **_kwargs: Any) -> str:
    ...
    return orjson.dumps({
        ...
        "verification_level": g.get("verification_level")}).decode('utf-8')
...
def _list_channels(token: str, guild_id: str, **_kwargs: Any) -> str:
    ...
    return orjson.dumps({"channel_groups": result, "total_channels": sum(len(g["channels"]) for g in result)}).decode('utf-8')
...
def _channel_info(token: str, channel_id: str, **_kwargs: Any) -> str:
    ...
    return orjson.dumps({
        ...
        "rate_limit_per_user": ch.get("rate_limit_per_user", 0), "last_message_id": ch.get("last_message_id")}).decode('utf-8')
...
def _member_info(token: str, guild_id: str, user_id: str, **_kwargs: Any) -> str:
    ...
    return orjson.dumps(_member_summary(m, full=True)).decode('utf-8')
...
def _create_thread(
    ...
    return orjson.dumps({"success": True, "thread_id": thread["id"], "name": thread.get("name")}).decode('utf-8')
...
def _mutation(method: str, path: str, message: str):
    ...
    def _action(token: str, **kw: Any) -> str:
        ...
        return orjson.dumps({"success": True, "message": message.format(**kw)}).decode('utf-8')
    return _action
