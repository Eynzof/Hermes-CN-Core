...
class _DrainTee:
    """``append``-only shim handing drained chunks to *on_chunk* before the collector.

    The drain loops live in :mod:`tools.environments.base_output` and only ever call
    ``output.append(text)``, so that single method is the whole interface they need.
    Used to keep the P-061 per-chunk hooks (``output_callback`` live observation and
    the ``wait_for_pattern`` scan) in this module.
    """

    __slots__ = ("_inner", "_on_chunk")

    def __init__(self, inner: object, on_chunk: Callable[[str], None]):
        self._inner = inner
...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(
        ...
        promote_callback: Callable[[Any], str | None] | None = None) -> dict:
        ...
        ``output_callback(text)`` receives every drained chunk (live observation by the caller,
        e.g. warming the background registry's buffer for an adopted process).

        ``wait_for_pattern`` (regex): when set, each drained chunk is scanned and the wait
        returns as soon as the pattern appears in output — via ``promote_callback`` when one
        is supplied (the still-running process is adopted into the background registry so the
        agent can poll/wait/kill it), otherwise the wait simply continues to exit. The drain
        keeps a rolling 4096-char tail so matches spanning chunk boundaries still hit.

        ``promote_callback(proc) -> session_id | None``: on pattern match or timeout (instead
        of the historical kill), hand the LIVE process to the caller (the background process
        registry) and return early. When it returns a session id the drain thread is
        deliberately left running so output keeps streaming into the adopted session's buffer
        via the caller's ``output_callback``.
        """
        output = _new_output_collector(proc, bounded_capture)
        _t0 = time.monotonic()
        # P-061 hooks: the drain itself lives in ``base_output``, so the per-chunk
        # hooks (live ``output_callback`` + ``wait_for_pattern`` scan) ride a tee on
        # the way to the collector.
        _pattern_re = None
        _pattern_tail = ""
        _pattern_hit: dict = {"matched": False, "text": ""}
        if wait_for_pattern:
            # Local import: the module-level ``re`` name is re-declared by the
            # plugin-compat block at the bottom of this file.
            import re
            ...
            try:
                ...
                logger.warning(
                    "Invalid wait_for_pattern %r ignored (bad regex)", wait_for_pattern
                )
                _pattern_re = None
        ...
        drain_target = (
            _DrainTee(output, _on_chunk)
            if (_pattern_re is not None or output_callback is not None)
            else output
        )
        drain_stop = threading.Event() if yield_handler is not None else None
        drain_thread = _start_drain_thread(proc, drain_target, drain_stop)
        _now = time.monotonic()
        ...
        try:
            ...
            while proc.poll() is None:
                ...
                if is_interrupted() or is_thread_interrupted(watch_interrupt_tid):
                    trace.interrupted()
                    _kill_and_join()
                    _interrupted = self._finalize_wait_result(
                        output, output.render(suffix="\n[Command interrupted]"), 130)
                    _interrupted["elapsed_seconds"] = round(time.monotonic() - _t0, 2)
                    return _interrupted
                # Pattern match while the process still runs: promote the live
                # process to the background registry (when a callback is available)
                # and return early with the match.
                if _pattern_hit["matched"]:
                    if promote_callback is not None:
                        try:
                            ...
                            _sid = None
                        if _sid:
                            _matched = self._finalize_wait_result(
                                output,
                                output.render(
                                    suffix=(
                            ...
                            _matched["elapsed_seconds"] = round(time.monotonic() - _t0, 2)
                            _matched["promoted"] = True
                            _matched["session_id"] = _sid
                            _matched["timed_out"] = False
                            _matched["pattern_matched"] = True
                            _matched["matched_pattern"] = _pattern_hit["text"]
                            return _matched
                ...
                if time.monotonic() > deadline:
                    if promote_callback is not None:
                        ...
                        if _sid:
                            ...
                            _promoted = self._finalize_wait_result(
                                output,
                                output.render(
                                    suffix=(
                            ...
                            _promoted["elapsed_seconds"] = round(time.monotonic() - _t0, 2)
                            _promoted["promoted"] = True
                            _promoted["session_id"] = _sid
                            _promoted["timed_out"] = True
                            _promoted["pattern_matched"] = False
                            return _promoted
                    trace.timed_out()
                    _kill_and_join()
                    ...
                    _timed_out = self._finalize_wait_result(output, rendered, 124)
                    _timed_out["elapsed_seconds"] = round(time.monotonic() - _t0, 2)
                    return _timed_out
        ...
        result["elapsed_seconds"] = round(time.monotonic() - _t0, 2)
    ...
    def execute(
        ...
        *,
        ...
        ``output_callback`` / ``wait_for_pattern`` / ``promote_callback`` are forwarded to
        ``_wait_for_process`` (live output observation, blocking pattern wait and
        foreground→background promotion; see there for semantics).
        """
