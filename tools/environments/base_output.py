...
class _BoundedOutputCollector:
    ...
    def close_spill(self) -> "str | None":
        ...
        [CN-fork] P-061: also caps future appends — after the caller has consumed
        the rendered output, any straggler chunks (e.g. the drain thread of a
        promoted foreground run) must NOT reopen and truncate the file.
        """
        with self._lock:
            if self._spill_fh is None:
                return None
