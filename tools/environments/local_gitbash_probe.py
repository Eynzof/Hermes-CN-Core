...
# platform.uname() into its WMI-backed path on Python 3.12+, so a module-level
# call fires two WMI queries on every interpreter start that imports the local
# backend (test_wmi_ssl_windows_overhead).
