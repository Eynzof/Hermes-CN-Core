...
class ManagedModalEnvironment(BaseEnvironment):
    ...
    def execute(self, command: str, cwd: str = "", *, timeout: int | None = None, stdin_data: str | None = None,
                rewrite_compound_background: bool = True, bounded_capture: bool = False,
                output_callback=None, wait_for_pattern: str | None = None, promote_callback=None,
                yield_handler=None) -> dict:
        # Signature parity with BaseEnvironment.execute: the foreground terminal tool passes
        # the live-output + handoff hooks unconditionally (only ``yield_handler`` is
        # local-only), so an override that omits them died with TypeError instead of running.
        # The gateway transport returns one terminal payload per exec — no incremental stdout
        # to drain — so ``wait_for_pattern``/``promote_callback``/``yield_handler`` (all of
        # which need to observe or adopt a live host process) degrade to "wait for the
        # command" here; the command itself still runs to completion.
        del rewrite_compound_background, bounded_capture, wait_for_pattern, promote_callback, yield_handler
        if output_callback is not None:
            logger.debug("Managed Modal cannot stream output; output_callback gets the final payload")

        def _finish(result: dict) -> dict:
            """Hand the terminal payload to the live-output sink exactly once."""
            if output_callback is not None and result.get("output"):
                try:
                    output_callback(result["output"])
                except Exception:
                    logger.debug("Managed Modal output_callback failed", exc_info=True)
            return result
        ...
        try:
            ...
            return _finish(_result(f"Managed Modal exec failed: {exc}"))
        if body is None:
            return _finish(_result(self._format_error("Managed Modal exec failed", response)))
        if (final := self._result_from_body(body)) is not None:
            return _finish(final)
        if body.get("execId") != exec_id:
            return _finish(_result("Managed Modal exec start did not return the expected exec id"))
        ...
        while True:
            if is_interrupted():
                ...
                return _finish(_result("[Command interrupted - Modal sandbox exec cancelled]", 130))
            try:
            ...
            if time.monotonic() >= deadline:
                self._cancel_exec(exec_id)
                return _finish(_result(f"Managed Modal exec timed out after {timeout}s", 124))
