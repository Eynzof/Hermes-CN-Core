...
def _parse_optional_int(value: Optional[str]) -> Optional[int]:
    """Parse an int from command/stat output, or ``None`` if not numeric."""
    try:
...
def _snippet_path_literal(path: str) -> str:
    """A Python string literal for ``path`` embedded in a ``python -c`` snippet.

    Windows backslashes are normalized to ``/`` first — every API the snippets
    call (``os.path.getsize``, ``open``, ``pathlib.Path``) accepts that form, and
    ``repr('C:\\Users\\x')`` would embed ``\\U``, which the child parses as a
    unicode escape and dies on with SyntaxError.  POSIX paths pass through
    untouched: a backslash is a legal (if rare) POSIX filename character and
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _exec(self, command: str, cwd: str = None, timeout: int = None,
        ...
        # PowerShell has no POSIX ``/dev/null``; translate the most common
        # redirects to ``$null`` — but ONLY for a shell that needs it (see
        # ``_needs_powershell_null_redirects``): a Git-Bash local backend and
        # every remote/container backend are POSIX shells, where ``2>$null``
        # is an "ambiguous redirect" that silently breaks the command.
        if self._needs_powershell_null_redirects():
    ...
    def _cat(self, path: str) -> ExecuteResult:
        ...
        if self._use_inproc_io():
            return self._prim_read_all(path)
        return self._exec(f"cat {self._escape_shell_arg(path)} 2>/dev/null")

    def _head(self, path: str, nbytes: int) -> ExecuteResult:
        if self._use_inproc_io():
            return self._prim_read_sample(path, nbytes)
        return self._exec(f"head -c {nbytes} {self._escape_shell_arg(path)} 2>/dev/null")

    @staticmethod
    def _quote_python_snippet(snippet: str) -> str:
        """Single-quote a Python *snippet* for the shell, byte-for-byte.

        ``_escape_shell_arg`` runs its argument through the env layer's
        ``_bash_safe_path`` so a native drive path survives Git Bash — correct
        for a path, ruinous for code: it rewrites every backslash to ``/``, so
        ``'\\r\\n'`` reaches the child as ``'/r/n'``, ``b'\\xff\\xfe'`` as
        ``b'/xff/xfe'``, and the snippet silently computes something else
        (the UTF-16 rescue read BOM + CRLF as ordinary text and reported one
        line).  Snippets get plain POSIX single-quoting — identical on every
        host, and a no-op rewrite off Windows.
        """
    ...
    def _run_python_snippet(self, snippet: str) -> ExecuteResult:
        ...
        arg = self._quote_python_snippet(snippet)
        result = self._exec(f"python3 -c {arg}")
        if result.exit_code != 0 and "python3" in (result.stdout or ""):
            result = self._exec(f"python -c {arg}")
        return result
    ...
    def _sample_file_bytes(self, path: str, length: int = 1000):
        ...
        """
        if self._use_inproc_io():
            # [CN-fork] P-033/P-037: sample in-process on a local Windows backend
            # (PowerShell 5.1 has no POSIX ``head``/``base64``) so binary detection
            # still happens at the byte layer, and a GBK/cp936 file is not
            # misclassified as binary by the lossy text heuristic.
            try:
    ...
    def _inproc_sample_is_binary(self, sample: bytes) -> bool:
        """[CN-fork] P-037: byte-layer binary test for in-process samples.

        Same rule as :meth:`_is_likely_binary_bytes` (#80308), except that bytes
        which fail strict UTF-8 but decode cleanly through a legacy fallback
        encoding (the Windows ANSI code page, e.g. GBK/cp936) are TEXT, not
        binary — otherwise every GBK file on a Chinese Windows box would be
        refused as binary.  A sample that still decodes with U+FFFD, or that is
        valid UTF-8 yet trips the byte layer (NUL/control bytes), stays binary so
        a read→edit→write round-trip can never persist mojibake.
        """
        if not self._is_likely_binary_bytes(sample):
            return False
        text = _decode_file_bytes(sample)
        if "\ufffd" in text:
            return True
        try:
            sample.decode("utf-8")
        except UnicodeDecodeError:
            return False  # clean legacy-encoding text (P-037)
        return True

    def _needs_powershell_null_redirects(self) -> bool:
        """True iff this backend's shell needs ``/dev/null`` rewritten to ``$null``.

        Only a Windows-local backend running PowerShell does.  A Windows-local
        backend on Git Bash (``_shell_type == "bash"``, what this fork's
        ``LocalEnvironment`` picks when Git Bash is installed) and every
        remote/container backend (docker/ssh/modal, ``is_local is False``) are
        POSIX shells where ``2>$null`` is an "ambiguous redirect" — the command
        dies with exit 127 and the file-op caller gets the shell's error text as
        output.  An env that reports no shell type at all (mocks, and any
        backend class predating the attribute) keeps the fork's historical
        Windows convention.
        """
        if not sys.platform == "win32":
            return False
        env = getattr(self, "env", None)
        shell_type = getattr(env, "_shell_type", None) or getattr(env, "shell_type", None)
        if isinstance(shell_type, str) and shell_type:
            return shell_type in ("powershell", "pwsh")
        if getattr(env, "is_local", None) is False:
            return False
        return True

    def _is_local_env(self) -> bool:
        """[CN-fork] P-033: True iff this FileOperations is wired to a local backend."""
        env = getattr(self, "env", None)
        if env is None:
            # Defensive: some tests build ShellFileOperations via ``__new__``
            # without going through ``__init__``, so ``self.env`` may be missing.
            return False
        try:
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        """
        # [CN-fork] P-033: the local Windows backend cannot run the POSIX
        # ``mktemp``/``cat``/``mv`` script under PowerShell 5.1 — write in-process.
        if self._use_inproc_io():
    ...
    def _probe_regular_file(self, path: str) -> tuple[int, str]:
        ...
        if self._use_inproc_io():
            # [CN-fork] P-033: stat in-process — the POSIX ``[ -f ]``/``wc -c``
            # probe cannot run under Windows PowerShell.
            stat_result = self._prim_stat_size(path)
            if stat_result.exit_code == 0 and stat_result.stdout == NOT_REGULAR_SENTINEL:
                return 0, "not_regular"
            if stat_result.exit_code != 0:
                return 0, "missing"
            size = _parse_optional_int(stat_result.stdout)
            return (size, "ok") if size is not None else (0, "bad_size")
    ...
    def _detect_binary(self, path: str) -> tuple[bool, Optional[bytes]]:
        ...
        if sample_bytes is not None:
            ...
            if self._use_inproc_io():
                # [CN-fork] P-037: a sample that fails strict UTF-8 but decodes
                # cleanly as the Windows ANSI code page (GBK/cp936) is TEXT.
                return ext_binary or self._inproc_sample_is_binary(sample_bytes), sample_bytes
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            "import sys, json, os\n"
            f"p = {_snippet_path_literal(path)}\n"
            f"offset = {int(offset)}\n"
            f"limit = {int(limit)}\n"
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 2000) -> ReadResult:
        ...
        if self._native_read_enabled() or self._use_inproc_io():
            # [CN-fork] P-033: the local Windows backend reads in-process too — the
            # compound shell probe (``[ -f ]``/``wc -c``/``base64``/``sed``) cannot
            # run under PowerShell 5.1.  ``_read_file_native`` is upstream's
            # in-process reader, already used for local POSIX backends.
    ...
    def _read_file_native(self, path: str, offset: int, limit: int) -> ReadResult:
        ...
        try:
            with open(full, "rb") as fh:
                ...
                ext_binary = has_binary_extension(path)
                sample_is_binary = (
                    self._inproc_sample_is_binary(sample) if _IS_WINDOWS
                    else self._is_likely_binary_bytes(sample))
                if ext_binary or sample_is_binary:
        ...
        # [CN-fork] P-037: decode through the same UTF-8 → ANSI-code-page chain as the
        # in-process primitives so a GBK/cp936 file is not mangled to U+FFFD.
        read_output = _strip_terminal_fence_leaks(
            _decode_file_bytes(raw_page) if _IS_WINDOWS
            else raw_page.decode("utf-8", errors="replace"))
        return self._assemble_read_result(
            read_output, offset=offset, end_line=end_line, total_lines=total_lines,
            file_size=file_size,
    ...
    def read_file_bytes(self, path: str, max_bytes: Optional[int] = None) -> ReadResult:
        ...
        if self._use_inproc_io():
            # [CN-fork] P-033: read the bytes in-process — PowerShell 5.1 has no POSIX
            # ``base64`` and no ``<`` redirection.
            try:
                with open(self._abs_local(path), "rb") as fh:
                    data = fh.read()
            except OSError as exc:
            ...
            return ReadResult(
                ...
                file_size=file_size, is_binary=True)
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        snippet = (
            ...
            f"p = pathlib.Path({_snippet_path_literal(path)})\n"
            "recursive = False\n"
            "try:\n"
    ...
    def _probe_write_target(self, path: str, pre_content: Optional[str], want_pre: bool,
        ...
        if self._use_inproc_io():
            # [CN-fork] P-033: the compound probe (``[ -f ]`` + ``base64`` +
            # ``head``/``cat``) cannot run under Windows PowerShell; the
            # sequential form reads through the in-process primitives instead.
    ...
    def _verify_written_hash(self, path: str, content_bytes: bytes) -> tuple[Optional[bool], Optional[WriteResult]]:
        ...
        try:
            if self._use_inproc_io():
                # [CN-fork] P-033b/P-037: ``sha256sum`` is a POSIX binary that does not
                # exist under Windows PowerShell, so hash the RAW bytes in-process
                # (hashing decoded text could never match a surrogateescape round-trip).
                try:
                    with open(self._abs_local(path), "rb") as fh:
                        disk_sha = hashlib.sha256(fh.read()).hexdigest()
                except OSError as exc:
                    # [CN-fork] P-033b: a file that cannot be read back right after a
                    # reported-success write IS the silent-no-op bug (#54) this
                    # verification exists to catch. Returning verified=None here (as the
                    # pre-merge-merge code did) reported a fabricated success for a write
                    # that never persisted — fail closed instead.
                    return False, WriteResult(error=(
                        f"Post-write verification failed: could not read back {path} "
                        f"after the write ({exc}). The write did not persist — "
