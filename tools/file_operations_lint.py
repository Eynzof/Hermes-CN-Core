...
class LintMixin:
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        if inproc is not None:
            if content is None:
                # [CN-fork] P-037: read through ``_cat`` — in-process on a
                # Windows-local backend, where the POSIX ``cat … 2>/dev/null``
                # read can't run (no `cat`, and `2>/dev/null` becomes a literal
                # \dev\null file under PowerShell 5.1).
