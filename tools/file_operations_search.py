...
# [CN-fork] P-030: the shell-command search paths assume a POSIX shell + GNU
# rg/grep/find. On the Windows/PowerShell local backend none of that holds (the
# very `command -v` probe in `_has_command` can't run), so search hard-errored
# even with rg on PATH. These constants bound a portable os.walk-based fallback
# used only on the local backend. It can't honor .gitignore the way rg does, so
# we prune the usual vendored/cache dirs to approximate it and keep wide trees
# fast.
...
class SearchMixin:
    ...
    def _resolve_command(self, cmd: str) -> Optional[str]:
        ...
        if result.exit_code == 0 and result.stdout.strip():
            ...
            return resolved
        resolved = self._windows_off_path_rg()
        if resolved:
            self._rg_resolution_cache[cmd] = resolved
            return resolved
        return None

    def _windows_off_path_rg(self) -> Optional[str]:
        """Off-PATH Windows rg candidates (cargo, scoop, winget), or None."""
    ...
    def _local_rg_executable(self) -> Optional[str]:
        """[CN-fork] P-030/P-049: rg for the local backend, resolved without the
        POSIX ``command -v`` probe (it cannot run under Windows PowerShell).
        ``_find_rg`` prefers the managed ``<HERMES_HOME>/tools`` copy, then PATH;
        the off-PATH Windows candidates (cargo, scoop, winget) are the last
        resort. Successful hits are cached like ``_resolve_command``'s; misses
        are re-probed so a mid-session install becomes visible."""
        cached = self._rg_resolution_cache.get("rg")
        if cached:
            return cached
        from hermes_cli.dep_ensure import _find_rg

        resolved = _find_rg() or self._windows_off_path_rg()
        if resolved:
            self._rg_resolution_cache["rg"] = resolved
        return resolved
 ...
 lane cannot express (the multi-root ``cd`` prefix); ``shell_prefix`` is shell-only."""
    ...
    def _path_exists_probe(self, path: str) -> ExecuteResult:
        ...
        ``cwd_error`` when the exec wrapper itself failed). [CN-fork] P-030: on the
        local backend this is an in-process ``os.path.exists`` — the POSIX
        ``test -e`` probe can't run under Windows PowerShell."""
        if self._is_local_env() or self._native_read_enabled():
    ...
    def _path_not_found_result(self, path: str) -> SearchResult:
        ...
        # [CN-fork] P-030: on the local backend suggest nearby entries in-process
        # (``os.listdir``/``os.path.isdir``) — the POSIX ``test -d`` and
        # ``ls | head`` probes can't run under Windows PowerShell.
    ...
    def _needs_inproc_search_fallback(self) -> bool:
        """True iff this local backend must resolve and run its search in-process (P-030).

        Only a Windows-local backend running PowerShell does: the POSIX ``command -v``
        probe in ``_has_command`` and the ``find``/``grep`` pipelines cannot run there
        (GitHub #334).  A Windows-local backend on Git Bash (``_shell_type == "bash"``,
        what this fork's ``LocalEnvironment`` picks when Git Bash is installed) and every
        remote/container backend are POSIX shells, so upstream's engine selection —
        ``_has_command`` then rg, else find/grep — stays reachable and must decide the
        engine.  Deliberately mirrors ``file_operations._needs_powershell_null_redirects``;
        an env whose shell type is unknown keeps the fork's Windows convention.
        """
    ...
    def _search_files(self, pattern: str, path: str | List[str], limit: int, offset: int,
        ...
        # [CN-fork] P-030: on a Windows-local PowerShell backend never shell out to the POSIX
        # `command -v` probe or the find pipeline (both break under Windows PowerShell).
        # Resolve rg in-process and fall back to a portable Python walk when it is absent
        # (GitHub #334). Upstream's broad-root refusal is kept for wide roots: neither
        # `find` nor the Python walk may run there. A POSIX-shell local backend (Git Bash,
        # and every POSIX host) keeps upstream's `_has_command` engine selection.
        if self._needs_inproc_search_fallback():
            broad = any(self._is_broad_local_search_root(root) for root in roots)
            rg_executable = self._resolve_command("rg") if broad else self._local_rg_executable()
            if rg_executable and order == "modified":
            ...
            if not rg_executable:
                ...
                return self._search_files_python(
                    ...
                    has_hidden_path_ancestor=any(
                        ...
                        for root in roots for part in Path(root).parts))
    ...
    def _search_content(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # [CN-fork] P-030: on a Windows-PowerShell local backend there is no reliable POSIX
        # shell (the `command -v` probe and the grep pipelines can't run), so pick the
        # engine in-process: rg when it resolves, else a portable Python scan (#334). A
        # POSIX-shell local backend keeps upstream's `_has_command` engine selection.
        if self._needs_inproc_search_fallback():
            rg_executable = self._local_rg_executable()
            used_rg = bool(rg_executable)
            if used_rg:
                result = self._search_with_rg(pattern, path, file_glob, limit, offset, output_mode, context,
                                              rg_executable=rg_executable)
            else:
                result = self._search_content_python(pattern, path, file_glob, limit, offset,
                                                     output_mode, context)
        ...
        # explanation only applies to the grep/python fallback.
        if used_rg:
            return result
    ...
    # =========================================================================
    # [CN-fork] P-030: portable in-process search fallback (local backend
    # without ripgrep)
    #
