...
# macOS resolves its per-user temporary directory below /private/var/folders. Keep the
# rest of /private/var protected while allowing normal workspace and test writes under
# that OS-managed temporary tree.
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # Check the exact Hermes config path BEFORE generic system prefixes so callers get the
    # actionable config-specific error even when a profile is rooted below a protected
    # location. approvals.mode and other security settings live in config.yaml; a
    ...
    for candidate in candidates:
        # $TMPDIR / /tmp / /var/folders all realpath() below /private/var on macOS:
        # the OS-managed temp tree stays writable.
        if any(candidate.startswith(prefix) for prefix in _WRITABLE_SYSTEM_TEMP_PREFIXES):
            continue
        if candidate.startswith(_SENSITIVE_PATH_PREFIXES) or candidate in _SENSITIVE_EXACT_PATHS:
            return (
