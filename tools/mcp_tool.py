...
# [CN-fork] P-014: emit the "MCP configured but SDK missing" warning at most once per process.
# discover_mcp_tools() is called repeatedly (startup, /reload-mcp, cron ticks), so without this
# guard a frozen build lacking the SDK would spam the log. Read through the module so
# ``patch("tools.mcp_tool._warned_mcp_sdk_unavailable", False)`` keeps working.
