...
def discover_mcp_tools(allowed_mcp_names: Optional[List[str]] = None) -> List[str]:
    ...
    # none — never pays it.
    if not _core._ensure_mcp_sdk():
        # [CN-fork] P-014: a frozen runtime shipped without the `mcp` package registers no
        # tools and used to log this only at debug level — invisible at the default INFO
        # level (issue #16). Stay loud ONCE per process, and only when servers are actually
        # configured: the no-MCP user keeps the quiet debug path, and repeated discovery
        # (reloads, cron ticks) must not spam the log.
        if not _core._warned_mcp_sdk_unavailable:
            logger.warning(
                "mcp_servers are configured but the MCP SDK is not available in this "
                "runtime -- no MCP tools will be registered. Install the 'mcp' package "
                "(frozen/desktop builds must bundle it)."
            )
            # The flag lives on the origin module (``_OriginProxy`` is read-only), which is
            # also the module ``mock.patch`` targets; late-import per the sibling/facade rule.
            from tools import mcp_tool as _mcp_origin

            _mcp_origin._warned_mcp_sdk_unavailable = True
        else:
