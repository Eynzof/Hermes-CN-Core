...
def load_on_disk_store() -> "MemoryStore":
    ...
    try:
        ...
        store = MemoryStore(normalize_memory_char_limit(mem_cfg.get("memory_char_limit", DEFAULT_MEMORY_CHAR_LIMIT)),
                            int(mem_cfg.get("user_char_limit", 1375)),
...
def _validate_single_op(store, action, target, content, old_text) -> Optional[str]:
    ...
    if action in ("replace", "remove") and not old_text:
        return orjson.dumps({
            ...
            "current_entries": store._entries_for(target), "usage": store._usage(target)}).decode('utf-8')
...
def memory_tool(action: str = None, target: str = "memory", content: str = None, old_text: str = None,
    ...
    if target_error is not None:
        return orjson.dumps(target_error).decode('utf-8')
    if operations:
        ...
        if gate_result is not None:
            return gate_result
        return orjson.dumps(store.apply_batch(target, operations)).decode('utf-8')
    ...
    if invalid is not None:
        return invalid
    return orjson.dumps(_STORE_ACTIONS[action][0](store, target, content, old_text)).decode('utf-8')
