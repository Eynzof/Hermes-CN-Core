...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 1375, *,
        ...
        # [CN-fork] P-055: clamp through the one shared normalizer (1..8000, default
        # 2200) so a config/env value outside the documented bounds can't make the
        # budget meaningless (e.g. a 0-char limit refusing every add).
        from tools.memory_tool import normalize_memory_char_limit  # lazy: memory_tool imports this module
    ...
    def load_from_disk(self):
        ...
        # [CN-fork] P-055 diagnostic for GitHub #335: log the resolved directory so a
        # "memory looks lost across restarts" report can be diagnosed — the store is
        # always persisted synchronously, so the usual cause is a HERMES_HOME /
        # profile drift making two launches read different `memories/` dirs.
        logger.info("memory: loading from %s", self._path_for("memory").parent)
    ...
    def _file_lock(path: Path):
        ...
        try:
            ...
            # [CN-fork] P-055/P-051: keep the text-mode decode pin explicit — a lock
            # file written by other tooling must never raise UnicodeDecodeError here.
            fd = os.fdopen(raw_fd, "r+", encoding="utf-8", errors="replace")
        except Exception:
            os.close(raw_fd)
            raise
        with fd:
    ...
    def _mutate(self, target: str, mutate, *, skip_drift: bool = False) -> Dict[str, Any]:
        ...
        with self._file_lock(path):
            ...
            mkdir_under_hermes_home(path.parent)
            # [CN-fork] P-055 diagnostic for GitHub #335: every mutation persists here
            # synchronously (temp file + atomic os.replace), so logging the path makes a
            # cross-launch "lost memory" report traceable to profile/HERMES_HOME drift
