...
# [CN-fork] P-044: WMI-free Windows probe (platform.system() triggers a WMI query on py3.12+).
...
# See #70716.
_IS_LINUX = is_linux()
# [CN-fork] P-058: the Windows background spawn follows the resolved shell
# (git-bash → pwsh → powershell), so the shell-aware wrapper builders are imported here.
...
class ProcessSession:
    ...
    # [CN-fork] P-061: True when this session was adopted from a foreground run
    # (foreground → background promotion): the registry has NO reader thread of its
    # own — a foreground drain thread owns the pipe and streams into output_buffer
    # via an output_callback; completion is detected lazily by
    # _reconcile_local_exit() from poll()/wait()/list_sessions().
    _adopted: bool = field(default=False, repr=False)
    # [CN-fork] P-061: True once the rolling output buffer dropped the oldest bytes
    # (the 200KB window wrapped). Surfaced as `output_truncated` so the model knows
    # the middle of the log is gone and should use output_path export.
...
# [CN-fork] P-061: output-buffer helper. All reader loops (local pipe, PTY,
# sandbox poller) and the adopt path funnel through here so ``output_truncated``
# metadata stays accurate: once the 200KB window wraps, ``_buffer_overflowed``
# is set and ``poll()``/``read_log()`` report it to the model.
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _scope_argv(self, session: ProcessSession, safe_command: str, unit_suffix: str, label: str,
                    cwd_file: str | None = None) -> List[str]:
        """Shell argv for *safe_command* (parity with LocalEnvironment: rc files
        ...
        [CN-fork] P-058: on Windows the argv follows the RESOLVED shell —
        git-bash (default when a working install exists) gets ``-lc`` with the
        bash wrapper script, PowerShell 7 / Windows PowerShell 5.1 keep
        ``-NoProfile -Command`` with the PowerShell wrapper — so a background
        spawn never depends on git-bash being installed.
        """
        if _IS_WINDOWS:
            ...
            if shell_type == "bash":
                script = _build_bash_background_script(
                    ...
                    cwd=_windows_to_msys_path(session.cwd),
                    cwd_file=_windows_to_msys_path(cwd_file) if cwd_file else None,
                )
                return [shell_path, "-lc", script]
    ...
    def _spawn_local_pty(self, session: ProcessSession, safe_command: str, env_vars: dict,
                         cwd_file: str | None = None) -> ProcessSession:
        ...
        pty_argv = self._scope_argv(session, safe_command, session.id, "PTY", cwd_file=cwd_file)
    ...
    def spawn_local(
        ...
        [CN-fork] P-058: ``cwd_file`` optionally receives the final working directory
        from the wrapper (PowerShell writes ``(Get-Location).Path``, bash writes
        ``pwd``) so later foreground commands pick up a background ``cd``.
        """
        ...
        if use_pty:
            try:
                return self._spawn_local_pty(session, safe_command, env_vars, cwd_file=cwd_file)
            except ImportError:
        ...
        spawn_argv = self._scope_argv(session, safe_command, unit_suffix, "Local", cwd_file=cwd_file)
    ...
    def adopt_local(
        self, proc: Any, session: Optional[ProcessSession] = None, *, command: str = "",
        cwd: Optional[str] = None, task_id: str = "", session_key: str = "",
        owner_task_id: str = "", output_so_far: str = "",
        notify_on_complete: bool = True) -> Any:
        """Take over a still-running foreground Popen as a tracked background session.

        Two call shapes share this name:

        * upstream yield-to-background — ``adopt_local(proc, command=..., cwd=...,
          ...)``: the caller has stopped its own drain thread and the registry's
          reader continues from the pipe's current position; ``output_so_far``
          seeds the buffer so nothing already captured is lost. Returns the
          ``ProcessSession``.
        * [CN-fork] P-061 background promotion — ``adopt_local(session, proc)``:
          register a session pre-built by :meth:`prepare_adopt_local` (foreground
          ``promote_on_timeout`` / ``wait_for_pattern``). The foreground drain
          thread keeps owning the pipe and streams into the session's buffer via
          the caller's ``output_callback``; the registry starts no reader of its
          own, so completion is detected lazily by ``_reconcile_local_exit()``
          from poll()/wait()/list_sessions(). Returns the session id.
        """
        if session is not None:
            # P-061 shape: adopt_local(<prepared session>, <live Popen>).
            adopt_session, live_proc = proc, session
            with adopt_session._lock:
                adopt_session.process = live_proc
                adopt_session.pid = live_proc.pid
                adopt_session.host_start_time = self._safe_host_start_time(adopt_session.pid)
                adopt_session._adopted = True
            with self._lock:
                self._prune_if_needed()
                self._running[adopt_session.id] = adopt_session
            self._write_checkpoint()
            logger.info(
                "Adopted foreground process %s (pid=%s) as background session %s",
                adopt_session.command[:80], adopt_session.pid, adopt_session.id,
            )
            return adopt_session.id
    ...
    def prepare_adopt_local(
        self, command: str, task_id: str = "", session_key: str = "", cwd: str | None = None,
    ) -> ProcessSession:
        """[CN-fork] P-061: create an UNREGISTERED session that a foreground run may adopt.

        Foreground→background promotion (terminal ``promote_on_timeout`` /
        ``wait_for_pattern``) hands a still-running foreground Popen to the registry
        instead of killing it at the timeout. The caller creates the session up
        front, streams output into it via an ``output_callback`` (so the buffer is
        already warm at adoption), and calls :meth:`adopt_local` when the
        timeout/pattern fires.
        """
        return ProcessSession(
    ...
    def poll(self, session_id: str, offset: int = None) -> dict:
        ...
        [CN-fork] P-061: ``offset`` (optional line number) turns poll into a
        new-output read — the result carries ``output`` = lines at/after
        ``offset`` plus ``total_lines``, so the model can page a log
        incrementally (the background counterpart of kimi's consuming
        ``pop_output``). ``elapsed_seconds``/``output_total_chars``/
        ``output_truncated`` ride every result; an exited process also gets
        ``exit_code_meaning`` (informational non-zero codes) or ``hint``.
        """
        from tools.ansi_strip import strip_ansi
        ...
        result = {
            **self._status_head(session), "pid": session.pid,
            ...
            "output_total_chars": len(full_buffer), "output_truncated": output_truncated}
        ...
        if session.exited:
            ...
            if session.exit_code:
                # Informational non-zero codes (grep=1 "no matches", diff=1, signal
                # deaths) get a meaning; otherwise fall back to the recovery hint.
                with suppress(Exception):
                    ...
                    if meaning:
                        ...
                        hint = annotate_failure(session.command, session.exit_code, output_preview)
                        if hint:
                            result["hint"] = hint
    ...
    def read_log(self, session_id: str, offset: int | None = None, limit: int = 200,
        ...
        [CN-fork] P-061: ``output_path`` optionally exports the FULL accumulated
        buffer to a file (a literal path, or ``"auto"`` for a session temp file)
        so the model can page the middle of a long log that the 200KB rolling
        window may have dropped — the background counterpart of the foreground
        spill file. The result always carries ``output_total_chars`` and
        ...
        result = {
            **self._status_head(session), "output": "\n".join(selected),
            ...
            "output_total_chars": len(full_output), "output_truncated": output_truncated}
        if output_path:
    ...
    def wait(
        ...
        pattern: str | None = None, inactivity_timeout: int | None = None, since_chars: int = 0,
    ) -> dict:
        ...
        ``timeout`` defaults to (and is clamped by) TERMINAL_TIMEOUT. Returns a dict with
        status exited|matched|output|timeout|interrupted|not_found|error and an output
        snapshot.

        [CN-fork] P-061:
        ``pattern`` — return as soon as the regex matches NEW output (``status="matched"``,
        ``wait_matched=True``) even while the process keeps running: the blocking
        counterpart of the push-only ``watch_patterns``.
        ``inactivity_timeout`` — seconds of output silence while the process still runs
        before returning early with partial output (``status="timeout"``,
        ``inactivity_timeout=True``, ``process_running=True``); defaults to 120s when
        ``pattern`` is set, so a silent process can never block the agent for the whole
        window.
        ``since_chars`` — character cursor into the rolling buffer; only output produced
        AFTER the cursor is reported and (without a pattern) the wait returns as soon as
        any new output arrives (``status="output"``, ``process_running=True``) — used by
        ...
        if pattern:
            try:
                ...
                return {"status": "error", "error": f"Invalid regex pattern: {pattern}"}
        # Inactivity default: when waiting on a pattern, cap silent periods so the agent
        # never blocks on a process that stopped producing output.
        ...
        # Cursor into the rolling buffer: ``scan_cursor`` advances as output is consumed
        # for pattern scanning; ``result_cursor`` stays at the original position so the
        # returned ``output`` covers everything produced since this wait began (or since
        # ``since_chars``).
        ...
        while time.monotonic() < deadline:
            ...
            if session is None:
                return _not_found(session_id)
            # Cursor clamp: if the 200KB rolling window wrapped past the since_chars
            # cursor, fall back to the whole buffer so a continuation can't hang
            # forever on a stale cursor.
            buf_len = _buf_len()
            ...
            if session.exited:
                ...
                if result_cursor:
                    result["output"] = _since(result_cursor)
                ...
                if new_text:
                    if pattern_re is not None:
                        ...
                        if match:
                            result = {
                                "status": "matched", "command": session.command,
                                "wait_matched": True, "matched_pattern": match.group(0),
                                "pattern": pattern,
                                "output": _since(result_cursor) or _output_tail(session, 2000),
                                "process_running": True}
                    if result is None:
                        # New output arrived — remember when, and consume the scan cursor
                        # so old text is never re-scanned.
                        last_output_at = time.monotonic()
                        scan_cursor = buf_len
                        if result_cursor and pattern_re is None:
                            ...
                            result = {
                                "status": "output", "command": session.command,
                                "output": _since(result_cursor), "process_running": True}
                # Inactivity early-return: a still-running process that stopped producing
                # output for `inactivity_timeout` seconds yields partial output instead of
                # blocking the whole window (kimi's DEFAULT_INACTIVITY_TIMEOUT semantics).
                if (
                    result is None and inactivity_timeout is not None
                    and (time.monotonic() - last_output_at) >= inactivity_timeout
                ):
                    result = {
                        "status": "timeout", "command": session.command,
                        "output": _since(result_cursor) or _output_tail(session, 1000),
                        "inactivity_timeout": True, "process_running": True}
                    if pattern_re is not None:
                        result["wait_matched"] = False
                        result["pattern"] = pattern
                    if not timeout_note:
                        result["timeout_note"] = (
                            f"No new output for {int(inactivity_timeout)}s while the process is "
                            "still running — returned early with partial output. Poll again later "
                            "or use process(action='wait', pattern=...) / terminal(process_id=...) "
                            "to wait on a signal.")
            if result is not None:
                result.setdefault("elapsed_seconds", round(time.monotonic() - started, 2))
                if timeout_note:
        ...
        result = {
            ...
            "process_running": True, "elapsed_seconds": round(time.monotonic() - started, 2)}
        if pattern_re is not None:
            result["wait_matched"] = False
            result["pattern"] = pattern
        base_note = (
    ...
    def kill_process(
        ...
        try:
            ...
            return {
                ...
                # [CN-fork] P-061: unified process metadata (process age, matching poll()).
                "elapsed_seconds": round(time.time() - session.started_at, 2) if session.started_at else None}
        except Exception as e:
...
PROCESS_SCHEMA = {
    ...
    "description": (
        ...
        # [CN-fork] P-061: the ergonomics pass (pattern waits, silence cap, new-output
        # reads, full-log export) rides the same verbs.
        " Prefer wait with pattern= for mid-process signals (server readiness, migration-done) "
        "instead of blind polling; inactivity_timeout= returns early on output silence. "
        "poll takes offset= for new-output-only reads; log takes output_path= to export the "
        "full accumulated buffer to a file."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            # --- [CN-fork] P-061: wait/poll/log ergonomics ---
            "pattern": {
                "type": "string",
...
_SESSION_ACTIONS = {
    "poll": (lambda sid, a: process_registry.poll(sid, offset=a.get("offset")), True),
    "log": (lambda sid, a: process_registry.read_log(
        sid, offset=a.get("offset"), limit=a.get("limit", 200),
        output_path=a.get("output_path")), True),
    "wait": (lambda sid, a: process_registry.wait(
        sid, timeout=a.get("timeout"), pattern=a.get("pattern"),
        inactivity_timeout=a.get("inactivity_timeout")), True),
...
def _handle_process(args, **kw):
    ...
    # [CN-fork] P-061: kimi-style convenience — a boolean `block` on poll upgrades it
    # to a blocking wait (per-tool alias maps `wait` → `block`).
