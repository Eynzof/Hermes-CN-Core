...
def _activated(proj, task_id: Optional[str]) -> str:
    ...
    return orjson.dumps({
        ...
        "primary_path": primary}).decode('utf-8')
...
def project_list(task_id: Optional[str] = None) -> str:
    ...
    return orjson.dumps({
        ...
        "projects": [
            ...
            for p in projects]}).decode('utf-8')
...
def _handle_project(args, **kw):
    ...
    if action is None:
        return orjson.dumps({"success": False, "error": "action must be one of: create, switch, list."}).decode('utf-8')
