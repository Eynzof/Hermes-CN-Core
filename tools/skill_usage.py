...
def load_usage() -> Dict[str, Dict[str, Any]]:
    ...
    path = _usage_file()
    try:
        data = orjson.loads(path.read_text(encoding="utf-8", errors="replace")) if path.exists() else {}
    except (OSError, orjson.JSONDecodeError) as e:
...
def save_usage(data: Dict[str, Dict[str, Any]]) -> bool:
    ...
    path = _usage_file()
    try:
        atomic_write_text(path, orjson.dumps(data, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS).decode('utf-8'), tmp_prefix=".usage_")
        return True
    except Exception as e:
