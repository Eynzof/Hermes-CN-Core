...
class GitHubSource(SkillSource):
    ...
    def _read_cache(self, key: str) -> Optional[list]:
        ...
        try:
            ...
            if time.time() - stat.st_mtime > INDEX_CACHE_TTL:
                return None
            return orjson.loads(cache_file.read_text(encoding="utf-8"))
    ...
    def _write_cache(self, key: str, data: list) -> None:
        ...
        try:
            cache_file.write_text(orjson.dumps(data).decode('utf-8'), encoding="utf-8")
        except OSError as e:
...
def _write_index_cache(key: str, data: Any) -> None:
    ...
    cache_file = index_cache_dir / f"{key}.json"
    try:
        cache_file.write_text(orjson.dumps(data, default=str).decode('utf-8'), encoding="utf-8")
    except OSError as e:
        logger.debug("Could not write cache: %s", e)
...
class HubLockFile:
    ...
    def load(self) -> dict:
        ...
        try:
            return orjson.loads(self.path.read_text(encoding="utf-8"))
...
def _load_hermes_index() -> Optional[dict]:
    ...
    if hermes_index_cache_file.exists():
        try:
            ...
            if age < HERMES_INDEX_TTL:
                return orjson.loads(hermes_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, orjson.JSONDecodeError):
            pass
    ...
    try:
        ...
        hermes_index_cache_file.write_text(orjson.dumps(data).decode('utf-8'), encoding="utf-8")
    except OSError:
        pass
