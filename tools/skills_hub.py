...
def _write_index_cache(key: str, data: Any) -> None:
    ...
    try:
        (index_cache_dir / f"{key}.json").write_text(
            orjson.dumps(data, default=str).decode('utf-8'), encoding="utf-8"
        )
    except OSError as e:
...
class _JsonStateFile:
    ...
    def _read(self) -> dict:
        try:
            ...
            return orjson.loads(orjson.dumps(self.EMPTY))

    def _write(self, data: dict, **dumps_kwargs) -> None:
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.path.write_text(
            orjson.dumps(data, option=orjson.OPT_INDENT_2, **dumps_kwargs).decode('utf-8') + "\n",
            encoding="utf-8")
