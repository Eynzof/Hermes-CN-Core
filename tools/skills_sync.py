...
def _read_manifest() -> Dict[str, str]:
    ...
    try:
        result = {}
        for line in _manifest_file().read_text(encoding="utf-8", errors="replace").splitlines():
            line = line.strip()
            if not line:
                continue
            if ":" in line:
...
def _backfill_optional_provenance(quiet: bool = False) -> List[str]:
    ...
    try:
        data = orjson.loads(lock_path.read_text(encoding="utf-8")) if lock_path.exists() else {"version": 1, "installed": {}}
