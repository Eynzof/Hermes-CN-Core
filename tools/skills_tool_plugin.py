...
def _available_skill_files(skill_dir: Path) -> Dict[str, List[str]]:
    ...
    for f in skill_dir.rglob("*"):
        if not f.is_file() or f.name == "SKILL.md":
            continue
        # POSIX separators: these paths are handed to the model (and matched
        # against "references/"-style prefixes) — os.sep would mis-group every
        # support file into "other" on Windows.
...
def _plugin_skill_linked_files(skill_root: Path) -> Dict[str, List[str]] | None:
    ...
    for category in _SUPPORT_DIRS:
        # POSIX-style relative paths, matching the ``file_path=`` values skill_view accepts
        # (and the 'references/api.md' form its usage hint advertises) — os-native
        # separators would hand the model a 'references\\api.md' on Windows.
        files = [
