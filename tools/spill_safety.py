...
def open_exclusive(path: Path, *, private: bool = True, overwrite: bool = False,
    ...
    if os.name == "nt" and os.path.lexists(path):
        # Windows has no O_NOFOLLOW, and CREATE_NEW is not the POSIX O_EXCL
        # guarantee here: a planted DANGLING symlink makes CreateFile resolve the
        # missing target and create THAT (the attacker's path, not ours). Refuse
        # any existing directory entry up front — the documented contract is
        # "fails on ANY existing path, even a dangling link".
        raise OSError(f"refusing to write through an existing path: {path}")
