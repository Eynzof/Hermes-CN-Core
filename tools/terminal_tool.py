...
# [CN-fork] P-054: tool_call_id-keyed live-output sink (desktop foreground streaming).
...
# [CN-fork] P-058: the first sentence is the anchor ``_build_dynamic_terminal_description``
# rewrites to the shell the terminal actually runs (git-bash / pwsh / powershell); upstream's
# "stated in your environment section" sentence is kept verbatim behind it. The pager hint is
# the second anchor (PowerShell gets "Out-Host -Paging").
TERMINAL_TOOL_DESCRIPTION = """Execute shell commands on a Linux environment. The host OS, shell, and terminal backend are stated in your environment section — write commands for THAT platform. Filesystem, current working directory, and exported environment variables persist between calls.
...
PTY: pty=true + background=true for interactive CLIs (they hang without a terminal); drive them with process(action="write"/"submit"). Local backend only. Pipe git output to cat if it might page.
"""


# [CN-fork] P-058: shell-aware tool description. Upstream states the host OS, shell and
# backend in the environment section; the fork additionally rewrites the platform sentence
# and the forbidden-command examples to the shell the terminal actually runs
# (git-bash → pwsh → powershell), so the model never emits PowerShell cmdlets into a
# git-bash session (or POSIX syntax into PowerShell).
...
def _build_dynamic_terminal_description() -> dict:
    ...
    # Replace the static platform sentence with the real platform/shell one.
...
# ---- [CN-fork] P-061: terminal LLM-ergonomics helpers -------------------------
...
def _plan_execution(
    ...
    interactive: bool = False, promote_on_timeout: bool = False,
    wait_for_pattern: Optional[str] = None,
) -> _ExecPlan:
    ...
    run in the background, an interactive session on a remote backend).

    [CN-fork] P-061: ``interactive`` requires the local backend (a persistent
    shell/REPL needs a host PTY); ``promote_on_timeout`` / ``wait_for_pattern``
    opt out of the detachment refusal below — the whole point is "run it; if it
    doesn't finish, keep it running as a background session instead of killing it".
    """
    ...
    # [CN-fork] P-061: interactive mode is a persistent shell/REPL session —
    # force background + PTY so stdin stays open for later continuation
    ...
    if not background:
        ...
        # [CN-fork] P-061: skipped when the model explicitly opted into promote_on_timeout /
        # wait_for_pattern — the whole point is "run it; if it doesn't finish, keep it running as a
        # background session instead of killing it".
        if not (promote_on_timeout or wait_for_pattern):
...
def _merge_result_metadata(result_json: str, **fields) -> str:
    """[CN-fork] P-061: attach result metadata (elapsed_seconds, wait_matched, cwd,
    original_path) to a finalized terminal result; anything None is skipped, and a
    non-dict payload or an error envelope is returned unchanged."""
    try:
        data = json.loads(result_json)
    except (TypeError, ValueError):
        return result_json
...
def _run_foreground(
    ...
    promote_on_timeout: bool = False, wait_for_pattern: Optional[str] = None,
    token_kill: bool = True, max_lines: Optional[int] = None,
    tool_call_id: Optional[str] = None,
) -> str:
    """Execute in the foreground with retry on transient errors, then finalize.

    [CN-fork] P-061: ``promote_on_timeout`` / ``wait_for_pattern`` (local backend)
    prepare an unregistered registry session up front and stream the live output
    into it, so a still-running process is ADOPTED at the timeout (or on the
    pattern match) instead of being killed — long work is never lost. On the
    continuation path the same hooks keyed by ``tool_call_id`` feed the desktop
    live-output sink (P-054), and ``token_kill`` / ``max_lines`` drive the rtk
    post-processing pipeline (P-049).
    """
    max_retries = 3
    env_type, eff, effective_timeout = plan.env_type, plan.effective_task_id, plan.effective_timeout
    started = time.monotonic()

    # Foreground→background promotion (promote_on_timeout / wait_for_pattern):
    # prepare an unregistered registry session UP FRONT and stream output into it,
    # so when the timeout/pattern fires the still-running process can be adopted
    # with its buffer already warm. Local backend only (adoption needs a host Popen).
    adopt_session = None
    if (promote_on_timeout or wait_for_pattern) and env_type == "local":
        try:
            ...
            adopt_session = _proc_registry.prepare_adopt_local(
                command=command, task_id=eff, session_key=session_key, cwd=plan.cwd,
            )
        except Exception:
            logger.warning(
                "promote_on_timeout requested but adopt session could not be prepared; "
                "falling back to kill-on-timeout", exc_info=True,
            )
            adopt_session = None
    ...
    def _session_sink(chunk: str) -> None:
        ...
        if tool_call_id and has_foreground_output_sink():
            emit_foreground_output(tool_call_id, chunk)
    ...
    for retry_count in range(max_retries + 1):
        try:
            ...
            # [CN-fork] P-061/P-054: live-output hooks (registry warm-up + the
            # tool_call_id-keyed desktop output sink).
            if adopt_session is not None:
                ...
                execute_kwargs["output_callback"] = (
                    lambda chunk: emit_foreground_output(tool_call_id, chunk)
                )
            # [CN-fork] P-049: apply token_kill command rewriting before execution.
            exec_command = command
            rtk_rewritten = False
            if token_kill:
    ...
    # [CN-fork] P-061: the command outlived the foreground timeout (or matched
    # wait_for_pattern) and is now tracked as a background session — return the
    # session_id so the model can poll/wait/log/kill it. Long work is never lost.
    if result.get("promoted"):
        promoted_result = {
            "output": result.get("output", ""), "exit_code": None, "error": None,
            "command": exec_command, "status": "promoted",
            ...
            "hint": (
                "The command exceeded the foreground timeout and was NOT killed — it is "
                f"now a tracked background process (session_id={result.get('session_id')}). "
                "Use process(action='poll'/'log'/'wait') to check on it, "
                "process(action='wait', pattern=...) to wait for a signal, or "
                "process(action='kill') to stop it."
            ),
        }
        promoted_cwd = getattr(env, "cwd", None)
        if promoted_cwd:
            promoted_result["cwd"] = str(promoted_cwd)
        return json.dumps(promoted_result, ensure_ascii=False)
    ...
    # [CN-fork] P-049: token-filter pipeline (dedup / max_lines / original export) —
    # upstream's finalizer then applies the byte cap + redaction on top.
    post_result = None
    with _quiet("Token-filter post-processing failed"):
        from tools.terminal_post_process import _token_filter_output, filter_output

        result["output"] = filter_output(result.get("output") or "")
        post_result = _token_filter_output(
            result["output"], token_kill=token_kill, rtk_rewritten=rtk_rewritten,
            max_lines=max_lines,
        )
        result["output"] = post_result.output
    finalized = finalize_foreground_result(
    ...
    return _merge_result_metadata(
        ...
        wait_matched=(bool(result.get("pattern_matched")) if wait_for_pattern else None),
        original_path=(post_result.original_path if post_result else None),
        # [CN-fork] P-052: surface the environment's shell-fix warnings so the model
        # sees the reason/alternative instead of puzzling over a missing native tool.
        pwsh_warnings=(result.get("pwsh_warnings") or None),
        bash_fix_warnings=(result.get("bash_fix_warnings") or None),
    )
...
def terminal_tool(
    ...
    ``"interactive"`` (start a persistent shell/REPL session you can continue later
    via ``process_id``). Overrides ``background`` when set.
    ``interactive`` — start a persistent interactive shell/REPL (local backend,
    PTY). Equivalent to ``mode="interactive"``. Continue the session later by
    passing its session_id as ``process_id`` with new commands.
    ``process_id`` — session id of an existing BACKGROUND process to CONTINUE:
    sends ``command`` to its stdin and waits for its NEW output.
    ...
    background ``session_id``. Also enables ``wait_for_pattern`` early-return.
    ``wait_for_pattern`` — regex: with ``promote_on_timeout`` (foreground) return
    as soon as it appears in output and keep the process running; on continuation
    (``process_id``) wait for new output matching it.
    ``inactivity_timeout`` — seconds of output silence before a wait returns early
    with partial output (continuation waits; default 120 when ``wait_for_pattern``
    is set).
    ``token_kill`` / ``max_lines`` — rtk command rewriting/dedup and output-line
    capping (P-049).
    """
    try:
        # [CN-fork] P-061: kimi-style mode aliases + interactive sessions.
        if mode:
            background, interactive = _normalize_terminal_mode(mode, interactive, background)
        ...
        if interactive:
            # Persistent shell/REPL: force background + PTY so stdin stays open
            # for later continuation (terminal(command=..., process_id=<session_id>)).
            background, pty = True, True

        # [CN-fork] P-061: session continuation — send the command to an EXISTING
        # background process's stdin and wait for its new output. No env creation,
        # no security re-check: the process was already vetted at spawn.
        if process_id:
            return _continue_background_session(
                process_id=str(process_id),
                command=command,
                timeout=timeout or _get_env_config()["timeout"],
        ...
        plan = _plan_execution(
            ...
            interactive=interactive, promote_on_timeout=promote_on_timeout,
            wait_for_pattern=wait_for_pattern,
        )
        ...
        if background:
            ...
            # [CN-fork] P-058: report the background session's working directory
            # (the shell's resolved cwd) so later calls don't have to re-prefix `cd`.
            return _merge_result_metadata(result, cwd=_resolve_command_cwd(
        ...
        return _run_foreground(
            ...
            promote_on_timeout=promote_on_timeout, wait_for_pattern=wait_for_pattern,
            token_kill=token_kill, max_lines=max_lines, tool_call_id=tool_call_id,
        )
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "command": {
                "type": "string",
                "description": "The shell command to execute. Optional when mode='interactive' (defaults to a persistent shell session)."
            },
            # --- [CN-fork] P-061: mode / interactive / session continuation ---
            "mode": {
                "type": "string",
            ...
            # --- [CN-fork] P-061: foreground timeout / pattern promotion ---
            "promote_on_timeout": {
                "type": "boolean",
...
def _handle_terminal(args, **kw):
    ...
    # [CN-fork] P-061: kimi-style mode aliases (mode='background'/'interactive'
    # imply background) — resolved BEFORE the background-only flag guards below.
    background, interactive = _normalize_terminal_mode(
        args.get("mode"), bool(args.get("interactive", False)), args.get("background", False)
    )
...
registry.register(
    ...
    # [CN-fork] P-058: the description follows the shell the terminal will run in.
