...
def _check_requirements(env_type: str, config: Dict[str, Any]) -> bool:
    ...
    if "binary" in spec:
        ...
        # [CN-fork] P-042: short-lived version probes must not flash a console
        # window on Windows (windowless gateways / kanban workers).
        from hermes_cli._subprocess_compat import windows_hide_flags
        probe = subprocess.run([executable, arg], capture_output=True, timeout=5, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
