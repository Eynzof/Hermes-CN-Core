...
def _spawn(process_registry, *, env, env_type, command, cwd, effective_task_id, task_id,
    ...
    if env_type == "local":
        # [CN-fork] P-058: hand the wrapper a file to publish the final working
        # directory into (PowerShell `(Get-Location).Path`, bash `pwd`) so a later
        # foreground command picks up a background `cd`.
        cwd_file = None
