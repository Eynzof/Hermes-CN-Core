...
def _is_unusable_container_cwd(cwd: str) -> bool:
    ...
    Absoluteness is judged with **posixpath**, not ``os.path``: the sandbox is
    Linux, and on a Windows controller ``os.path.isabs("/workspace/task42")`` is
    False (ntpath wants a drive/UNC), which would reject every legitimate
    in-container cwd — including ``/workspace`` itself — and silently fall back
    to ``/root``.
    """
