...
from hermes_constants import get_hermes_home, get_hermes_home_override, get_managed_tools_dir, hermes_home_key
...
def _verify_checksum(archive_path: str, checksums_path: str, archive_name: str) -> bool:
    ...
    with open(checksums_path, encoding="utf-8", errors="replace") as f:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": $HERMES_HOME/tools, PATH, then $HERMES_HOME/bin."""
    hermes_tools = os.path.join(_hermes_tools_dir(), "tirith")
    if _is_executable(hermes_tools):
        return hermes_tools
