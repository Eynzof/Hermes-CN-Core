...
def estimate_tokens_from_schemas(tool_defs: Iterable[Dict[str, Any]]) -> int:
    ...
    def _chars(td: Dict[str, Any]) -> int:
        try:
            return len(orjson.dumps(td).decode('utf-8'))
        except (TypeError, ValueError):
            return len(str(td))
...
def dispatch_tool_search(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    return orjson.dumps({"queries": queries, "total_available": len(catalog) + remote_count, "results": results,
                       "tools": tools_map}).decode('utf-8')
