...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        return orjson.dumps(error_response(
            ...
            error_type="provider_not_registered", provider=configured)).decode('utf-8')
    return orjson.dumps(error_response(
        ...
        error_type="no_provider_configured")).decode('utf-8')
...
def _handle_video_generate(args: Dict[str, Any], **_kw: Any) -> str:
    ...
    def _err(error: str, error_type: str) -> str:
        return orjson.dumps(error_response(
            ...
            provider=getattr(provider, "name", ""), model=model or "", prompt=prompt)).decode('utf-8')
    try:
