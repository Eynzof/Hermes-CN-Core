...
def _load_jsonl(path: Path, on_error: Optional[Callable[[int, orjson.JSONDecodeError], None]] = None, start: int = 0) -> List[Tuple[int, Any]]:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, start):
            if not line.strip():
                continue
            try:
                entries.append((line_num, orjson.loads(line)))
