...
def _agent_cbs(sid: str) -> dict:
    ...
    callbacks = {
        # [CN-fork P-041] Emitted when the assistant message that requested the current batch of
        # tool calls has been appended to the conversation and is about to be executed. Gives the
        # UI a clean boundary for tool-call-only turns that would otherwise have no
        # ``message.delta`` between ``message.start`` and ``tool.start``.
        "tool_calls_committed_callback": lambda assistant_msg: _emit(
            "assistant.tool_calls_committed", sid,
            {
                ...
                "tool_call_ids": [
                    tc.get("id") for tc in (assistant_msg.get("tool_calls") or [])
                    if isinstance(tc, dict)
                ] if isinstance(assistant_msg, dict) else [],
                "has_content": bool(assistant_msg.get("content"))
                if isinstance(assistant_msg, dict) else False,
            }),
