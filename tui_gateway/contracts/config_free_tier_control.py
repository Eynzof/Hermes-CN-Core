...
# ── provider.probe / provider.models ([CN-fork] P-011 / P-036 / P-046) ────────────────────────
# Both handlers live in ``tui_gateway/server.py`` beside the shared ``_fetch_provider_model_ids``.


class ProviderProbeParams(Params):
    """``provider`` plus optional overrides for the settings-UI connectivity button. ``api_mode``
    ``"anthropic_messages"`` probes with the Anthropic protocol (x-api-key + ``{base}/v1/models``);
    anything else keeps the OpenAI-style bearer + ``/v1/models`` heuristics."""
...
class ProviderModelsParams(ProviderProbeParams):
    """Same params as ``provider.probe`` (``timeout_ms`` defaults to 8000 server-side)."""


class ProviderProbeResult(Result):
    """``_fetch_provider_model_ids`` sampled to 5 ids; failures are data (``ok=False``), not RPC errors."""
...
class ProviderModelsResult(Result):
    """The FULL ``/models`` id list (no sample truncation)."""
...
method("provider.probe", params=ProviderProbeParams, result=ProviderProbeResult,
      doc="Connectivity check against a provider's /models endpoint.")
method("provider.models", params=ProviderModelsParams, result=ProviderModelsResult,
      doc="Full model-id list for a provider (LAN/self-hosted picker refresh through the backend).")
