...
class ToolCallsCommittedPayload(Payload):
    """``agent_callbacks._agent_cbs`` tool_calls_committed_callback [CN-fork P-041]: the assistant
    message that requested the current batch of tool calls has been appended to the conversation and
    is about to be executed.  Gives the UI a boundary for tool-call-only turns, which otherwise have
    no ``message.delta`` between ``message.start`` and ``tool.start``."""
    role: str | None = None
...
event("assistant.tool_calls_committed", ToolCallsCommittedPayload,
      doc="Tool-call-only assistant message committed; the tool batch is about to run.")
