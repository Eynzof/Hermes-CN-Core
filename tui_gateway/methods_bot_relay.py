...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery,
    ...
    try:
        ...
        try:
            ...
            # The tempfile's fd belongs to the ``with os.fdopen(...)`` above, which closes it on every
            # normal path. When that wrapper (or a caller standing in for it) never closed it, the
            # number is still ours — and Windows cannot unlink a file that still has an open handle,
            # so the tempfile would leak. Close it here unless the fd was recycled by another open in
            # the meantime (inode mismatch) — the ``finally`` can run after a long relay turn.
            with contextlib.suppress(OSError, ValueError):
                fd_inode = os.fstat(fd).st_ino
                if fd_inode and os.stat(tmp).st_ino == fd_inode:
                    os.close(fd)
