...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # sources (``tool`` sub-agent runs, ``kanban`` dispatcher
            # workers, and desktop ``storm`` multi-agent role backends)
            # rather than allow-listing a fixed set of platform names
            # that goes stale whenever a new platform is added or a user names
            # their own source.
            deny = frozenset({"kanban", "tool", "storm"})
