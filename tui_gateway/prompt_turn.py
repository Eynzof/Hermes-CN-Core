...
def _invoke_agent(
    ...
    def _stream(delta):
        if getattr(agent, "_mute_notification_reply", False):
            return
        # [CN-fork P-041] ``delta`` can be ``None`` when the core agent loop flushes an open
        # stream before tool execution.  The gateway treats ``tool.start`` as the authoritative
        # transition, so a null delta must not become a ``message.delta {text: null}`` event.
        if delta is None:
            return
...
def _turn_watchdog_seconds() -> int:
    ...
    default.  ``0`` or a negative value disables the watchdog.  [CN-fork P-041]
    """
    try:
        cfg = _load_cfg() or {}
...
def _start_turn_watchdog(sid: str, session: dict) -> Callable[[], None]:
    ...
    [CN-fork P-041] — a wedged provider/tool must not leave the surface on a
    spinner with ``session["running"]`` stuck true forever.
    """
    ...
    def _watch() -> None:
        while not cancel_event.is_set():
            ...
            if time.time() - last < threshold:
                continue
            logger.warning(
                "Turn watchdog fired for session %s after %ss of inactivity", sid, threshold)
            try:
    ...
    # ``_REAL_THREAD`` (server.py) keeps this a real thread even when tests stub
    # ``threading.Thread`` for synchronous worker submission — otherwise the poll loop would run inline.
    thread = _REAL_THREAD(target=_watch, daemon=True, name=f"turn-watchdog-{sid}")
    thread.start()
...
def _run_prompt_submit(
    ...
    def run_body():
        ...
        goal_followup = None
        watchdog_cancel = None  # P-041: turn inactivity guard, armed for the whole turn
        try:
            ...
            # P-041: the turn is over — stop the inactivity guard before the rest of the
            # bookkeeping so it can never release a session that is already settling.
