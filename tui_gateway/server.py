...
def _(rid, params: dict) -> dict:
    try:
        ...
        # [CN-fork] slug_filter (optional list[str]) restricts which
        # canonical providers are emitted, so a region-specific client
        # (e.g. the CN desktop app) can avoid surfacing 20+ providers
        # its users will never configure. User-defined / custom providers
        # always pass through. Omit the param to preserve historical
        # behavior. Filtering is implemented in inventory.py:build_models_payload.
