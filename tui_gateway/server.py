...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # Spawn through the hidden ``__slash-worker`` CLI subcommand (P-063): the frozen CN
        # portable runtime has no standalone python, so ``[sys.executable, "-m", ...]`` would
        # run ``hermes -m tui_gateway.slash_worker`` and argparse rejects it as an invalid
        # subcommand. hermes_cli_argv() keeps the source-checkout shape (-m hermes_cli.main).
        argv = hermes_cli_argv("__slash-worker", "--session-key", session_key) + (["--model", model] if model else [])
        self._closed = False
...
def _emit(event: str, sid: str, payload: dict | None = None) -> bool:
    ...
    if event_presentation_muted(event, sid):
        return False
    # [CN-fork P-041] Keep the turn watchdog's inactivity detector alive.
...
def _broadcast_global_event(event: str, payload: dict | None = None) -> None:
    ...
    # A surface-global event belongs to no session, so it traces under the empty sid
    # (the turn-activity clock is kept alive in ``_emit`` for session-scoped events).
...
# ---------------------------------------------------------------------------
# [CN-fork] P-011 / P-036 / P-046 — provider probe + full model-list RPCs.
# `provider.probe` (`hermes_cli/AGENTS` 測試連接) and `provider.models` (LAN/self-hosted
# model-picker refresh, so a private IP is fetched from the BACKEND instead of the desktop's
# SSRF-guarded proxy) share `_fetch_provider_model_ids`; `api_mode="anthropic_messages"`
# switches both to the Anthropic protocol (x-api-key + `/v1/models` candidates) for the CN
# Claude-Code relays. `hermes_embedded/api.py` and the desktop call these names.
# ---------------------------------------------------------------------------
