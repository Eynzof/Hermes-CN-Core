...
def _wire_desktop_sinks() -> None:
    ...
    # P-047/P-054: the CLI-delegation tracker emits its own events through this module's emitter, and
    # foreground delegations stream through the same sink terminal_tool already writes to. Idempotent
    # (configure/reset are single-slot), so wiring more than once is harmless.
    with contextlib.suppress(Exception):
        from tui_gateway import cli_delegation
        cli_delegation.tracker.configure(emit=_emit, is_alive=lambda sid: sid in _sessions)
    ...
    def _emit_process_output(session, chunk) -> None:
        _emit("agent.terminal.output", _owner_sid(session), {"process_id": session.id, "chunk": chunk})
        # P-047: second consumer on the same sink — a background CLI delegation buffers its chunks here
        # and the tracker's watcher thread coalesces them into delegation.cli.output.
        with contextlib.suppress(Exception):
            from tui_gateway import cli_delegation
            cli_delegation.tracker.on_chunk(session, chunk)
    if getattr(process_registry, "on_output", None) is None:
        process_registry.on_output = _emit_process_output
