...
def _on_tool_start(sid: str, tool_call_id: str, name: str, args: dict):
    ...
    if (_tool_progress_enabled(sid) or _tool_lifecycle_required_for_ui(name)
        ...
        # P-047: a ``terminal`` call that delegates to Claude Code / Codex also emits
        # ``delegation.cli.started`` (delegation_id == tool_id, so clients upgrade the existing tool card
        # instead of double-rendering). Gated with the tool lifecycle above: with tool progress off the
        # delegation events stay silent too. The tracker classifies; plain terminal commands stay silent.
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    ...
    # P-047: close the tracked CLI delegation — a foreground one goes straight to a terminal state
    # (session_id/num_turns/cost parsed out of the result), a background one binds its process session
    # and lets the watcher thread stream the terminal. Both gated on tool progress, like the fork.
    if _tool_progress_enabled(sid):
        with contextlib.suppress(Exception):
            from tui_gateway import cli_delegation
            cli_delegation.tracker.handle_tool_complete(sid, tool_call_id, name, payload.get("result"))
