...
def _atomic_write(path: Path, write, *, prefix: str, encoding: str = "utf-8", mode: "int | None" = None,
    ...
    *write(f)* emits the payload into the open handle (text, or bytes when *binary*). The temp file
    ...
    try:
        # [CN-fork] newline="": preserve the caller's line endings byte-for-byte. With
        # newline=None, TextIOWrapper rewrites every \n to os.linesep on Windows; content
        # that already contains CRLF then becomes CRCRLF. Binary mode rejects the kwarg.
        with os.fdopen(fd, "wb" if binary else "w", encoding=None if binary else encoding,
                       **({} if binary else {"newline": ""})) as f:
