...
| `HERMES_SHELL_TYPE` | **Windows only.** Select the terminal shell: `auto` (default → Git Bash → PowerShell 7 → Windows PowerShell 5.1), `pwsh` / `powershell` (explicit PowerShell, never Git Bash), `bash` (explicit Git Bash, requires pre-installed Git for Windows). Non-Windows always uses `bash`. See the [Windows (Native) Guide](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
