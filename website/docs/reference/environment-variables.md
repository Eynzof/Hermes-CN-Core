...
| `HERMES_SHELL_TYPE` | **Windows only.** Select the terminal shell: `auto` (default → PowerShell 5.1), `powershell` (explicit). `bash` is not supported on Windows. Non-Windows always uses `bash`. See the [Windows (Native) Guide](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
