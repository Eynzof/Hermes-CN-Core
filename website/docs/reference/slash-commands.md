...
| `/swarm [on\|off\|<prompt>]` (alias: `/sw`) | Toggle swarm mode for parallel subagent dispatch. /swarm on keeps it enabled across turns so the agent can fan a task out to parallel subagents; /swarm off disables it; /swarm <prompt> enables it for that one task turn only and exits automatically when the turn ends. Bare /swarm reports the current state and trigger. |
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | Toggle the optional [Codex app-server runtime](../user-guide/features/codex-app-server-runtime) for OpenAI/Codex models and named custom providers that are also defined in `~/.codex/config.toml`. `auto` (default) uses Hermes' standard chat completions; `codex_app_server` hands eligible turns to a `codex app-server` subprocess for native shell, apply_patch, ChatGPT subscription auth, and migrated Codex plugins. Effective on next session. |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.hermes/config.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
