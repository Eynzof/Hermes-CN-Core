...
| `HERMES_SHELL_TYPE` | **仅 Windows。** 选择终端 shell：`auto`（默认→Git Bash → PowerShell 7 → Windows PowerShell 5.1）、`pwsh` / `powershell`（显式指定 PowerShell，绝不使用 Git Bash）、`bash`（显式指定 Git Bash，需要预装 Git for Windows）。非 Windows 始终使用 `bash`。参见 [Windows（原生）指南](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
