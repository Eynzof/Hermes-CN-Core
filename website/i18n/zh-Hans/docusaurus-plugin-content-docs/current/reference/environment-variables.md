...
| `HERMES_SHELL_TYPE` | **仅 Windows。** 选择终端 shell：`auto`（默认→PowerShell 5.1）、`powershell`（显式指定）。Windows 不支持 `bash`。非 Windows 始终使用 `bash`。参见 [Windows（原生）指南](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
